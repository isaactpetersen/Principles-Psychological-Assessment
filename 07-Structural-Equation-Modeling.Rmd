...
In Section \@ref(semModel-sem), I present an example of a SEM model.
...
In a reflective model, the answers that answers people give on the items are thought to reflect a certain disposition, in this case, extraversion.
...
[Formative construct](#formativeConstruct) are challenging to use, and must be used in the context of a model that allows some constraints.
...
You plan to assess stress with three methods (self-report, friend report, cortisol), harsh parenting with four methods (parents' self-report, spousal report, child report, observation), and children's antisocial behavior with four methods (parent report, teacher report, child report, observation).
	...
	Use robust standard errors to account for non-normally distributed data.
	Using a seed of 52242, an alpha level of .05, and 2 repetitions per sample size, in the `simsem` package:
    ...
    d. Re-run the power analysis with normally distributed values (skewness $= 0$, kurtosis $= 0$).
	With a sample size of 150, did power to detect the hypothesized effects increase or decrease?
What does this indicate?
    e. Re-run the power analysis using multiple imputation instead of FIML (and normally distributed values).
	Use five imputations.
	With a sample size of 150, did power to detect the hypothesized effects increase or decrease?
What does this indicate?
