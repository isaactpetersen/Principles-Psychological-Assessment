...
The `petersenlab` package [@R-petersenlab] includes a [`complement()` function](https://stats.stackexchange.com/a/313138/20338) that simulates data with a specified correlation in relation to an existing variable.\index{petersenlab}
...
The measurement model can also specify correlated residuals.\index{residuals!correlated}\index{structural equation modeling!measurement model}
...
SEM is flexible in allowing you to specify [measurement error](#measurementError) and correlated errors.\index{structural equation modeling}\index{measurement error}\index{residuals!correlated}
...
All things equal (i.e., in terms of model fit with the same number of manifest variables), a model with more degrees of freedom is preferred for its parsimony, because fewer parameters are estimated.\index{structural equation modeling!degrees of freedom}
...
Up to this point, we have discussed SEM with continuous constructs.\index{continuous}
It also worth knowing about additional types of SEM models, including latent class models and mixture models, that handle categorical constructs.\index{categorical}\index{structural equation modeling!latent class model}\index{structural equation modeling!mixture model}
However, most disorders are more accurately conceptualized as dimensional than as categorical [@Markon2011], so just because you can estimate categorical latent factors does not necessarily mean that one should.\index{continuous}\index{categorical}\index{structural equation modeling!latent class model}\index{structural equation modeling!mixture model}

### Latent Class Models {#latentClassModels}

In *latent class models*, the construct is not continuous, but rather categorical.\index{categorical}\index{structural equation modeling!latent class model}\index{continuous}
The categorical constructs are latent classifications and are called latent classes.\index{categorical}\index{structural equation modeling!latent class model}
For instance, the construct could be a diagnosis that influences scores on the measures.\index{categorical}\index{structural equation modeling!latent class model\index{diagnosis}
Latent class models examine qualitative differences in kind, rather than quantitative differences in degree.\index{categorical}\index{structural equation modeling!latent class model\index{diagnosis}

### Mixture Models {#MixtureModels}

*Mixture models* allow for a combination of latent categorical constructs (classes) and latent continuous constructs.\index{continuous}\index{categorical}\index{structural equation modeling!mixture model}
That is, it allows for both qualitative and quantitative differences.\index{continuous}\index{categorical}\index{structural equation modeling!mixture model}
However, this additional model complexity also necessitates a larger sample size for estimation.\index{structural equation modeling!mixture model}
SEM generally requires a 3-digit sample size ($N = 100+$), whereas mixture models typically require a 4- or 5-digit sample size ($N = 1,000+$).\index{structural equation modeling!mixture model}
...
Parsimony fit include fit indices that use information criteria fit indices, including the the Akaike Information Criterion (AIC) and the Bayesian Information Criterion (BIC).\index{structural equation modeling!fit index}
...
Here are correlation matrices of various types using the `cor.table()` function from the `petersenlab` package [@R-petersenlab]:\index{petersenlab}
...
For instance, the modification indices below indicate a few additional factor loadings (i.e., cross loadings) or correlated residuals that could substantially improve model fit.\index{residuals!correlated}\index{cross loading}\index{structural equation modeling!fit index}\index{structural equation modeling!modification indices}
...
Model fit of nested models can be compared with a chi-square difference test.\index{chi-square difference test}\index{structural equation modeling!model comparison}

```{r}
anova(cfaModelFit, cfaModel2Fit)
```

In this case, the model with the cross loading fits significantly better (i.e., has a significantly smaller chi-square value) than the model without the cross loading.\index{cross loading}\index{structural equation modeling!model comparison}
...
With SEM, you do not need to make as restrictive assumptions as in [classical test theory](#ctt).\index{classical test theory}\index{structural equation modeling}\index{residuals!correlated}
...
Statistical power for these more straightforward analytical approaches can be estimated in G*Power [@Faul2009]:\index{power analysis} https://www.psychologie.hhu.de/arbeitsgruppen/allgemeine-psychologie-und-arbeitspsychologie/gpower.html
...
2. Generate a large number (e.g., 1,000) of random samples based on the hypothesized model and population values specified;\index{power analysis}\index{structural equation modeling!power analysis}
3. Fit a SEM model to each of the generated samples, and for each one, record whether the target parameter is significantly different from zero;\index{power analysis}\index{structural equation modeling!power analysis}
4. Calculate power as the proportion of simulated samples that produce a statistically significant estimate of the target parameter.\index{power analysis}\index{structural equation modeling!power analysis}
...
The reader is referred to examples by @Vispoel2018.\index{generalizability theory}\index{structural equation modeling}

## Conclusion {#conclusion-sem}

Structural equation modeling (SEM) is an advanced modeling approach that allows estimating latent variables as the common variance from multiple measures.\index{structural equation modeling}\index{latent variable}
