...
However, most disorders are more accurately conceptualized as dimensional than as categorical [@Markon2011], so just because you can estimate categorical latent factors does not necessarily mean that one should.\index{dimensional}\index{categorical}\index{structural equation modeling!latent class model}\index{structural equation modeling!mixture model}

### Latent Class Models {#latentClassModels}

In *latent class models*, the construct is not dimensional, but rather categorical.\index{categorical}\index{structural equation modeling!latent class model}\index{dimensional}
...
*Mixture models* allow for a combination of latent categorical constructs (classes) and latent dimensional constructs.\index{dimensional}\index{categorical}\index{structural equation modeling!mixture model}
That is, it allows for both qualitative and quantitative differences.\index{dimensional}\index{categorical}\index{structural equation modeling!mixture model}
