...
Categorical data are often dummy-coded into binary (i.e., dichotomous) variables, which represent nominal data.\index{data!nominal}
...
For instance, the person who finished the race first might have finished 10 minutes before the second-place finisher; whereas the 3rd-place finisher might have finished 1 second after the second-place finisher.\index{data!ordinal}
...
With deviation scores, you have similar challenges as with percentile ranks, including which reference group to use, but there are additional challenges.\index{data!standardized}\index{data!percentile rank}
...
Thus, you can compute a deviation score, but it may not be meaningful if the data and underlying construct are not normally distributed.\index{data!standardized}
...
*z* scores have a mean of zero and a standard deviation of one.\index{data!\textit{z} score}
To get a *z* score that uses the sample as its own norm, subtract the mean from all scores and divide by the standard deviation.\index{data!\textit{z} score}
...
68% of scores fall within one standard deviation of the mean.\index{data!\textit{z} score}
95% of scores fall within two standard deviations of the mean.\index{data!\textit{z} score}
99.7% of scores fall within three standard deviations of the mean.\index{data!\textit{z} score}
...
Alternatively, if you want to determine the *z* score associated with a particular percentile in a normal distribution, you can use the `qnorm()` function.\index{data!\textit{z} score}\index{data!percentile rank}
...
*z* scores are calculated using Equation \@ref(eq:zScore):\index{data!\textit{z} score}
...
An example histogram of *z* scores is in Figure \@ref(fig:zScores).\index{data!\textit{z} score}\index{histogram}
...
For the Minnesota Multiphasic Personality Inventory (MMPI), you would examine peaks (elevations $\ge$ 70) and absences ($\le$ 30).\index{data!\textit{T} score}\index{Minnesotal Multiphasic Personality Inventory}
...
Standard scores are frequently used for tests of intellectual ability, academic achievement, and cognitive ability.\index{data!standard score}\index{intellectual assessment}\index{achievement testing}
Intellectual disability is generally considered an I.Q. standard score less than 70 (two standard deviations below the mean), whereas giftedness is at 130 (two standard deviations above the mean).\index{data!standard score}\index{intellectual assessment}
...
An example histogram of scaled scores is in Figure \@ref(fig:scaledScores).\index{data!scaled score}\index{histogram}
...
The scores range from 1–9.\index{data!stanine score}
Stanine scores are calculated using the bracketed proportions in Table \@ref(tab:stanineCalculation).\index{data!stanine score}
...
A histogram of stanine scores is in Figure \@ref(fig:stanineScores).\index{data!stanine score}{data!histogram}
