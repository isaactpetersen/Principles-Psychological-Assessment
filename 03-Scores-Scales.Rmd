...
For this example, I simulate data below.\index{simulate data}
...
Deviation scores are more informative when the scores are normally distributed compared to when the scores are skewed.\index{data!standardized}\index{data!nonnormal distribution}
...
Many constructs we study in psychology are not normally distributed.\index{data!nonnormal distribution}
For example, the frequency of hallucinations among people would show a positively skewed distribution with a truncation at zero, representing a floor effect—i.e., most people do not show hallucinations.\index{data!nonnormal distribution}
For instance, consider a hypothetical distribution of hallucinations.
It might follow a folded distribution, as depicted in Figure \@ref(fig:foldedDistribution):\index{data!nonnormal distribution}\index{histogram}
...
Thus, you can compute a deviation score, but it may not be meaningful if the data and underlying construct are not normally distributed.\index{data!standardized}\index{data!nonnormal distribution}

##### *z* scores {#zScores}

The *z* score is the most common standardized score, and it can help put different measures with different scales on the same playing field.\index{data!standardized}\index{data!\textit{z} score}
...
The area under a normal curve within thee standard deviations of the mean is calculated below:\index{data!standardized}
...
On tests of intellectual or cognitive ability, scaled scores commonly have a mean of 10 and a standard deviation of three.\index{data!standardized}\index{intellectual assessment}\index{data!scaled score}
...
Stanine scores (short for STANdard Nine), have a mean of five and a standard deviation of two.\index{data!standardized}\index{data!stanine score}
...
A histogram of stanine scores is in Figure \@ref(fig:stanineScores).\index{data!standardized}\index{data!stanine score}{data!histogram}
