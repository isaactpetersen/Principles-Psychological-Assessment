...
The organization of types of measurement validity that are subsumed by construct validity is depicted in Figure \@ref(fig:constructValidityOrganization).
...
[Reliability](#reliability) and validity are not the same thing. Reliability deals with consistency whereas validity deals with accuracy. A measure can be consistent but not accurate (see Figure \@ref(fig:reliabilityVsValidity2)). That is, a measure can be reliable but not valid. However, a measure cannot be accurate if it is inconsistent; that is, a measure cannot be valid and unreliable.
...
The typical way of depicting the distinction between [reliability](#reliability) and validity is in Figure \@ref(fig:reliabilityVsValidity2), in which a measure can either have (a) low reliability and low validity, (b) high reliability and low validity, or (c) high reliability and high validity. However, it can be worth thinking about validity in terms of accuracy at the person-level versus group-level. When we distinguish between person- versus group-level accuracy, we can distinguish four general combinations of [reliability](#reliability) and validity, as depicted in Figure \@ref(fig:reliabilityVsValidity1)): (a) low reliability, low accuracy at person-level, and low accuracy at group-level, (b) low reliability, low accuracy at person-level, and high accuracy at group-level, (c) high reliability, low accuracy at person-level, and low accuracy at group-level, and (d) high reliability, high accuracy at person-level, and high accuracy at group-level. However, as discussed before, [reliability](#reliability) and validity are not binary states of low versus high—they exist to varying degrees.
...
As described above, (random) measurement error weakens (or attenuates) the association between variables [@Goodwin2006; @Schmidt1996]. The greater the random measurement error, the weaker the association. Thus, the correlation between $x$ and $y$ depends on both the true correlation of x and y ($r_{x_{t}y_{t}}$) and the reliabilities of x ($r_{xx}$) and y ($r_{yy}$). So, measurement error in $x$ and $y$ can reduce the observed correlation below the true correlation. This is known as the *attenuation formula*:
...
All of these $r$ values (excluding the true correlation) are just estimates unless the sample size is infinite—so, the observed association is going to be an imperfect measure. Hence, we need a correction for this attenuation [@Schmidt1996]. This correction for the attenuation of an association due to measurement error (unreliability) is known as the disattenuation of a correlation, i.e., correction for the attenuation of an association due to measurement error to get a more accurate estimate of the true association between constructs. Rearranging the terms from the attenuation formula, the formula for disattenuation of a correlation (i.e., the *disattenuation formula*) is as follows:
...
The disattenuation of an observed correlation due to measurement error can be demonstrated using [structural equation modeling](#sem):
