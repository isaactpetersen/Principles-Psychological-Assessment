...
There already exist many resources that summarize and compare the [reliability](#sec-reliability) and [validity](#sec-validity) of measures in psychology [@Buros2021].\index{reliability}\index{validity}
...
The chapters provide an overview of topics—each of which could have its own class and textbook, such as [structural equation modeling](#sec-sem), [item response theory](#sec-irt), [generalizability theory](#sec-gTheory), [factor analysis](#sec-factor-analysis-PCA), [prediction](#sec-prediction), [cognitive assessment](#sec-cognition), [psychophysiological assessment](#sec-psychophysiological), etc.
...
## About the Author {#sec-aboutTheAuthor}
...
## What Is Assessment? {#sec-whatIsAssessment}

Measurement is the principled assignment of numbers to objects or events [@Stevens1946].
Assessment is a broader process that integrates measurement along with interpretation and decision-making.
Assessment involves gathering of information about a person, group, setting, or context.\index{assessment!what is}
...
## Why Should We Care About Assessment (and Science)? {#sec-whyCare}
...
### Assessment and the Replication Crisis in Science {#sec-assessmentReplicationCrisis}
...
This is similar to @Gelman2013's description of research as the garden of forking paths, where different decisions along the way can lead to different outcomes (see @fig-gardenForkingPaths).\index{replication}\index{replication!failure to replicate}
...
::: {#fig-gardenForkingPaths}
![](images/gardenForkingPaths.png){width=100% fig-alt="Garden of Forking Paths. (Adapted from <https://www.si.umich.edu/about-umsi/news/ditch-stale-pdf-making-research-papers-interactive-and-more-transparent> [archived at <https://perma.cc/R2V9-CP3F>].)"}
...
The field has paid insufficient attention to measurement [unreliability](#sec-reliability) as a key culprit in the replication crisis.\index{reliability}
As @Loken2017 demonstrated, when measures are less noisy, [measurement error](#sec-measurementError) weakens the association between the measures.\index{measurement error}
But when using noisy measures and selecting what to publish based on statistical significance, [measurement error](#sec-measurementError) can make the association appear stronger than it is.\index{measurement error}\index{statistical significance!filter}
...
However, the effect of noise ([measurement error](#sec-measurementError)) diminishes as the sample size increases.\index{effect size}\index{statistical significance!filter}
...
And, as discussed in @sec-ethics on ethical considerations in psychological assessment, the use of pre-registration could be useful to control researcher degrees of freedom.\index{ethics}\index{pre-registration}
...
Psychometrics refer to the [reliability](#sec-reliability) and [validity](#sec-validity) of measures.\index{reliability}\index{validity}
These concepts are described in greater detail in Chapters [-@sec-reliability] and [-@sec-validity], but for now, think about [reliability](#sec-reliability) as *consistency* of measurement and [validity](#sec-validity) as *accuracy* of measurement.\index{reliability}\index{validity}

### Science Versus Pseudoscience in Assessment {#sec-scienceVsPseudoscience}
...
[Ethically](#sec-ethics), you should be making these predictions based on science and a risk-benefit analysis.\index{ethics}\index{science}\index{clinical science}
...
1. Scientifically supported measurement strategies are used based on their psychometrics, including [reliability](#sec-reliability) and [validity](#sec-validity).\index{science}\index{reliability}\index{validity}

Science does not progress without advances in measurement, including\index{science}

- more efficient measurement (see Chapters [-@sec-irt] and [-@sec-cat])\index{brief assessment}
- more [precise](#sec-reliability) measurement (i.e., reliability; see @sec-reliability)\index{reliability}
- more [accurate](#sec-validity) measurement (i.e., validity; see @sec-validity)\index{validity}
- more sophisticated modeling (see @sec-cognition)
- more sophisticated [biopsychological](#sec-psychophysiological) (e.g., cognitive neuroscience) techniques, as opposed to self-report and neuropsychological techniques (see @sec-psychophysiological)\index{psychophysiological assessment}
- considerations of cultural and individual [diversity](#sec-diversity) (see @sec-diversity)\index{diversity}
- [ethical](#sec-ethics) considerations (see @sec-ethics)\index{ethics}

These considerations serve as the focus of this book.

## Prerequisites {#sec-prerequisites}

This book was written using `Quarto` [@Quarto] in `R` [@R-base].\index{R}
...
To that aim, I provide information—in each chapter using R—on my setup that was used to generate the results found in this book.\index{reproducibility}
