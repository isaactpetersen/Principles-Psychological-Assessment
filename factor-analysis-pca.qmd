...
As discussed in Section \@ref(factorAnalysisDecisions), however, there are many decisions to make in [factor analysis](#sec-factorAnalysis), in addition to questions about which variables to use, how to scale the variables, etc. [@Floyd1995; @Sarstedt2024].\index{factor analysis}
...
However, [multitrait-multimethod](#sec-MTMM) approaches to [factor analysis](#sec-factorAnalysis) (such as in Section \@ref(mtmmCFA)) help better partition the variance in variables that reflects method variance versus construct variance, to get more accurate estimates of constructs.\index{factor analysis}\index{multitrait-multimethod matrix}
...
Analytical examples of [factor analysis](#sec-factorAnalysis) are presented in Section \@ref(factorAnalysisExamples).\index{factor analysis}
...
And, using latent factors in [SEM](#sec-sem) helps disattenuate associations for measurement error, as described in Section \@ref(disattenuation).
...
I introduced [exploratory factor analysis](#sec-efa) (EFA) models in Section \@ref(efa).\index{factor analysis!exploratory}
...
I introduced [confirmatory factor analysis](#sec-cfa) (CFA) models in Section \@ref(cfa-sem) in the chapter on [structural equation models](#sec-sem).\index{factor analysis!confirmatory}
...
We simulated the data for the bifactor model in Section \@ref(mtmmCorrelationMatrix).
...
We simulated the data for the [multitrait-multimethod](#sec-MTMM) model (MTMM) in Section \@ref(mtmmCorrelationMatrix).\index{simulate data}\index{multitrait-multimethod matrix}\index{factor analysis!confirmatory}
...
In this example, I use the factor loadings from the three-factor [EFA](#sec-efa) model, as specified in Section \@ref(obliqueLavaan-efa).\index{structural equation modeling!exploratory}
