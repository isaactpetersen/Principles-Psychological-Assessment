...
Before pursuing [factor analysis](#factorAnalysis), it is helpful to evaluate the extent to which the variables are intercorrelated.\index{factor analysis}
If the variables are not correlated (or are correlated only weakly), there is no reason to believe that a common factor influences them, and thus [factor analysis](#factorAnalysis) would not be useful.\index{factor analysis}
So, before conducting a [factor analysis](#factorAnalysis), it is important to examine the correlation matrix of the variables to determine whether the variables intercorrelate strongly enough to justify a [factor analysis](#factorAnalysis).\index{factor analysis}
...
Equivalent models are different models with the same degrees of freedom that have the same fit to the data—i.e., they generate the same model-implied covariance matrix and have the same [fit indices](#modelFitIndices-sem).\index{factor analysis!indeterminacy}
...
Before performing a [PCA](#pca), it is generally important to ensure that the variables included in the PCA are on the same scale.\index{factor analysis!decisions}\index{principal component analysis}
...
A common way of ensuring that variables are on the same scale is to standardize them using, for example,  [*z*-scores](#zScores) that have a mean of zero and standard deviation of one.\index{factor analysis!decisions}\index{data!standardized}\index{data!\textit{z} score}
By contrast, [factor analysis](#factorAnalysis) can better accommodate variables that are on different scales.\index{factor analysis!decisions}\index{factor analysis}
...
In reality, part of the unique variance is [random error](#randomError) and part of it is specific variance—systematic variance unique to the item, which may reflect either a unique facet of the construct or [systematic error](#systematicError) such as [method bias](#methodBias) that is specific to the item.\index{factor analysis!decisions}\index{factor analysis}\index{measurement error}\index{systematic error}\index{method bias}
...
However, the common variance includes all variance that is shared across all variables, including construct variance (i.e., [true score](#cttOverview) variance) and [systematic error](#systematicError) such as [method bias](#methodBias).\index{factor analysis!decisions}\index{factor analysis}\index{method bias}\index{true score}
...
Moreover, our initial hypothesized [CFA](#cfa) model is often not a good fit to the data—especially for models with many items [@Floyd1995]—and requires modification for adequate fit, such as cross loadings and correlated residuals.\index{factor analysis!decisions}\index{factor analysis!exploratory}\index{factor analysis!confirmatory}
Such modifications are often made based on empirical criteria (e.g., [modification indices](#cfaModelModIndices-sem)) rather than our initial hypothesis.\index{factor analysis!decisions}\index{factor analysis!exploratory}\index{factor analysis!confirmatory}
In [EFA](#efa), researchers often set constraints and assumptions.
Moreover, the initial items for the [EFA](#efa) were selected by the researcher, who often has expectations about the number and types of factors that will be extracted.\index{factor analysis!decisions}\index{factor analysis!exploratory}\index{factor analysis!confirmatory}
...
- Kaiser-Guttman criterion [@Kaiser1960]: in [PCA](#pca), components with eigenvalues greater than 1\index{factor analysis!decisions}\index{principal component analysis}\index{eigenvalue}
...
A non-positive definite matrix could also be caused by a variety of things such as [multicollinearity](#multiCollinearity) or singularity, variables with zero variance, too many variables relative to the sample size, or too many factors.\index{factor analysis}\index{factor analysis!improper solution}
...
The determinant of a correlation matrix can be used to evaluate the potential for [multicollinearity](#multiCollinearity), which can be problematic in factor analysis.
...
To specify an [exploratory structural equation model](#efa-cfa-esem) (ESEM), you use the factor loadings from an [EFA](#efa) model as the starting values in a [structural equation model](#sem).\index{structural equation modeling!exploratory}\index{factor analysis!exploratory}
...
That is, [PCA](#pca) seeks to identify the smallest set of components that explain the most variance in the variables.\index{principal component analysis}
...
First, factor analysis can be used for data reduction, similar to [PCA](#pca).\index{factor analysis}\index{factor analysis!benefits}\index{principal component analysis}\index{data!reduction}
...
In addition, this data reduction can help to address [multicollinearity](#multiCollinearity) issues in regression analyses.\index{factor analysis}\index{factor analysis!benefits}\index{data!reduction}\index{multicollinearity}
