...
[PCA](#pca) is a technique for data reduction, whereas [factor analysis](#factorAnalysis) uses latent variables and can be used to identify the structure of measures/constructs or for data reduction. Many people use [PCA](#pca) when they should use [factor analysis](#factorAnalysis) instead, such as when they are assessing latent constructs. [Factor analysis](#factorAnalysis) has [indeterminacy](#indeterminacy)—i.e., a given data matrix can produce many different factor models, and you cannot determine which one is correct based on the data matrix alone. There are many [decisions to make in factor analysis](#factorAnalysisDecisions). These decisions can have important impacts on the resulting solution. Thus, it can be helpful for theory and interpretability to help guide the decision making.
