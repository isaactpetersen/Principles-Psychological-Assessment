...
pB(pBgivenA = .95, pA = pAgivenB(pBgivenA = .95, pA = .003, pB = .01),
   pBgivenNotA = pBgivenNotA(pBgivenA = .95, pA = .003, pB = .01))
```

We then substitute the updated [marginal probability](#baseRate) of HIV ($P(C_i)$) and the updated [marginal probability](#baseRate) of getting a second positive test ($P(R_i)$) into Bayes' theorem to get the probability that the person has HIV if they have a second positive test (assuming the errors of each test are independent, i.e., uncorrelated), as in Equation \@ref(eq:baseRateUpdated2):
...
pAgivenBalternative(pBgivenA = .95, pA = .003,
                    pBgivenNotA = pBgivenNotA(pBgivenA = .95, pA = .003, pB = .01))
```
...
```{r}
pAgivenBalternative(pBgivenA = .95, pA = .285,
                    pBgivenNotA = .007171515)
pAgivenBalternative(pBgivenA = .95, pA = .285,
                    pBgivenNotA = pBgivenNotA(pBgivenA = .95, pA = .003, pB = .01))

pAgivenB(pBgivenA = .95, pA = .285,
         pBgivenNotA = .007171515)
pAgivenB(pBgivenA = .95, pA = .285,
         pBgivenNotA = pBgivenNotA(pBgivenA = .95, pA = .003, pB = .01))
```
...
The estimates eneral prediction accuracy are separated below into [scale-dependent](#scaleDependentAccuracy) and [scale-independent](#scaleIndependentAccuracy) accuracy estimates.
...
The formula for mean absolute error is in Equation \@ref(eq:meanAbsoluteError):
...
Smaller mean squared error (MSE) values (closer to zero) reflect greater accuracy.
The formula for mean squared error is in Equation \@ref(eq:meanSquaredError):
...
Smaller root mean squared error (RMSE) values (closer to zero) reflect greater accuracy.
RMSE penalizes larger errors more so than MAE does.
The formula for root mean squared error is in Equation \@ref(eq:rootMeanSquaredError):
...
We then take the mean of the percentage errors to get mean percentage error.
The formula for mean percentage error is in Equation \@ref(eq:meanPercentageError):
...
The formula for mean absolute percentage error is in Equation \@ref(eq:meanAbsolutePercentageError):
...
The formula for symmetric mean absolute percentage error is in Equation \@ref(eq:symmetricMeanAbsolutePercentageError):
...
Then, we calculate the mean of the absolute value of the scaled errors to get mean absolute scaled error, as in Equation \@ref(eq:meanAbsoluteScaledError):
...
To re-calibrate the predicted probabilities, it would be necessary to necessary to increase them so they are consistent with observed probabilties.
