...
```{r}
accuracyOverall(predicted = mydataSDT$testScore,
                actual = mydataSDT$disorder) %>% t

accuracyOverall(predicted = mydataSDT$testScore, 
                actual = mydataSDT$disorder,
                dropUndefined = TRUE) %>% t
```
...
The predictor (`testScore`) explains $`r round(rSquaredValue * 100, 2)`$% of the variance $(R^2 = `r round(rSquaredValue, 2, leading = FALSE)`)$ in the outcome (`disorder` status).
...
```{r}
accuracyAtCutoff(predicted = mydataSDT$testScore,
                 actual = mydataSDT$disorder,
                 cutoff = cutoff) %>% t
```
