...
#From here: https://perma.cc/F6F9-VG2K
...
The adapted formula for MASE with non-time series data is described [here (https://perma.cc/G469-8NAJ)](https://perma.cc/G469-8NAJ).\index{mean absolute scaled error}
...
Multiple regression can be conceptualized with overlapping circles in what is called a [Ballantine graph](https://perma.cc/C7CU-KFVG).\index{multiple regression}
...
Predictive $R^2$ is described here:\index{predictive $R^2$} https://perma.cc/BK8J-HFUK.
...
This calibration plot was adapted from Darren Dahly's example here:\index{calibration!plot} https://perma.cc/6J3J-69G7
...
Instead, [multicollinearity increases the uncertainty (i.e., standard errors) around the parameter estimates](https://perma.cc/DJ7L-TCUK), which makes it more challenging to detect an effect as statistically significant.\index{multicollinearity}\index{multiple regression}\index{uncertainty}
[Some forms of multicollinearity are ignorable](https://perma.cc/2JV5-2QEZ), including when the multicollinearity is among (a) the control variables rather than the variables of interest, (b) the the powers (e.g., quadratic term) or products (e.g., interaction term) of other variables, or (c) dummy-coded categories.\index{multicollinearity}\index{multiple regression}
Ultimately, [it is important to examine the question of interest, even if that means inclusion of predictors that are inter-correlated in a regression model](https://perma.cc/DJ7L-TCUK).\index{multicollinearity}\index{multiple regression}
...
The data are publicly available (https://perma.cc/B4AV-YH4V).
