...
Unlike MAPE, symmetric mean asbolute percentage error (sMAPE) is symmetric because it equally weights underestimates and overestimates.\index{symmetric mean asbolute percentage error}
Smaller sMAPE values (closer to zero) reflect greater accuracy.\index{symmetric mean asbolute percentage error}
...
This is known as the root mean squared log error (RMSLE), and is in Equation \@ref(eq:rootMeanSquaredLogError):\index{root mean squared log error}
...
When predicted values are -1, there is a zero in the numerator of the accuracy ratio, which leads to $log(0)$, which is undefined.\index{root mean squared log error}
When actual values are -1, there is division by zero in the accuracy ratio.
...
Adjusted $R^2$ is similar to the [coefficient of determination](#coefficientOfDetermination), but it accounts for the number of predictors included in the regression model to penalize [overfitting](#overfitting).\index{adjusted $R^2$}\index{multiple regression}\index{overfitting}
...
Predictive $R^2$ penalizes [overfitting](#overfitting), unlike traditional $R^2$.\index{predictive $R^2$}\index{coefficient of determination}\index{overfitting}
