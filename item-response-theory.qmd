...
In the chapter on [reliability](#sec-reliability), we introduced [classical test theory](#sec-ctt).\index{reliability}\index{classical test theory}
[Classical test theory](#sec-ctt) is a measurement theory of how test scores relate to a construct.\index{classical test theory}
[Classical test theory](#sec-ctt) provides a way to estimate the relation between the measure (or item) and the construct.\index{classical test theory}
For instance, with a [classical test theory](#sec-ctt) approach, to estimate the relation between an item and the construct, you would compute an [item–total correlation](#sec-itemTotalCorrelation-reliability).\index{classical test theory}\index{reliability!internal consistency!average item–total correlation}
An [item–total correlation](#sec-itemTotalCorrelation-reliability) is the correlation of an item with the total score on the measure (e.g., sum score).\index{reliability!internal consistency!average item–total correlation}
The [item–total correlation](#sec-itemTotalCorrelation-reliability) approximates the relation between an item and the construct.\index{reliability!internal consistency!average item–total correlation}
However, the [item–total correlation](#sec-itemTotalCorrelation-reliability) is a crude estimate of the relation between an item and the construct.\index{reliability!internal consistency!average item–total correlation}
...
## Overview of IRT {#sec-overview-irt}

Unlike [classical test theory](#sec-ctt), which is a measurement theory of how test scores relate to a construct, IRT is a measurement theory that describes how an *item* is related to a construct.\index{classical test theory}\index{item response theory}
...
IRT is an approach to [latent variable modeling](#sec-latentVariableModeling).\index{item response theory}\index{latent variable}
In IRT, we estimate a person's construct score (i.e., level on the construct) based on their item responses.
The construct is estimated as a latent factor that represents the common variance among all items as in [structural equation modeling](#sec-sem) or [confirmatory factor analysis](#sec-cfa).\index{item response theory}
...
### Item Characteristic Curve {#sec-icc}
...
For instance, we can create empirical ICCs that can take any shape (see @fig-empiricalICC).\index{item response theory!item characteristic curve}
...
```{r}
#| label: fig-empiricalICC
#| fig-cap: "Empirical Item Characteristic Curves of the Probability of Endorsement of a Given Item as a Function of the Person's Sum Score."
#| fig-alt: "Empirical Item Characteristic Curves of the Probability of Endorsement of a Given Item as a Function of the Person's Sum Score."
#| fig-height: 8
#| code-fold: true
...
The model-implied ICCs for the same 10 items from @fig-empiricalICC are depicted in @fig-modelImpliedICC.\index{item response theory!item characteristic curve}
...
```{r}
#| label: fig-modelImpliedICC
#| fig-cap: "Item Characteristic Curves of the Probability of Endorsement of a Given Item as a Function of the Person's Level on the Latent Construct."
#| fig-alt: "Item Characteristic Curves of the Probability of Endorsement of a Given Item as a Function of the Person's Level on the Latent Construct."
#| code-fold: true
...
```{r}
#| label: fig-modelImpliedTCC
#| fig-cap: "Test Characteristic Curve of the Expected Total Score on the Test as a Function of the Person's Level on the Latent Construct."
#| fig-alt: "Test Characteristic Curve of the Expected Total Score on the Test as a Function of the Person's Level on the Latent Construct."
#| code-fold: true
...
An ICC provides more information than an [item–total correlation](#sec-itemTotalCorrelation-reliability).\index{item response theory!item characteristic curve}\index{reliability!internal consistency!average item–total correlation}
...
For a particular item, among those with a low total score (level on the construct), 90% respond with "TRUE" to the item, whereas among everyone else, 100% respond with "TRUE" (see @fig-iccCeilingEffect).\index{item response theory!item characteristic curve}
...
```{r}
#| label: fig-iccCeilingEffect
#| fig-cap: "Item Characteristic Curve of an Item with a Ceiling Effect That is not Diagnostically Useful."
#| fig-alt: "Item Characteristic Curve of an Item with a Ceiling Effect That is not Diagnostically Useful."
#| code-fold: true
...
But for those with a higher level on the construct, 70% respond with true (see @fig-iccDiagnosticallyUseful).\index{item response theory!item characteristic curve}
...
```{r}
#| label: fig-iccDiagnosticallyUseful
#| fig-cap: "Item Characteristic Curve of an Item With a Floor Effect That is Diagnostically Useful."
#| fig-alt: "Item Characteristic Curve of an Item With a Floor Effect That is Diagnostically Useful."
#| code-fold: true
...
#### Difficulty (Severity) {#sec-itemDifficulty}
...
It is quantified by the intercept, i.e., the location on the x-axis of the inflection point of the [ICC](#sec-icc).\index{item response theory!item difficulty}\index{item response theory!item characteristic curve}
In a 1- or 2-parameter model, the inflection point is where 50% of the sample endorses the item (or gets the item correct), that is, the point on the x-axis where the [ICC](#sec-icc) crosses .5 probability on the y-axis (i.e., the level on the construct at which the probability of endorsing the item is equal to the probability of not endorsing the item).\index{item response theory!item difficulty}\index{item response theory!item characteristic curve}
Item difficulty is similar to item means or intercepts in [structural equation modeling](#sec-sem) or [factor analysis](#sec-factorAnalysis).\index{item response theory!item difficulty}\index{structural equation modeling!intercept}
...
See @fig-iccDifficulty for an example of an item with a low difficulty and an item with a high difficulty.\index{item response theory!item difficulty}
...
#### Discrimination {#sec-itemDiscrimination}
...
It is similar to the factor loading in [structural equation modeling](#sec-sem) or [factor analysis](#sec-factorAnalysis).\index{item response theory!item discrimination}\index{structural equation modeling!factor loading}
It is quantified by the slope of the [ICC](#sec-icc), i.e., the steepness of the line at its steepest point.\index{item response theory!item discrimination}\index{item response theory!item characteristic curve}
...
Some items have [ICCs](#sec-icc) that go up fast (have a steep slope).\index{item response theory!item discrimination}\index{item response theory!item characteristic curve}
...
See @fig-iccDiscrimination for an example of an item with a low discrimination and an item with a high discrimination.\index{item response theory!item discrimination}\index{item response theory!item characteristic curve}
...
```{r}
#| label: fig-iccDiscrimination
#| fig-cap: "Item Characteristic Curves of an Item With Low Discrimination Versus High Discrimination. The discrimination of an item is the slope of the line at its inflection point."
#| fig-alt: "Item Characteristic Curves of an Item With Low Discrimination Versus High Discrimination. The discrimination of an item is the slope of the line at its inflection point."
#| fig-scap: "Item Characteristic Curves of an Item With Low Discrimination Versus High Discrimination."
#| code-fold: true
...
#### Guessing {#sec-itemGuessing}

The item's *guessing* parameter is reflected by the lower asymptote of the [ICC](#sec-icc).\index{item response theory!item guessing}
...
See @fig-iccGuessingTF for an example of an item from a true/false exam and @fig-iccGuessingMC for an example of an item from a 4-option multiple choice exam.\index{item response theory!item guessing}
...
```{r}
#| label: fig-iccGuessingTF
#| fig-cap: "Item Characteristic Curve of an Item from a True/False Exam, There Test Takers Get the Item Correct at Least 50% of the Time."
#| fig-alt: "Item Characteristic Curve of an Item from a True/False Exam, There Test Takers Get the Item Correct at Least 50% of the Time."
#| code-fold: true
...
```{r}
#| label: fig-iccGuessingMC
#| fig-cap: "Item Characteristic Curve of an Item From a 4-Option Multiple Choice Exam, Where Test Takers Get the Item Correct at Least 25% of the Time."
#| fig-alt: "Item Characteristic Curve of an Item From a 4-Option Multiple Choice Exam, Where Test Takers Get the Item Correct at Least 25% of the Time."
#| code-fold: true
...
#### Inattention/Careless Errors {#sec-itemCarelessErrors}

The item's *inattention* (or *careless error*) parameter is the reflected by the upper asymptote of the [ICC](#sec-icc).\index{item response theory!item careless errors}
...
See @fig-iccInattention for an example of an item whose probability of endorsement (or getting it correct) exceeds .85.\index{item response theory!item careless errors}
...
```{r}
#| label: fig-iccInattention
#| fig-cap: "Item Characteristic Curve of an Item Where the Probability of Getting an Item Correct Never Exceeds .85."
#| fig-alt: "Item Characteristic Curve of an Item Where the Probability of Getting an Item Correct Never Exceeds .85."
#| code-fold: true
...
### Models {#sec-models-irt}

IRT models can be fit that estimate one or more of these four item parameters.\index{item response theory}

#### 1-Parameter and Rasch models {#sec-onePL}

A Rasch model estimates the [item difficulty](#sec-itemDifficulty) parameter and holds everything else fixed across items.\index{item response theory!one-parameter model}\index{item response theory!item difficulty}
It fixes the [item discrimination](#sec-itemDiscrimination) to be one for each item.\index{item response theory!one-parameter model}\index{item response theory!item discrimination}
In the Rasch model, the probability that a person $j$ with a level on the construct of $\theta$ gets a score of one (instead of zero) on item $i$, based on the difficulty ($b$) of the item, is estimated using @eq-raschModel:\index{item response theory!one-parameter model}\index{item response theory!item difficulty}\index{item response theory!theta}
...
A one-parameter logistic (1-PL) IRT model, similar to a Rasch model, estimates the [item difficulty](#sec-itemDifficulty) parameter, and holds everything else fixed across items (see @fig-irt1PL).\index{item response theory!one-parameter model}\index{item response theory!item difficulty}
The one-parameter logistic model holds the [item discrimination](#sec-itemDiscrimination) fixed across items, but does not fix it to one, unlike the Rasch model.\index{item response theory!one-parameter model}\index{item response theory!item difficulty}

In the one-parameter logistic model, the probability that a person $j$ with a level on the construct of $\theta$ gets a score of one (instead of zero) on item $i$, based on the [difficulty](#sec-itemDifficulty) ($b$) of the item and the items' (fixed) [discrimination](#sec-itemDiscrimination) ($a$), is estimated using @eq-onePL:\index{item response theory!one-parameter model}\index{item response theory!item difficulty}\index{item response theory!item discrimination}\index{item response theory!theta}
...
They assume that items have the same [discrimination](#sec-itemDiscrimination).\index{item response theory!one-parameter model}\index{item response theory!item discrimination}
...
```{r}
#| label: fig-irt1PL
#| fig-cap: "One-Parameter Logistic Model in Item Response Theory."
#| fig-alt: "One-Parameter Logistic Model in Item Response Theory."
#| code-fold: true
...
A one-parameter logistic model is only valid if there is not crossing of lines in empirical [ICCs](#sec-icc) (see @fig-empiricalICCnoCrossing).\index{item response theory!one-parameter model}\index{item response theory!item characteristic curve}
...
```{r}
#| label: fig-empiricalICCnoCrossing
#| fig-cap: "Empirical Item Characteristic Curves of the Probability of Endorsement of a Given Item as a Function of the Person's Sum Score. The empirical item characteristic curves of these items do not cross each other."
#| fig-alt: "Empirical Item Characteristic Curves of the Probability of Endorsement of a Given Item as a Function of the Person's Sum Score. The empirical item characteristic curves of these items do not cross each other."
#| fig-scap: "Empirical Item Characteristic Curves of the Probability of Endorsement of a Given Item as a Function of the Person's Sum Score."
#| fig-height: 8
#| code-fold: true
...
#### 2-Parameter {#sec-twoPL}

A two-parameter logistic (2-PL) IRT model estimates item [difficulty](#sec-itemDifficulty) and [discrimination](#sec-itemDiscrimination), and it holds the asymptotes fixed across items (see @fig-irt2PL).\index{item response theory!two-parameter model}\index{item response theory!item difficulty}\index{item response theory!item discrimination}
Two-parameter logistic models are also common.\index{item response theory!two-parameter model}

In the two-parameter logistic model, the probability that a person $j$ with a level on the construct of $\theta$ gets a score of one (instead of zero) on item $i$, based on the [difficulty](#sec-itemDifficulty) ($b$) and [discrimination](#sec-itemDiscrimination) ($a$) of the item, is estimated using @eq-twoPL:\index{item response theory!two-parameter model}\index{item response theory!item difficulty}\index{item response theory!item discrimination}\index{item response theory!theta}
...
```{r}
#| label: fig-irt2PL
#| fig-cap: "Two-Parameter Logistic Model in Item Response Theory."
#| fig-alt: "Two-Parameter Logistic Model in Item Response Theory."
#| code-fold: true
...
Three-parameter logistic models are less common to estimate because it adds considerable computational complexity and requires a large sample size, and the [guessing](#sec-itemGuessing) parameter is often not as important as [difficulty](#sec-itemDifficulty) and [discrimination](#sec-itemDiscrimination).\index{item response theory!three-parameter model}\index{item response theory!item difficulty}\index{item response theory!item discrimination}\index{item response theory!item guessing}
...
In the three-parameter logistic model, the probability that a person $j$ with a level on the construct of $\theta$ gets a score of one (instead of zero) on item $i$, based on the [difficulty](#sec-itemDifficulty) ($b$), [discrimination](#sec-itemDiscrimination) ($a$), and [guessing parameter](#sec-itemGuessing) ($c$) of the item, is estimated using @eq-threePL:\index{item response theory!three-parameter model}\index{item response theory!item difficulty}\index{item response theory!item discrimination}\index{item response theory!item guessing}\index{item response theory!theta}
...
```{r}
#| label: fig-irt3PL
#| fig-cap: "Three-Parameter Logistic Model in Item Response Theory."
#| fig-alt: "Three-Parameter Logistic Model in Item Response Theory."
#| code-fold: true
...
#### 4-Parameter {#sec-fourPL}

A four-parameter logistic (4-PL) IRT model estimates item [difficulty](#sec-itemDifficulty), [discrimination](#sec-itemDiscrimination), [guessing](#sec-itemGuessing), and [careless errors](#sec-itemCarelessErrors) (see @fig-irt4PL).\index{item response theory!four-parameter model}\index{item response theory!item difficulty}\index{item response theory!item discrimination}\index{item response theory!item guessing}\index{item response theory!item careless errors}
...
```{r}
#| label: fig-irt4PL
#| fig-cap: "Four-Parameter Logistic Model in Item Response Theory."
#| fig-alt: "Four-Parameter Logistic Model in Item Response Theory."
#| code-fold: true
...
The [two-parameter](#sec-twoPL) graded response model takes the general form of @eq-gradedResponseModel1:\index{item response theory!graded response model}\index{item response theory!two-parameter model}
...
In the model, $a_i$ an item-specific [discrimination parameter](#sec-itemDiscrimination), $b_{ic}$ is an item- and category-specific [difficulty parameter](#sec-itemDifficulty), and $θ_n$ is an estimate of a person's standing on the latent variable.\index{item response theory!graded response model}\index{item response theory!two-parameter model}\index{item response theory!item difficulty}\index{item response theory!item discrimination}
...
### Type of Data {#sec-dataTypes-irt}
...
For example, see @fig-polytomousItemBoundaryCurves for an example of an item boundary characteristic curve for an item from a 5-level likert scale (based on a cumulative distribution).\index{item response theory}\index{item response theory!item boundary characteristic curve}\index{Likert scale}
...
```{r}
#| label: fig-polytomousItemBoundaryCurves
#| fig-cap: "Item Boundary Characteristic Curves From Two-Parameter Graded Response Model in Item Response Theory."
#| fig-alt: "Item Boundary Characteristic Curves From Two-Parameter Graded Response Model in Item Response Theory."
#| code-fold: true
...
```{r}
#| label: fig-polytomousItemResponseCategoryCurves
#| fig-cap: "Item Response Category Characteristic Curves From Two-Parameter Graded Response Model in Item Response Theory."
#| fig-alt: "Item Response Category Characteristic Curves From Two-Parameter Graded Response Model in Item Response Theory."
#| code-fold: true
...
If you want to use continuous data, you might consider moving to a [factor analysis](#sec-factorAnalysis) framework.\index{factor analysis}

### Sample Size {#sec-sampleSize-irt}
...
### Reliability (Information) {#sec-irtReliability}

IRT conceptualizes [reliability](#sec-reliability) in a different way than [classical test theory](#sec-ctt) does.\index{item response theory}\index{classical test theory}\index{item response theory!information}\index{reliability}
Both IRT and [classical test theory](#sec-ctt) conceptualize [reliability](#sec-reliability) as involving the *precision* of a measure's scores.\index{item response theory}\index{classical test theory}\index{item response theory!information}\index{reliability}\index{reliability!precision}
In [classical test theory](#sec-ctt), (im)precision—as operationalized by the [standard error of measurement](#sec-standardErrorOfMeasurement)—is estimated with a single index across the whole range of the construct.\index{classical test theory}\index{reliability}\index{reliability!precision}\index{reliability!standard error of measurement}
That is, in [classical test theory](#sec-ctt), the same [standard error of measurement](#sec-standardErrorOfMeasurement) applies to all scores in the population [@Embretson1996].\index{classical test theory}\index{reliability}\index{reliability!precision}\index{reliability!standard error of measurement}
However, IRT estimates how much measurement precision (information) or imprecision ([standard error of measurement](#sec-standardErrorOfMeasurement)) each item, and the test as a whole, has at different construct levels.\index{item response theory!information}\index{reliability}
This allows IRT to conceptualize [reliability](#sec-reliability) in such a way that precision/[reliability](#sec-reliability) can *differ* at different construct levels, unlike in [classical test theory](#sec-ctt) [@Embretson1996].\index{item response theory}\index{classical test theory}\index{item response theory!information}\index{reliability}
Thus, IRT does not have one index of [reliability](#sec-reliability); rather, its estimate of [reliability](#sec-reliability) differs at different levels on the construct.\index{item response theory}\index{classical test theory}\index{item response theory!information}\index{reliability}
As a result, from the perspective of IRT, it does not make sense to talk about the [reliability](#sec-reliability) of a scale, because the [reliability](#sec-reliability) of the scale depends on the person's score on that scale.\index{item response theory}\index{classical test theory}\index{item response theory!information}\index{reliability}

Based on an item's [difficulty](#sec-itemDifficulty) and [discrimination](#sec-itemDiscrimination), we can calculate how much information each item provides.\index{item response theory}\index{item response theory!information}\index{reliability}\index{item response theory!item difficulty}\index{item response theory!item discrimination}
...
In other words, information is the degree to which an item (or measure) reduces the [standard error of measurement](#sec-standardErrorOfMeasurement), that is, how much it reduces uncertainty of a person's level on the construct.\index{item response theory}\index{item response theory!information}\index{reliability}\index{reliability!standard error of measurement}
As a reminder (from @eq-standardErrorOfMeasurement), the [standard error of measurement](#sec-standardErrorOfMeasurement) is calculated as:\index{reliability!standard error of measurement}
...
The [standard error of measurement](#sec-standardErrorOfMeasurement) is used to generate confidence intervals for people's scores.\index{reliability!standard error of measurement}
In IRT, the [standard error of measurement](#sec-standardErrorOfMeasurement) (at a given construct level) can be calculated as the inverse of the square root of the amount of test information at that construct level, as in @eq-semIRT:\index{item response theory}\index{item response theory!information}\index{reliability}\index{reliability!standard error of measurement}\index{item response theory!standard error of measurement}\index{item response theory!test information curve}

$$
\text{SEM}(\theta) = \frac{1}{\sqrt{\text{information}(\theta)}}
$$ {#eq-semIRT}

The [`petersenlab`](https://github.com/DevPsyLab/petersenlab) package [@R-petersenlab] contains the `standardErrorIRT()` function that estimates the [standard error of measurement](#sec-standardErrorOfMeasurement) at a person's level on the construct (theta) from the amount of information that the item (or test) provides.\index{petersenlab package}\index{item response theory}\index{item response theory!information}\index{reliability}\index{reliability!standard error of measurement}\index{item response theory!standard error of measurement}\index{item response theory!theta}
...
The [standard error of measurement](#sec-standardErrorOfMeasurement) tends to be higher (i.e., [reliability](#sec-reliability)/information tends to be lower) at the extreme levels of the construct where there are fewer items.\index{item response theory}\index{item response theory!information}\index{reliability}\index{reliability!standard error of measurement}\index{item response theory!standard error of measurement}

The formula for information for item $i$ at construct level $\theta$ in a Rasch model is in @eq-itemInformationRasch [@Baker2017]:\index{item response theory}\index{item response theory!information}\index{reliability}\index{item response theory!theta}
...
The formula for information for item $i$ at construct level $\theta$ in a two-parameter logistic model is in @eq-itemInformation2PL [@Baker2017]:\index{item response theory}\index{item response theory!information}\index{reliability}\index{item response theory!theta}\index{item response theory!two-parameter model}
...
The formula for information for item $i$ at construct level $\theta$ in a four-parameter logistic model is in @eq-itemInformation4PL [@Magis2013]:\index{item response theory}\index{item response theory!information}\index{reliability}\index{item response theory!theta}\index{item response theory!four-parameter model}
...
[Reliability](#sec-irtReliability) at a given level of the construct ($\theta$) can be estimated as in @eq-reliabilityIRT:\index{item response theory}\index{item response theory!information}\index{reliability}\index{item response theory!theta}
...
Consider some hypothetical items depicted with [ICCs](#sec-icc) in @fig-reliabilityIRTicc.\index{item response theory!item characteristic curve}
...
```{r}
#| label: fig-reliabilityIRTicc
#| fig-cap: "Item Characteristic Curves From Two-Parameter Logistic Model in Item Response Theory. The dashed horizontal line indicates a probability of item endorsement of .50. The dashed vertical line is the item difficulty, i.e., the person's level on the construct (the location on the x-axis) at the inflection point of the item characteristic curve. In a two-parameter logistic model, the inflection point corresponds to the probability of item endorsement is 50%. Thus, in a two-parameter logistic model, the difficulty of an item is the person's level on the construct where the probability of endorsing the item is 50%."
#| fig-alt: "Item Characteristic Curves From Two-Parameter Logistic Model in Item Response Theory. The dashed horizontal line indicates a probability of item endorsement of .50. The dashed vertical line is the item difficulty, i.e., the person's level on the construct (the location on the x-axis) at the inflection point of the item characteristic curve. In a two-parameter logistic model, the inflection point corresponds to the probability of item endorsement is 50%. Thus, in a two-parameter logistic model, the difficulty of an item is the person's level on the construct where the probability of endorsing the item is 50%."
#| fig-scap: "Item Characteristic Curves From Two-Parameter Logistic Model in Item Response Theory."
#| code-fold: true
...
We can present the [ICC](#sec-icc) in terms of an item information curve (see @fig-reliabilityIRTitemInformation).\index{item response theory}\index{item response theory!item characteristic curve}\index{item response theory!information}\index{reliability}\index{item response theory!item information curve}
On the x-axis, the information peak is located at the [difficulty/severity](#sec-itemDifficulty) of the item.
The higher the [discrimination](#sec-itemDiscrimination), the higher the information peak on the y-axis.\index{item response theory}\index{item response theory!item characteristic curve}\index{item response theory!information}\index{reliability}\index{item response theory!item difficulty}\index{item response theory!item discrimination}

```{r}
#| label: fig-reliabilityIRTitemInformation
#| fig-cap: "Item Information From Two-Parameter Logistic Model in Item Response Theory. The dashed vertical line is the item difficulty, which is located at the peak of the item information curve."
#| fig-alt: "Item Information From Two-Parameter Logistic Model in Item Response Theory. The dashed vertical line is the item difficulty, which is located at the peak of the item information curve."
#| fig-scap: "Item Information From Two-Parameter Logistic Model in Item Response Theory."
#| code-fold: true
...
This is called the test information curve (see @fig-reliabilityIRTtestInformation).\index{item response theory}\index{item response theory!item characteristic curve}\index{item response theory!information}\index{reliability}\index{item response theory!test information curve}
...
```{r}
#| label: fig-reliabilityIRTtestInformation
#| fig-cap: "Test Information Curve From Two-Parameter Logistic Model in Item Response Theory."
#| fig-alt: "Test Information Curve From Two-Parameter Logistic Model in Item Response Theory."
#| code-fold: true
...
Based on test information, we can calculate the [standard error of measurement](#sec-standardErrorOfMeasurement) (see @fig-reliabilityIRTtestSE).\index{item response theory}\index{item response theory!item characteristic curve}\index{item response theory!information}\index{reliability}\index{reliability!standard error of measurement}\index{item response theory!standard error of measurement}\index{item response theory!test information curve}
Notice how the degree of [(un)reliability](#sec-reliability) differs at different construct levels.\index{item response theory}\index{item response theory!item characteristic curve}\index{item response theory!information}\index{reliability}\index{reliability!standard error of measurement}\index{item response theory!standard error of measurement}\index{item response theory!test information curve}

```{r}
#| label: fig-reliabilityIRTtestSE
#| fig-cap: "Test Standard Error of Measurement From Two-Parameter Logistic Model in Item Response Theory."
#| fig-alt: "Test Standard Error of Measurement From Two-Parameter Logistic Model in Item Response Theory."
#| code-fold: true
...
Based on test information, we can estimate the [reliability](#sec-irtReliability) (see @fig-reliabilityIRTtestReliability).\index{item response theory}\index{item response theory!item characteristic curve}\index{item response theory!information}\index{reliability}\index{reliability!standard error of measurement}\index{item response theory!standard error of measurement}\index{item response theory!test information curve}
Notice how the degree of [(un)reliability](#sec-reliability) differs at different construct levels.\index{item response theory}\index{item response theory!item characteristic curve}\index{item response theory!information}\index{reliability}\index{reliability!standard error of measurement}\index{item response theory!standard error of measurement}

```{r}
#| label: fig-reliabilityIRTtestReliability
#| fig-cap: "Test Reliability From Two-Parameter Logistic Model in Item Response Theory."
#| fig-alt: "Test Reliability From Two-Parameter Logistic Model in Item Response Theory."
#| code-fold: true
...
### Efficient Assessment {#sec-efficientAssessment}
...
For instance, you could use two items to estimate where the person is on the construct: low, middle, or high (see @fig-efficientAssessment).\index{item response theory}\index{brief assessment}
...
If two items perform similarly, that is, they have the same [difficulty](#sec-itemDifficulty) and [discrimination](#sec-itemDiscrimination), they are redundant, and we can sacrifice one of them.\index{item response theory}\index{brief assessment}\index{item response theory!item difficulty}\index{item response theory!item discrimination}
This leads to greater efficiency and better measurement in terms of [reliability](#sec-reliability) and [validity](#sec-validity).\index{item response theory}\index{brief assessment}\index{reliability}\index{validity}
...
```{r}
#| label: fig-efficientAssessment
#| fig-cap: "Visual Representation of an Efficient Assessment Based on Item Characteristic Curves from Two-Parameter Logistic Model in Item Response Theory."
#| fig-alt: "Visual Representation of an Efficient Assessment Based on Item Characteristic Curves from Two-Parameter Logistic Model in Item Response Theory."
#| code-fold: true
...
IRT forms the basis of [computerized adaptive testing](#sec-cat), which is discussed in @sec-cat.\index{item response theory}\index{brief assessment}\index{adaptive testing}
As discussed earlier, briefer measures can increase [reliability](#sec-reliability) and [validity](#sec-validity) of measurement if the items are tailored to the ability level of the participant.\index{brief assessment}\index{reliability}\index{validity}
The idea of [adaptive testing](#sec-cat) is that, instead of having a standard scale for all participants, the items adapt to each person.\index{brief assessment}\index{adaptive testing}
An example of a measure that has used [computerized adaptive testing](#sec-cat) is the Graduate Record Examination (GRE).\index{adaptive testing}

With [adaptive testing](#sec-cat), it is important to develop a comprehensive item bank that spans the difficulty range of interest.\index{adaptive testing}
The starting construct level is the 50th percentile.
If the respondent gets the first item correct, it moves to the next item that would provide the most [information](#sec-irtReliability) for the person, based on a split of the remaining sample (e.g., 75th percentile).\index{adaptive testing}\index{reliability}\index{item response theory!information}
And so on...\index{adaptive testing}
The goal of [adaptive testing](#sec-cat) is to find the construct level where the respondent keeps getting items right and wrong 50% of the time.\index{adaptive testing}
[Adaptive testing](#sec-cat) is a promising approach that saves time because it tailors which items are administered to which person (based on their construct level) to get the most [reliable](#sec-reliability) estimate in the shortest time possible.\index{adaptive testing}\index{reliability}\index{brief assessment}
...
#### A Good Measure {#sec-goodMeasure}
...
1. have good items that yield lots of [information](#sec-irtReliability), and\index{item response theory}\index{item response theory!information}\index{reliability}
...
The items' [difficulty](#sec-itemDifficulty) should span the range of interest.\index{item response theory}\index{item response theory!item difficulty}

Second, a good measure should have good items that yield lots of [information](#sec-irtReliability).\index{item response theory}\index{item response theory!information}\index{reliability}
For example, the items should have strong [discrimination](#sec-itemDiscrimination), that is, the items are strongly related to the construct.\index{item response theory}\index{item response theory!item discrimination}
...
The items should not have the same [difficulty](#sec-itemDifficulty) or they would be considered redundant, and one of the redundant items could be dropped.\index{item response theory}\index{item response theory!item difficulty}
The items' [difficulty](#sec-itemDifficulty) should densely cover the construct within the range of interest.\index{item response theory}\index{item response theory!item difficulty}
For instance, if the construct range of interest is 1–2 standard deviations above the mean, the items should have [difficulty](#sec-itemDifficulty) that densely cover this range (e.g., 1.0, 1.05, 1.10, 1.15, 1.20, 1.25, 1.30, ..., 2.0).\index{item response theory}\index{item response theory!item difficulty}

With items that (1) span the range of interest, (2) have high [discrimination](#sec-itemDiscrimination) and [information](#sec-irtReliability), and (3) densely cover the range of interest without redundancy, the measure should have a high [information](#sec-irtReliability) in the range of interest.\index{item response theory}\index{item response theory!item difficulty}\index{item response theory!item discrimination}\index{item response theory!information}\index{reliability}
...
```{r}
#| label: fig-badMeasureICCs
#| fig-cap: "Visual Representation of a Bad Measure Based on Item Characteristic Curves of Items From a Bad Measure Estimated from Two-Parameter Logistic Model in Item Response Theory."
#| fig-alt: "Visual Representation of a Bad Measure Based on Item Characteristic Curves of Items From a Bad Measure Estimated from Two-Parameter Logistic Model in Item Response Theory."
#| code-fold: true
...
```{r}
#| label: fig-badMeasureInfo
#| fig-cap: "Visual Representation of a Bad Measure Based on the Test Information Curve."
#| fig-alt: "Visual Representation of a Bad Measure Based on the Test Information Curve."
#| code-fold: true
...
An example of a good measure for distinguishing clinical-range versus sub-clinical range is depicted in terms of [ICCs](#sec-icc) in @fig-goodMeasureICCs and in terms of [test information](#sec-irtReliability) in @fig-goodMeasureInfo.\index{item response theory}\index{item response theory!information}\index{reliability}\index{item response theory!item characteristic curve}\index{item response theory!test information curve}
The measure is good for the intended purpose, in terms of having items that (a) span the range of interest (1–3 standard deviations above the mean of the latent construct), (b) have high [discrimination](#sec-itemDiscrimination) and [information](#sec-irtReliability), and (c) densely cover the range of interest without redundancy.\index{item response theory}\index{item response theory!information}\index{item response theory!item discrimination}\index{reliability}\index{item response theory!test information curve}
...
```{r}
#| label: fig-goodMeasureICCs
#| fig-cap: "Visual Representation of a Good Measure (For Distinguishing Clinical-Range Versus Sub-Clinical Range) Based on Item Characteristic Curves of Items From a Good Measure Estimated From Two-Parameter Logistic Model in Item Response Theory."
#| fig-alt: "Visual Representation of a Good Measure (For Distinguishing Clinical-Range Versus Sub-cClinical Range) Based on Item Characteristic Curves of Items From a Good Measure Estimated From Two-Parameter Logistic Model in Item Response Theory."
#| code-fold: true
...
```{r}
#| label: fig-goodMeasureInfo
#| fig-cap: "Visual Representation of a Good Measure (For Distinguishing Clinical-Range Versus Sub-Clinical Range) Based on the Test Information Curve."
#| fig-alt: "Visual Representation of a Good Measure (For Distinguishing Clinical-Range Versus Sub-Clinical Range) Based on the Test Information Curve."
#| code-fold: true
...
### Assumptions of IRT {#sec-assumptions-irt}

IRT has several assumptions:\index{item response theory!assumptions}

- monotonicity\index{item response theory!assumptions}
- unidimensionality\index{item response theory!assumptions}\index{unidimensional}
- item invariance\index{item response theory!assumptions}
- local independence\index{item response theory!assumptions}

#### Monotonicity {#sec-monotonicity-irt}

The monotonicity assumption holds that a person's probability of endorsing a higher level on the item increases as a person's level on the latent construct increases.\index{item response theory!assumptions}
For instance, for each item assessing externalizing problems, as a child increases in their level of externalizing problems, they are expected to be rated with a higher level on that item.\index{item response theory!assumptions}
Monotonicity can be evaluated in multiple ways.\index{item response theory!assumptions}
For instance, monotonicity can be evaluated using visual inspection of empirical [item characteristic curves](#sec-icc).\index{item response theory!assumptions}\index{item response theory!item characteristic curve}
Another way to evaluate monotonicity is with Mokken scale analysis, such as using the `mokken` package in `R`.\index{item response theory!assumptions}

#### Unidimensionality {#sec-unidimensionality-irt}

The unidimensionality assumption holds that the items have one predominant dimension, which reflects the underlying (latent) construct.\index{item response theory!assumptions}\index{unidimensional}
The dimensionality of a set of items can be evaluated using [factor analysis](#sec-factorAnalysis).\index{item response theory!assumptions}\index{factor analysis}
Although items that are intended to assess a given latent latent construct are expected to be unidimensional, models have been developed that allow multiple latent dimensions, as shown in @sec-irt-twoPLmultidimensional.\index{item response theory!assumptions}\index{multidimensional}\index{unidimensional}
These [multidimensional IRT models](#sec-irt-twoPLmultidimensional) allow borrowing information from a given latent factor in the estimation of other latent factor(s) to account for the covariation.\index{item response theory!assumptions}\index{multidimensional}

#### Item Invariance {#sec-itemInvariance-irt}

The item invariance assumption holds that the items function similarly (i.e., have the same parameters) for all people and subgroups in the population.\index{item response theory!assumptions}
The extent to which items may violate the item invariance assumption can be evaluated empirically using tests of [differential item functioning](#sec-dif) (DIF).\index{item response theory!assumptions}\index{item response theory!differential item functioning}
Tests of [measurement invariance](#sec-measurementInvariance) are the equivalent of tests of [differential item functioning](#sec-dif) for [factor analysis](#sec-factorAnalysis)/[structural equation models](#sec-sem).\index{item response theory!assumptions}\index{item response theory!differential item functioning}\index{structural equation modeling!measurement invariance}\index{factor analysis}\index{structural equation modeling}
Test of [differential item functioning](#sec-dif) and [measurement invariance](#sec-measurementInvariance) are described in the chapter on [test bias](#sec-bias).\index{item response theory!assumptions}\index{item response theory!differential item functioning}\index{structural equation modeling!measurement invariance}

#### Local Independence {#sec-localIndependence-irt}

The local independence assumptions holds that the items are uncorrelated when controlling for the latent dimension.\index{item response theory!assumptions}
That is, IRT models assume that the items' errors (residuals) are uncorrelated with each other.\index{item response theory!assumptions}
[Factor analysis](#sec-factorAnalysis) and [structural equation models](#sec-sem) can relax this assumption and allow items' error terms to correlate with each other.\index{item response theory!assumptions}\index{structural equation modeling!residual!correlated}\index{factor analysis}\index{structural equation modeling}

## Getting Started {#sec-gettingStarted-irt}

### Load Libraries {#sec-loadLibraries-irt}
...
### Load Data {#sec-loadData-irt}

`LSAT7` is a data set from the `mirt` package [@R-mirt] that contains five items from the Law School Admissions Test.
`SAT12` is a data set from the `mirt` package [@R-mirt] that contains 32 items for a grade 12 science assessment test (SAT) measuring topics of chemistry, biology, and physics.
...
### Prepare Data {#sec-prepareData-irt}

```{r}
mydataSAT <- mirt::key2binary(
  SAT12,
  key = c(1,4,5,2,3,1,2,1,3,1,2,4,2,1,5,3,4,4,1,4,3,3,4,1,3,5,1,3,1,5,4,5))
```

### Descriptive Statistics {#sec-descriptiveStats-IRT}

```{r}
itemstats(mydataIRT, ts.tables = TRUE)
```

## Comparison of Scoring Approaches {#sec-scoringApproaches}
...
A weighted symptom sum is the [classical test theory](#sec-ctt) analog of IRT.\index{scoring}\index{classical test theory}\index{item response theory}
In [classical test theory](#sec-ctt), proportion correct (or endorsed) would correspond to item [difficulty](#sec-itemDifficulty) and the [item–total correlation](#sec-itemTotalCorrelation-reliability) (i.e., a point-biserial correlation) would correspond to item [discrimination](#sec-itemDiscrimination).\index{scoring}\index{classical test theory}\index{item response theory!item difficulty}\index{reliability!internal consistency!average item–total correlation}
If we were to compute a weighted sum of each item according to its strength of association with the construct (i.e., the [item–total correlation](#sec-itemTotalCorrelation-reliability)), this measure would be somewhat more precise than the raw symptom sum, but it is not a latent variable method.\index{scoring}\index{reliability!internal consistency!average item–total correlation}\index{latent variable}
...
IRT down-weights the poorly [discriminating](#sec-itemDiscrimination) items and up-weights the strongly [discriminating](#sec-itemDiscrimination) items.\index{item response theory}\index{item response theory!item discrimination}
This leads to greater [precision](#sec-reliability) and a lower [standard error of measurement](#sec-standardErrorOfMeasurement) than non-latent scoring approaches.\index{scoring}\index{item response theory}\index{latent variable}\index{reliability}\index{reliability!precision}\index{reliability!standard error of measurement}

According to @Embretson1996, many perspectives have changed because of IRT.\index{item response theory}
First, according to [classical test theory](#sec-ctt), longer tests are more [reliable](#sec-reliability) than shorter tests, as described in @sec-splitHalf-reliability in the chapter on [reliability](#sec-reliability).\index{classical test theory}\index{reliability}
However, according to IRT, shorter tests (i.e., tests with fewer items) can be more [reliable](#sec-reliability) than longer tests.\index{item response theory}\index{brief assessment}\index{reliability}
Item selection using IRT can lead to briefer assessments that have greater [reliability](#sec-reliability) than longer scales.\index{item response theory}\index{brief assessment}\index{reliability}
For example, [adaptive tests](#sec-cat) that tailor the [difficulty](#sec-itemDifficulty) of the items to the ability level of the participant.\index{item response theory}\index{brief assessment}\index{reliability}\index{adaptive testing}\index{item response theory!item difficulty}

Second, in [classical test theory](#sec-ctt), a score's meaning is tied to its location in a distribution (i.e., the [norm-referenced](#sec-norm) standard).\index{classical test theory}
In IRT, however, the people and items are calibrated on a common scale.\index{item response theory}
Based on a child's IRT-estimated ability level (i.e., level on the construct), we can have a better sense of what the child knows and does not know, because it indicates the [difficulty](#sec-itemDifficulty) level at which they would tend to get items correct 50% of the time; the person would likely fail items with a higher [difficulty](#sec-itemDifficulty) compared to this level, whereas the person would likely pass items with a lower [difficulty](#sec-itemDifficulty) compared to this level.\index{item response theory}\index{item response theory!item difficulty}\index{item response theory!theta}
Consider Binet's distribution of ability that arranges the items from easiest to most difficult.
Based on the item [difficulty](#sec-itemDifficulty) and content of the items and the child's performance, we can have a better indication that a child can perform items successfully in a particular range (e.g., count to 10) but might not be able to perform more difficult items (e.g., tie their shoes).\index{item response theory}\index{item response theory!item difficulty}
...
Thus, IRT can provide more meaningful understanding of a person's ability compared to traditional [classical test theory](#sec-ctt) interpretations such as the child being at the "63rd percentile" for a child of their age, which lacks conceptual meaning.\index{item response theory}\index{classical test theory}\index{data!percentile rank}

According to @Cooper2009a, our current diagnostic system relies heavily on how many symptoms a person endorses as an index of severity, but this assumes that all symptom endorsements have the same overall weight (severity).\index{scoring}
Using IRT, we can determine the relative [severity](#sec-itemDifficulty) of each item (symptom)—and it is clear that some symptoms indicate more severity than others.\index{scoring}\index{item response theory}\index{item response theory!item difficulty}
From this analysis, a respondent can endorse fewer, more severe items, and have overall more severe psychopathology than an individual who endorses more, less [severe](#sec-itemDifficulty) items.\index{scoring}\index{item response theory}\index{item response theory!item difficulty}
Basically, not all items are equally [severe](#sec-itemDifficulty)—know your items!\index{scoring}\index{item response theory}\index{item response theory!item difficulty}

## Rasch Model (1-Parameter Logistic) {#sec-irt-onePL}

A one-parameter logistic (1PL) item response theory (IRT) model is a model fit to dichotomous data, which estimates a different [difficulty](#sec-itemDifficulty) ($b$) parameter for each item.\index{item response theory!one-parameter model}\index{item response theory!item difficulty}
[Discrimination](#sec-itemDiscrimination) ($a$) is not estimated (i.e., it is fixed at the same value—one—across items).\index{item response theory!one-parameter model}\index{item response theory!item discrimination}
Rasch models were fit using the `mirt` package [@R-mirt].\index{item response theory!one-parameter model}

### Fit Model {#sec-irt-onePLfit}
...
### Model Summary {#sec-irt-onePLoutput}
...
### Factor Scores {#sec-irt-onePLfactorScores}
...
### Plots {#sec-irt-onePLplots}

#### Test Curves {#sec-irt-onePLtestCurves}

The test curves suggest that the measure is most [reliable](#sec-reliability) (i.e., provides the most [information](#sec-irtReliability) has the smallest [standard error of measurement](#sec-irtReliability)) at lower levels of the construct.\index{item response theory!information}\index{item response theory!standard error of measurement}\index{reliability!standard error of measurement}

##### Test Characteristic Curve {#sec-irt-onePLtcc}

A test characteristic curve ([TCC](#sec-icc)) plot of the expected total score as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-onePLtcc.\index{item response theory!item characteristic curve}

```{r}
#| label: fig-onePLtcc
#| fig-cap: "Test Characteristic Curve From Rasch Item Response Theory Model."
#| fig-alt: "Test Characteristic Curve From Rasch Item Response Theory Model."

plot(raschModel, type = "score")
```

##### Test Information Curve {#sec-irt-onePLtif}

A plot of [test information](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-onePLtif.\index{item response theory!information}\index{item response theory!test information curve}

```{r}
#| label: fig-onePLtif
#| fig-cap: "Test Information Curve From Rasch Item Response Theory Model."
#| fig-alt: "Test Information Curve From Rasch Item Response Theory Model."

plot(raschModel, type = "info")
```

##### Test Reliability {#sec-irt-onePLreliability}

The estimate of marginal [reliability](#sec-irtReliability) is below:\index{reliability}

```{r}
marginal_rxx(raschModel)
```

The estimate of empirical [reliability](#sec-irtReliability) is below:\index{reliability}

```{r}
empirical_rxx(raschModel_factorScores)
```

A plot of test [reliability](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-onePLreliability.\index{reliability}

```{r}
#| label: fig-onePLreliability
#| fig-cap: "Test Reliability From Rasch Item Response Theory Model."
#| fig-alt: "Test Reliability From Rasch Item Response Theory Model."

plot(raschModel, type = "rxx")
```

##### Test Standard Error of Measurement {#sec-irt-onePLsem}

A plot of test [standard error of measurement](#sec-irtReliability) (SEM) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-onePLsem.\index{reliability!standard error of measurement}

```{r}
#| label: fig-onePLsem
#| fig-cap: "Test Standard Error of Measurement From Rasch Item Response Theory Model."
#| fig-alt: "Test Standard Error of Measurement From Rasch Item Response Theory Model."

plot(raschModel, type = "SE")
```

##### Test Information Curve and Test Standard Error of Measurement {#sec-irt-onePLtifSEM}

A plot of [test information](#sec-irtReliability) and [standard error of measurement](#sec-irtReliability) (SEM) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-onePLtifSEM.\index{item response theory!information}\index{item response theory!standard error of measurement}\index{reliability!standard error of measurement}\index{item response theory!test information curve}

```{r}
#| label: fig-onePLtifSEM
#| fig-cap: "Test Information Curve and Standard Error of Measurement From Rasch Item Response Theory Model."
#| fig-alt: "Test Information Curve and Standard Error of Measurement From Rasch Item Response Theory Model."

plot(raschModel, type = "infoSE")
```

#### Item Curves {#sec-irt-onePLitemCurves}

##### Item Characteristic Curves {#sec-irt-onePLicc}

Item characteristic curve ([ICC](#sec-icc)) plots of the probability of item endorsement (or getting the item correct) as a function of a person's level on the latent construct (theta; $\theta$) are in Figures [-@fig-onePLicc] and [-@fig-onePLiccFacet].\index{item response theory!item characteristic curve}

```{r}
#| label: fig-onePLicc
#| fig-cap: "Item Characteristic Curves From Rasch Item Response Theory Model."
#| fig-alt: "Item Characteristic Curves From Rasch Item Response Theory Model."

plot(raschModel, type = "itemscore", facet_items = FALSE)
```

```{r}
#| label: fig-onePLiccFacet
#| fig-cap: "Item Characteristic Curves From Rasch Item Response Theory Model."
#| fig-alt: "Item Characteristic Curves From Rasch Item Response Theory Model."
...
##### Item Information Curves {#sec-irt-onePLiif}

Plots of [item information](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) are in Figures [-@fig-onePLiif] and [-@fig-onePLiifFacet].\index{item response theory!information}\index{item response theory!item information curve}

```{r}
#| label: fig-onePLiif
#| fig-cap: "Item Information Curves from Rasch Item Response Theory Model."
#| fig-alt: "Item Information Curves from Rasch Item Response Theory Model."

plot(raschModel, type = "infotrace", facet_items = FALSE)
```

```{r}
#| label: fig-onePLiifFacet
#| fig-cap: "Item Information Curves from Rasch Item Response Theory Model."
#| fig-alt: "Item Information Curves from Rasch Item Response Theory Model."
...
### CFA {#sec-irt-onePLcfa}

A one-parameter logistic model can also be fit in a [CFA](#sec-cfa) framework, sometimes called item factor analysis.\index{factor analysis!confirmatory}
...
maximalRelia(onePLModel_cfa_fit)

onePLModel_cfa_factorScores <- lavPredict(onePLModel_cfa_fit)
```

A path diagram of the one-parameter item factor analysis is in @fig-onePLifa.\index{item response theory!one-parameter model}

```{r}
#| label: fig-onePLifa
#| fig-cap: "Item Factor Analysis Diagram of One-Parameter Logistic Model."
#| fig-alt: "Item Factor Analysis Diagram of One-Parameter Logistic Model."
...
### Mixed Model {#sec-irt-onePLmixed}
...
## Two-Parameter Logistic Model {#sec-irt-twoPL}

A two-parameter logistic (2PL) IRT model is a model fit to dichotomous data, which estimates a different [difficulty](#sec-itemDifficulty) ($b$) and [discrimination](#sec-itemDiscrimination) ($a$) parameter for each item.\index{item response theory!two-parameter model}\index{item response theory!item difficulty}\index{item response theory!item discrimination}
2PL models were fit using the `mirt` package [@R-mirt].\index{item response theory!two-parameter model}

### Fit Model {#sec-irt-twoPLfit}
...
### Model Summary {#sec-irt-twoPLoutput}
...
### Factor Scores {#sec-irt-twoPLfactorScores}

```{r}
twoPLModel_factorScores <- fscores(twoPLModel, full.scores.SE = TRUE)
```

### Plots {#sec-irt-twoPLplots}

#### Test Curves {#sec-irt-twoPLtestCurves}

The test curves suggest that the measure is most [reliable](#sec-reliability) (i.e., provides the most [information](#sec-irtReliability) has the smallest [standard error of measurement](#sec-irtReliability)) at lower levels of the construct.\index{item response theory!information}\index{reliability}\index{item response theory!standard error of measurement}

##### Test Characteristic Curve {#sec-irt-twoPLtcc}

A test characteristic curve ([TCC](#sec-icc)) plot of the expected total score as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-twoPLtcc.\index{item response theory!item characteristic curve}

```{r}
#| label: fig-twoPLtcc
#| fig-cap: "Test Characteristic Curve From Two-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Test Characteristic Curve From Two-Parameter Logistic Item Response Theory Model."

plot(twoPLModel, type = "score")
```

##### Test Information Curve {#sec-irt-twoPLtif}

A plot of [test information](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-twoPLtif.\index{item response theory!information}\index{item response theory!test information curve}

```{r}
#| label: fig-twoPLtif
#| fig-cap: "Test Information Curve From Two-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Test Information Curve From Two-Parameter Logistic Item Response Theory Model."

plot(twoPLModel, type = "info")
```

##### Test Reliability {#sec-irt-twoPLreliability}
...
```{r}
empirical_rxx(twoPLModel_factorScores)
```

A plot of test [reliability](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-twoPLreliability.\index{reliability}

```{r}
#| label: fig-twoPLreliability
#| fig-cap: "Test Reliability From Two-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Test Reliability From Two-Parameter Logistic Item Response Theory Model."

plot(twoPLModel, type = "rxx")
```

##### Test Standard Error of Measurement {#sec-irt-twoPLsem}

A plot of test [standard error of measurement](#sec-irtReliability) (SEM) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-twoPLsem.\index{item response theory!standard error of measurement}

```{r}
#| label: fig-twoPLsem
#| fig-cap: "Test Standard Error of Measurement From Two-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Test Standard Error of Measurement From Two-Parameter Logistic Item Response Theory Model."

plot(twoPLModel, type = "SE")
```

##### Test Information Curve and Standard Errors {#sec-irt-twoPLtifSEM}

A plot of [test information](#sec-irtReliability) and [standard error of measurement](#sec-irtReliability) (SEM) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-twoPLtifSEM.\index{item response theory!information}\index{reliability}\index{item response theory!standard error of measurement}\index{item response theory!test information curve}

```{r}
#| label: fig-twoPLtifSEM
#| fig-cap: "Test Information Curve and Standard Error of Measurement From Two-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Test Information Curve and Standard Error of Measurement From Two-Parameter Logistic Item Response Theory Model."

plot(twoPLModel, type = "infoSE")
```

#### Item Curves {#sec-irt-twoPLitemCurves}

##### Item Characteristic Curves {#sec-irt-twoPLicc}

Item characteristic curve ([ICC](#sec-icc)) plots of the probability of item endorsement (or getting the item correct) as a function of a person's level on the latent construct (theta; $\theta$) are in Figures [-@fig-twoPLicc] and [-@fig-twoPLiccFacet].\index{item response theory!item characteristic curve}

```{r}
#| label: fig-twoPLicc
#| fig-cap: "Item Characteristic Curves From Two-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Item Characteristic Curves From Two-Parameter Logistic Item Response Theory Model."

plot(twoPLModel, type = "itemscore", facet_items = FALSE)
```

```{r}
#| label: fig-twoPLiccFacet
#| fig-cap: "Item Characteristic Curves From Two-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Item Characteristic Curves From Two-Parameter Logistic Item Response Theory Model."
...
##### Item Information Curves {#sec-irt-twoPLiif}

Plots of [item information](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) are in Figures [-@fig-twoPLiif] and [-@fig-twoPLiifFacet].\index{item response theory!information}\index{item response theory!item information curve}

```{r}
#| label: fig-twoPLiif
#| fig-cap: "Item Information Curves From Two-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Item Information Curves From Two-Parameter Logistic Item Response Theory Model."

plot(twoPLModel, type = "infotrace", facet_items = FALSE)
```

```{r}
#| label: fig-twoPLiifFacet
#| fig-cap: "Item Information Curves From Two-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Item Information Curves From Two-Parameter Logistic Item Response Theory Model."
...
The [`petersenlab`](https://github.com/DevPsyLab/petersenlab) package [@R-petersenlab] contains the `discriminationToFactorLoading()` function that converts discrimination parameters to standardized factor loadings.\index{petersenlab package}
...
### CFA {#sec-irt-twoPLcfa}

A two-parameter logistic model can also be fit in a [CFA](#sec-cfa) framework, sometimes called item factor analysis.\index{factor analysis}
...
maximalRelia(twoPLModel_cfa_fit)
...
```{r}
#| label: fig-twoPLifa
#| fig-cap: "Item Factor Analysis Diagram of Two-Parameter Logistic Model."
#| fig-alt: "Item Factor Analysis Diagram of Two-Parameter Logistic Model."
...
## Two-Parameter Multidimensional Logistic Model {#sec-irt-twoPLmultidimensional}

A 2PL multidimensional IRT model is a model that allows multiple dimensions (latent factors) and is fit to dichotomous data, which estimates a different [difficulty](#sec-itemDifficulty) ($b$) and [discrimination](#sec-itemDiscrimination) ($a$) parameter for each item.\index{item response theory!two-parameter model}\index{item response theory!multidimensional model}\index{item response theory!item difficulty}\index{item response theory!item discrimination}
Multidimensional IRT models were fit using the `mirt` package [@R-mirt].\index{item response theory!multidimensional model}
In this example, I estimate a 2PL multidimensional IRT model by estimating two factors.\index{item response theory!two-parameter model}\index{item response theory!multidimensional model}

### Fit Model {#sec-irt-twoPLmultidimensionalFit}
...
### Model Summary {#sec-irt-twoPLmultidimensionalOutput}
...
### Factor Scores {#sec-irt-twoPLmultidimensionalFactorScores}

```{r}
twoPL2FactorModel_factorScores <- fscores(twoPL2FactorModel, full.scores.SE = TRUE)
```

### Compare model fit {#sec-irt-twoPLmultidimensionalCompare}
...
### Plots {#sec-irt-twoPLmultidimensionalPlots}

#### Test Curves {#sec-irt-twoPLmultidimensionalTestCurves}

##### Test Characteristic Curve {#sec-irt-twoPLmultidimensionalTCC}

A test characteristic curve ([TCC](#sec-icc)) plot of the expected total score as a function of a person's level on each latent construct (theta; $\theta$) is in @fig-twoPLmultidimensionalTCC.\index{item response theory!item characteristic curve}

```{r}
#| label: fig-twoPLmultidimensionalTCC
#| fig-cap: "Test Characteristic Curve From Two-Parameter Multidimensional Item Response Theory Model."
#| fig-alt: "Test Characteristic Curve From Two-Parameter Multidimensional Item Response Theory Model."

plot(twoPL2FactorModel, type = "score")
```

##### Test Information Curve {#sec-irt-twoPLmultidimensionalTIF}

A plot of [test information](#sec-irtReliability) as a function of a person's level on each latent construct (theta; $\theta$) is in @fig-twoPLmultidimensionalTIF.\index{item response theory!information}\index{item response theory!test information curve}

```{r}
#| label: fig-twoPLmultidimensionalTIF
#| fig-cap: "Test Information Curve From Two-Parameter Multidimensional Item Response Theory Model."
#| fig-alt: "Test Information Curve From Two-Parameter Multidimensional Item Response Theory Model."

plot(twoPL2FactorModel, type = "info")
```

##### Test Reliability {#sec-irt-twoPLmultidimensionalReliability}
...
marginal_rxx(twoPL2FactorModel)
```

The estimate of empirical [reliability](#sec-irtReliability) is below:\index{reliability}

```{r}
empirical_rxx(twoPL2FactorModel_factorScores)
```

##### Test Standard Error of Measurement {#sec-irt-twoPLmultidimensionalSEM}

A plot of test [standard error of measurement](#sec-irtReliability) (SEM) as a function of a person's level on each latent construct (theta; $\theta$) is in @fig-twoPLmultidimensionalSEM.\index{item response theory!standard error of measurement}

```{r}
#| label: fig-twoPLmultidimensionalSEM
#| fig-cap: "Test Standard Error of Measurement From Two-Parameter Multidimensional Item Response Theory Model."
#| fig-alt: "Test Standard Error of Measurement From Two-Parameter Multidimensional Item Response Theory Model."

plot(twoPL2FactorModel, type = "SE")
```

#### Item Curves {#sec-irt-twoPLmultidimensionalItemCurves}

##### Item Characteristic Curves {#sec-irt-twoPLmultidimensionalICC}

Item characteristic curve ([ICC](#sec-icc)) plots of the probability of item endorsement (or getting the item correct) as a function of a person's level on each latent construct (theta; $\theta$) are in Figures [-@fig-twoPLmultidimensionalICC] and [-@fig-twoPLmultidimensionalICCfacet].\index{item response theory!item characteristic curve}

```{r}
#| label: fig-twoPLmultidimensionalICC
#| fig-cap: "Item Characteristic Curves From Two-Parameter Multidimensional Item Response Theory Model."
#| fig-alt: "Item Characteristic Curves From Two-Parameter Multidimensional Item Response Theory Model."

plot(twoPL2FactorModel, type = "itemscore", facet_items = FALSE)
```

```{r}
#| label: fig-twoPLmultidimensionalICCfacet
#| fig-cap: "Item Characteristic Curves From Two-Parameter Multidimensional Item Response Theory Model."
#| fig-alt: "Item Characteristic Curves From Two-Parameter Multidimensional Item Response Theory Model."
...
##### Item Information Curves {#sec-irt-twoPLmultidimensionalIIF}

Plots of [item information](#sec-irtReliability) as a function of a person's level on each latent construct (theta; $\theta$) are in Figures [-@fig-twoPLmultidimensionalIIF] and [-@fig-twoPLmultidimensionalIIFfacet].\index{item response theory!information}\index{item response theory!item information curve}

```{r}
#| label: fig-twoPLmultidimensionalIIF
#| fig-cap: "Item Information Curves From Two-Parameter Multidimensional Item Response Theory Model."
#| fig-alt: "Item Information Curves From Two-Parameter Multidimensional Item Response Theory Model."

plot(twoPL2FactorModel, type = "infotrace", facet_items = FALSE)
```

```{r}
#| label: fig-twoPLmultidimensionalIIFfacet
#| fig-cap: "Item Information Curves From Two-Parameter Multidimensional Item Response Theory Model."
#| fig-alt: "Item Information Curves From Two-Parameter Multidimensional Item Response Theory Model."
...
### CFA {#sec-irt-twoPLmultidimensionalCFA}

A two-parameter multidimensional model can also be fit in a [CFA](#sec-cfa) framework, sometimes called item factor analysis.\index{factor analysis!confirmatory}
...
maximalRelia(twoPLModelMultidimensional_cfa_fit)

twoPLModelMultidimensional_cfa_factorScores <- lavPredict(twoPLModelMultidimensional_cfa_fit)
```

```{r}
#| label: fig-twoPLmultidimensionalIFA
#| fig-cap: "Item Factor Analysis Diagram of Two-Parameter Multidimensional Logistic Model."
#| fig-alt: "Item Factor Analysis Diagram of Two-Parameter Multidimensional Logistic Model."
...
## Three-Parameter Logistic Model {#sec-irt-threePL}

A three-parameter logistic (3PL) IRT model is a model fit to dichotomous data, which estimates a different [difficulty](#sec-itemDifficulty) ($b$), [discrimination](#sec-itemDiscrimination) ($a$), and [guessing](#sec-itemGuessing) parameter for each item.\index{item response theory!three-parameter model}\index{item response theory!item difficulty}\index{item response theory!item discrimination}\index{item response theory!item guessing}
3PL models were fit using the `mirt` package [@R-mirt].\index{item response theory!three-parameter model}

### Fit Model {#sec-irt-threePLfit}
...
### Model Summary {#sec-irt-threePLoutput}
...
### Factor Scores {#sec-irt-threePLfactorScores}

```{r}
threePLModel_factorScores <- fscores(threePLModel, full.scores.SE = TRUE)
```

### Plots {#sec-irt-threePLplots}

#### Test Curves {#sec-irt-threePLtestCurves}

The test curves suggest that the measure is most [reliable](#sec-reliability) (i.e., provides the most [information](#sec-irtReliability) has the smallest [standard error of measurement](#sec-irtReliability)) at lower levels of the construct.\index{item response theory!information}\index{reliability}\index{item response theory!standard error of measurement}

##### Test Characteristic Curve {#sec-irt-threePLtcc}

A test characteristic curve ([TCC](#sec-icc)) plot of the expected total score as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-threePLtcc.\index{item response theory!item characteristic curve}

```{r}
#| label: fig-threePLtcc
#| fig-cap: "Test Characteristic Curve From Three-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Test Characteristic Curve From Three-Parameter Logistic Item Response Theory Model."

plot(threePLModel, type = "score")
```

##### Test Information Curve {#sec-irt-threePLtif}

A plot of [test information](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-threePLtif.\index{item response theory!information}

```{r}
#| label: fig-threePLtif
#| fig-cap: "Test Information Curve From Three-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Test Information Curve From Three-Parameter Logistic Item Response Theory Model."

plot(threePLModel, type = "info")
```

##### Test Reliability {#sec-irt-threePLreliability}
...
```{r}
empirical_rxx(threePLModel_factorScores)
```

A plot of test [reliability](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-threePLreliability.\index{reliability}

```{r}
#| label: fig-threePLreliability
#| fig-cap: "Test Reliability From Three-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Test Reliability From Three-Parameter Logistic Item Response Theory Model."

plot(threePLModel, type = "rxx")
```

##### Test Standard Error of Measurement {#sec-irt-threePLsem}

A plot of test [standard error of measurement](#sec-irtReliability) (SEM) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-threePLsem.\index{item response theory!standard error of measurement}

```{r}
#| label: fig-threePLsem
#| fig-cap: "Test Standard Error of Measurement From Three-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Test Standard Error of Measurement From Three-Parameter Logistic Item Response Theory Model."

plot(threePLModel, type = "SE")
```

##### Test Information Curve and Standard Errors {#sec-irt-threePLtifSEM}

A plot of [test information](#sec-irtReliability) and [standard error of measurement](#sec-irtReliability) (SEM) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-threePLtifSEM.\index{item response theory!information}\index{reliability}\index{item response theory!standard error of measurement}\index{item response theory!test information curve}

```{r}
#| label: fig-threePLtifSEM
#| fig-cap: "Test Information Curve and Standard Error of Measurement From Three-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Test Information Curve and Standard Error of Measurement From Three-Parameter Logistic Item Response Theory Model."

plot(threePLModel, type = "infoSE")
```

#### Item Curves {#sec-irt-threePLitemCurves}

##### Item Characteristic Curves {#sec-irt-threePLicc}

Item characteristic curve ([ICC](#sec-icc)) plots of the probability of item endorsement (or getting the item correct) as a function of a person's level on the latent construct (theta; $\theta$) are in Figures [-@fig-threePLicc] and [-@fig-threePLiccFacet].\index{item response theory!item characteristic curve}

```{r}
#| label: fig-threePLicc
#| fig-cap: "Item Characteristic Curves From Three-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Item Characteristic Curves From Three-Parameter Logistic Item Response Theory Model."

plot(threePLModel, type = "itemscore", facet_items = FALSE)
```

```{r}
#| label: fig-threePLiccFacet
#| fig-cap: "Item Characteristic Curves From Three-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Item Characteristic Curves From Three-Parameter Logistic Item Response Theory Model."
...
##### Item Information Curves {#sec-irt-threePLiif}

Plots of [item information](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) are in Figures [-@fig-threePLiif] and [-@fig-threePLiifFacet].\index{item response theory!information}\index{item response theory!item information curve}

```{r}
#| label: fig-threePLiif
#| fig-cap: "Item Information Curves From Three-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Item Information Curves From Three-Parameter Logistic Item Response Theory Model."

plot(threePLModel, type = "infotrace", facet_items = FALSE)
```

```{r}
#| label: fig-threePLiifFacet
#| fig-cap: "Item Information Curves From Three-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Item Information Curves From Three-Parameter Logistic Item Response Theory Model."
...
## Four-Parameter Logistic Model {#sec-irt-fourPL}

A four-parameter logistic (4PL) IRT model is a model fit to dichotomous data, which estimates a different [difficulty](#sec-itemDifficulty) ($b$), [discrimination](#sec-itemDiscrimination) ($a$), [guessing](#sec-itemGuessing), and [careless errors](#sec-itemCarelessErrors) parameter for each item.\index{item response theory!three-parameter model}\index{item response theory!item difficulty}\index{item response theory!item discrimination}\index{item response theory!item guessing}\index{item response theory!item careless errors}
4PL models were fit using the `mirt` package [@R-mirt].\index{item response theory!four-parameter model}

### Fit Model {#sec-irt-fourPLfit}
...
### Model Summary {#sec-irt-fourPLoutput}
...
### Factor Scores {#sec-irt-fourPLfactorScores}

```{r}
fourPLModel_factorScores <- fscores(fourPLModel, full.scores.SE = TRUE)
```

### Plots {#sec-irt-fourPLplots}

#### Test Curves {#sec-irt-fourPLtestCurves}

The test curves suggest that the measure is most [reliable](#sec-reliability) (i.e., provides the most [information](#sec-irtReliability) has the smallest [standard error of measurement](#sec-irtReliability)) at middle to lower levels of the construct.\index{item response theory!information}\index{reliability}\index{item response theory!standard error of measurement}

##### Test Characteristic Curve {#sec-irt-fourPLtcc}

A test characteristic curve ([TCC](#sec-icc)) plot of the expected total score as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-fourPLtcc.\index{item response theory!item characteristic curve}

```{r}
#| label: fig-fourPLtcc
#| fig-cap: "Test Characteristic Curve From Four-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Test Characteristic Curve From Four-Parameter Logistic Item Response Theory Model."

plot(fourPLModel, type = "score")
```

##### Test Information Curve {#sec-irt-fourPLtif}

A plot of [test information](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-fourPLtif.\index{item response theory!information}\index{item response theory!test information curve}

```{r}
#| label: fig-fourPLtif
#| fig-cap: "Test Information Curve From Four-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Test Information Curve From Four-Parameter Logistic Item Response Theory Model."

plot(fourPLModel, type = "info")
```

##### Test Reliability {#sec-irt-fourPLreliability}
...
```{r}
empirical_rxx(fourPLModel_factorScores)
```

A plot of test [reliability](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-fourPLreliability.\index{reliability}

```{r}
#| label: fig-fourPLreliability
#| fig-cap: "Test Reliability From Four-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Test Reliability From Four-Parameter Logistic Item Response Theory Model."

plot(fourPLModel, type = "rxx")
```

##### Test Standard Error of Measurement {#sec-irt-fourPLsem}

A plot of test [standard error of measurement](#sec-irtReliability) (SEM) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-fourPLsem.\index{item response theory!standard error of measurement}

```{r}
#| label: fig-fourPLsem
#| fig-cap: "Test Standard Error of Measurement From Four-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Test Standard Error of Measurement From Four-Parameter Logistic Item Response Theory Model."

plot(fourPLModel, type = "SE")
```

##### Test Information Curve and Standard Errors {#sec-irt-fourPLtifSEM}

A plot of [test information](#sec-irtReliability) and [standard error of measurement](#sec-irtReliability) (SEM) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-fourPLtifSEM.\index{item response theory!information}\index{reliability}\index{item response theory!standard error of measurement}\index{item response theory!test information curve}

```{r}
#| label: fig-fourPLtifSEM
#| fig-cap: "Test Information Curve and Standard Error of Measurement From Four-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Test Information Curve and Standard Error of Measurement From Four-Parameter Logistic Item Response Theory Model."

plot(fourPLModel, type = "infoSE")
```

#### Item Curves {#sec-irt-fourPLitemCurves}

##### Item Characteristic Curves {#sec-irt-fourPLicc}

Item characteristic curve ([ICC](#sec-icc)) plots of the probability of item endorsement (or getting the item correct) as a function of a person's level on the latent construct (theta; $\theta$) are in Figures [-@fig-fourPLicc] and [-@fig-fourPLiccFacet].\index{item response theory!item characteristic curve}

```{r}
#| label: fig-fourPLicc
#| fig-cap: "Item Characteristic Curves From Four-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Item Characteristic Curves From Four-Parameter Logistic Item Response Theory Model."

plot(fourPLModel, type = "itemscore", facet_items = FALSE)
```

```{r}
#| label: fig-fourPLiccFacet
#| fig-cap: "Item Characteristic Curves From Four-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Item Characteristic Curves From Four-Parameter Logistic Item Response Theory Model."
...
##### Item Information Curves {#sec-irt-fourPLiif}

Plots of [item information](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) are in Figures [-@fig-fourPLiif] and [-@fig-fourPLiifFacet].\index{item response theory!information}\index{item response theory!item information curve}

```{r}
#| label: fig-fourPLiif
#| fig-cap: "Item Information Curves From Four-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Item Information Curves From Four-Parameter Logistic Item Response Theory Model."

plot(fourPLModel, type = "infotrace", facet_items = FALSE)
```

```{r}
#| label: fig-fourPLiifFacet
#| fig-cap: "Item Information Curves From Four-Parameter Logistic Item Response Theory Model."
#| fig-alt: "Item Information Curves From Four-Parameter Logistic Item Response Theory Model."
...
## Graded Response Model {#sec-irt-gradedResponseModel}

A two-parameter graded response model (GRM) is an IRT model fit to polytomous data (in this case, a 1–4 likert scale), which estimates a different [difficulty](#sec-itemDifficulty) ($b$) and [discrimination](#sec-itemDiscrimination) ($a$) parameter for each item.\index{item response theory!graded response model}\index{item response theory!item difficulty}\index{item response theory!item discrimination}
It estimates four parameters for each item: [difficulty](#sec-itemDifficulty) [for each of three threshold transitions: 1–2 ($b_1$), 2–3 ($b_2$), and 3–4 ($b_3$)] and [discrimination](#sec-itemDiscrimination) ($a$).\index{item response theory!graded response model}\index{item response theory!item difficulty}\index{item response theory!item discrimination}
GRM models were fit using the `mirt` package [@R-mirt].\index{item response theory!graded response model}

### Fit Model {#sec-irt-gradedResponseModelFit}
...
save(
  gradedResponseModel,
  file = "./data/gradedResponseModel.RData"
)
```

### Model Summary {#sec-irt-gradedResponseModelOutput}
...
### Factor Scores {#sec-irt-gradedResponseModelFactorScores}

```{r}
gradedResponseModel_factorScores <- fscores(gradedResponseModel, full.scores.SE = TRUE)
```

### Plots {#sec-irt-gradedResponseModelPlots}

#### Test Curves {#sec-irt-gradedResponseModelTestCurves}

The test curves suggest that the measure is most [reliable](#sec-reliability) (i.e., provides the most [information](#sec-irtReliability) and has the smallest [standard error of measurement](#sec-irtReliability)) across a wide range of construct.\index{item response theory!information}\index{reliability}\index{item response theory!standard error of measurement}
...
##### Test Characteristic Curve {#sec-irt-gradedResponseModelTCC}

A test characteristic curve ([TCC](#sec-icc)) plot of the expected total score as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-gradedResponseModelTCC.\index{item response theory!item characteristic curve}

```{r}
#| label: fig-gradedResponseModelTCC
#| fig-cap: "Test Characteristic Curve From Graded Response Model."
#| fig-alt: "Test Characteristic Curve From Graded Response Model."

plot(gradedResponseModel, type = "score")
```

##### Test Information Curve {#sec-irt-gradedResponseModelTIF}

A plot of [test information](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-gradedResponseModelTIF.\index{item response theory!information}\index{item response theory!test information curve}

```{r}
#| label: fig-gradedResponseModelTIF
#| fig-cap: "Test Information Curve From Graded Response Model."
#| fig-alt: "Test Information Curve From Graded Response Model."

plot(gradedResponseModel, type = "info")
```

##### Test Reliability {#sec-irt-gradedResponseModelReliability}
...
```{r}
empirical_rxx(gradedResponseModel_factorScores)
```

A plot of test [reliability](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-gradedResponseModelReliability.\index{reliability}

```{r}
#| label: fig-gradedResponseModelReliability
#| fig-cap: "Test Reliability From Graded Response Model."
#| fig-alt: "Test Reliability From Graded Response Model."

plot(gradedResponseModel, type = "rxx")
```

##### Test Standard Error of Measurement {#sec-irt-gradedResponseModelSEM}

A plot of test [standard error of measurement](#sec-irtReliability) (SEM) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-gradedResponseModelSEM.\index{item response theory!standard error of measurement}

```{r}
#| label: fig-gradedResponseModelSEM
#| fig-cap: "Test Standard Error of Measurement From Graded Response Model."
#| fig-alt: "Test Standard Error of Measurement From Graded Response Model."

plot(gradedResponseModel, type = "SE")
```

##### Test Information Curve and Standard Errors {#sec-irt-gradedResponseModelTIFsem}

A plot of [test information](#sec-irtReliability) and [standard error of measurement](#sec-irtReliability) (SEM) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-gradedResponseModelTIFsem.\index{item response theory!information}\index{reliability}\index{item response theory!standard error of measurement}\index{item response theory!test information curve}

```{r}
#| label: fig-gradedResponseModelTIFsem
#| fig-cap: "Test Information Curve and Standard Error of Measurement From Graded Response Model."
#| fig-alt: "Test Information Curve and Standard Error of Measurement From Graded Response Model."

plot(gradedResponseModel, type = "infoSE")
```

#### Item Curves {#sec-irt-gradedResponseModelItemCurves}

##### Item Characteristic Curves {#sec-irt-gradedResponseModelICC}

Item characteristic curve ([ICC](#sec-icc)) plots of the expected score on the item as a function of a person's level on the latent construct (theta; $\theta$) are in [Figures -@fig-gradedResponseModelICC] and [-@fig-gradedResponseModelICCfacet].\index{item response theory!item characteristic curve}

```{r}
#| label: fig-gradedResponseModelICC
#| fig-cap: "Item Characteristic Curves From Graded Response Model."
#| fig-alt: "Item Characteristic Curves From Graded Response Model."

plot(gradedResponseModel, type = "itemscore", facet_items = FALSE)
```

```{r}
#| label: fig-gradedResponseModelICCfacet
#| fig-cap: "Item Characteristic Curves From Graded Response Model."
#| fig-alt: "Item Characteristic Curves From Graded Response Model."
...
##### Item Information Curves {#sec-irt-gradedResponseModelIIF}

Plots of [item information](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) are in Figures [-@fig-gradedResponseModelIIF] and [-@fig-gradedResponseModelIIFfacet].\index{item response theory!information}\index{item response theory!item information curve}

```{r}
#| label: fig-gradedResponseModelIIF
#| fig-cap: "Item Information Curves From Graded Response Model."
#| fig-alt: "Item Information Curves From Graded Response Model."

plot(gradedResponseModel, type = "infotrace", facet_items = FALSE)
```

```{r}
#| label: fig-gradedResponseModelIIFfacet
#| fig-cap: "Item Information Curves From Graded Response Model."
#| fig-alt: "Item Information Curves From Graded Response Model."
...
##### Item Response Category Characteristic Curves {#sec-irt-gradedResponseModelIRCCC}

A plot of the probability of item threshold endorsement as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-gradedResponseModelIRCCC.\index{item response theory!item response category characteristic curve}

```{r}
#| label: fig-gradedResponseModelIRCCC
#| fig-cap: "Item Response Category Characteristic Curves From Graded Response Model."
#| fig-alt: "Item Response Category Characteristic Curves From Graded Response Model."

plot(gradedResponseModel, type = "trace")
```

##### Item Boundary Characteristic Curves (aka Item Operation Characteristic Curves) {#sec-irt-gradedResponseModelIOCC}

A plot of item boundary characteristic curves is in @fig-gradedResponseModelIOCC.\index{item response theory!item boundary characteristic curve}
...
```{r}
#| label: fig-gradedResponseModelIOCC
#| fig-cap: "Item Boundary Category Characteristic Curves From Graded Response Model."
#| fig-alt: "Item Boundary Category Characteristic Curves From Graded Response Model."
...
A two-parameter graded response model can also be fit in a [CFA](#sec-cfa) framework, sometimes called item factor analysis.\index{factor analysis}
The item factor analysis models were fit in the `lavaan` package [@R-lavaan].\index{factor analysis}

```{r}
twoParameterGRM_cfa <- '
# Factor Loadings (i.e., discrimination parameters)
latent =~ loading1*Comfort + loading2*Work + loading3*Future + 
  loading4*Benefit

# Item Thresholds (i.e., difficulty parameters)
Comfort | b11*t1 + b12*t2 + b13*t3
Work    | b21*t1 + b22*t2 + b23*t3
Future  | b31*t1 + b32*t2 + b33*t3
Benefit | b41*t1 + b42*t2 + b43*t3
'

twoParameterGRM_cfa_fit <- sem(
  model = twoParameterGRM_cfa,
  data = Science,
  ordered = c("Comfort", "Work", "Future", "Benefit"),
  mimic = "Mplus",
  estimator = "WLSMV",
  std_lv = TRUE,
  parameterization = "theta")
...
residuals(twoParameterGRM_cfa_fit, type = "cor")

modificationindices(twoParameterGRM_cfa_fit, sort = TRUE)

compRelSEM(twoParameterGRM_cfa_fit)
AVE(twoParameterGRM_cfa_fit)
maximalRelia(twoParameterGRM_cfa_fit)

twoParameterGRM_cfa_factorScores <- lavPredict(twoParameterGRM_cfa_fit)
```

```{r}
#| label: fig-grmCFA
#| fig-cap: "Item Factor Analysis Diagram of Two-Parameter Graded Response Model."
#| fig-alt: "Item Factor Analysis Diagram of Two-Parameter Graded Response Model."
...
## Bifactor Model {#sec-irt-bifactor}

Bifactor models were fit using the `mirt` package [@R-mirt].\index{item response theory!bifactor model}\index{factor analysis!bifactor}

### Fit Model {#sec-irt-bifactorFit}

```{r}
#| output: false

specific <- "S1 = 7,9,10,11,13,15,17,18,21,22,24,27,31
             S2 = 1,3,6,8,16,29,32
             S3 = 2,4,5,12,14,19,20,23,25,26,28,30"

bifactorModel <- bfactor(
  data = mydataSAT,
  model = specific,
  SE = TRUE)
```

### Model Summary {#sec-irt-bifactorOutput}

```{r}
summary(bifactorModel)
coef(bifactorModel, simplify = TRUE)
```

### Bifactor Indices {#sec-irt-bifactorIndices}
...
### Factor Scores {#sec-irt-bifactorFactorScores}

One can obtain factor scores (i.e., theta; and their associated standard errors) for each participant using the `fscores()` function.

```{r}
bifactorModel_factorScores <- fscores(
  bifactorModel,
  full.scores.SE = TRUE,
  QMC = TRUE)
```

### Plots {#sec-irt-bifactorPlots}

#### Test Curves {#sec-irt-bifactorTestCurves}

The test curves suggest that the measure is most [reliable](#sec-reliability) (i.e., provides the most [information](#sec-irtReliability) has the smallest [standard error of measurement](#sec-irtReliability)) at lower levels of the construct.\index{item response theory!information}\index{item response theory!standard error of measurement}\index{reliability!standard error of measurement}

##### Test Characteristic Curve {#sec-irt-bifactorTCC}

A test characteristic curve ([TCC](#sec-icc)) plot of the expected total score as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-bifactorTCC.\index{item response theory!item characteristic curve}

```{r}
#| label: fig-bifactorTCC
#| fig-cap: "Test Characteristic Curve From Bifactor Item Response Theory Model."
#| fig-alt: "Test Characteristic Curve From Bifactor Item Response Theory Model."
#| eval: false

plot(bifactorModel, type = "score")
```

##### Test Information Curve {#sec-irt-bifactorTIF}

A plot of [test information](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-bifactorTIF.\index{item response theory!information}\index{item response theory!test information curve}

```{r}
#| label: fig-bifactorTIF
#| fig-cap: "Test Information Curve From Bifactor Item Response Theory Model."
#| fig-alt: "Test Information Curve From Bifactor Item Response Theory Model."
#| eval: false

plot(bifactorModel, type = "info")
```

##### Test Reliability {#sec-irt-bifactorReliability}
...
marginal_rxx(bifactorModel)
```

The estimate of empirical [reliability](#sec-irtReliability) is below:\index{reliability}

```{r}
empirical_rxx(bifactorModel_factorScores)
```

A plot of test [reliability](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-bifactorReliability.\index{reliability}

```{r}
#| label: fig-bifactorReliability
#| fig-cap: "Test Reliability From Bifactor Item Response Theory Model."
#| fig-alt: "Test Reliability From Bifactor Item Response Theory Model."
#| eval: false

plot(bifactorModel, type = "rxx")
```

##### Test Standard Error of Measurement {#sec-irt-bifactorSEM}

A plot of test [standard error of measurement](#sec-irtReliability) (SEM) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-bifactorSEM.\index{reliability!standard error of measurement}

```{r}
#| label: fig-bifactorSEM
#| fig-cap: "Test Standard Error of Measurement From Bifactor Item Response Theory Model."
#| fig-alt: "Test Standard Error of Measurement From Bifactor Item Response Theory Model."
#| eval: false

plot(bifactorModel, type = "SE")
```

##### Test Information Curve and Test Standard Error of Measurement {#sec-irt-bifactorTIFsem}

A plot of [test information](#sec-irtReliability) and [standard error of measurement](#sec-irtReliability) (SEM) as a function of a person's level on the latent construct (theta; $\theta$) is in @fig-bifactorTIFsem.\index{item response theory!information}\index{item response theory!standard error of measurement}\index{reliability!standard error of measurement}\index{item response theory!test information curve}

```{r}
#| label: fig-bifactorTIFsem
#| fig-cap: "Test Information Curve and Standard Error of Measurement From Bifactor Item Response Theory Model."
#| fig-alt: "Test Information Curve and Standard Error of Measurement From Bifactor Item Response Theory Model."
#| eval: false

plot(bifactorModel, type = "infoSE")
```

#### Item Curves {#sec-irt-bifactorItemCurves}

##### Item Characteristic Curves {#sec-irt-bifactorICC}

Item characteristic curve ([ICC](#sec-icc)) plots of the probability of item endorsement (or getting the item correct) as a function of a person's level on the latent construct (theta; $\theta$) are in Figures [-@fig-bifactorICC] and [-@fig-bifactorICCfacet].\index{item response theory!item characteristic curve}

```{r}
#| label: fig-bifactorICC
#| fig-cap: "Item Characteristic Curves From Bifactor Item Response Theory Model."
#| fig-alt: "Item Characteristic Curves From Bifactor Item Response Theory Model."
#| eval: false

plot(bifactorModel, type = "itemscore", facet_items = FALSE)
```

```{r}
#| label: fig-bifactorICCfacet
#| fig-cap: "Item Characteristic Curves From Bifactor Item Response Theory Model."
#| fig-alt: "Item Characteristic Curves From Bifactor Item Response Theory Model."
#| eval: false

plot(bifactorModel, type = "itemscore", facet_items = TRUE)
```

##### Item Information Curves {#sec-irt-bifactorIIF}

Plots of [item information](#sec-irtReliability) as a function of a person's level on the latent construct (theta; $\theta$) are in Figures [-@fig-bifactorIIF] and [-@fig-bifactorIIFfacet].\index{item response theory!information}\index{item response theory!item information curve}

```{r}
#| label: fig-bifactorIIF
#| fig-cap: "Item Information Curves from Bifactor Item Response Theory Model."
#| fig-alt: "Item Information Curves from Bifactor Item Response Theory Model."
#| eval: false

plot(bifactorModel, type = "infotrace", facet_items = FALSE)
```

```{r}
#| label: fig-bifactorIIFfacet
#| fig-cap: "Item Information Curves from Bifactor Item Response Theory Model."
#| fig-alt: "Item Information Curves from Bifactor Item Response Theory Model."
#| eval: false

plot(bifactorModel, type = "infotrace", facet_items = TRUE)
```

### CFA {#sec-irt-bifactorCFA}

A bifactor model can also be fit in a [CFA](#sec-cfa) framework, sometimes called item factor analysis.\index{factor analysis!confirmatory}\index{item response theory!bifactor model}\index{factor analysis!bifactor}
The item factor analysis models were fit in the `lavaan` package [@R-lavaan].\index{factor analysis!confirmatory}\index{item response theory!bifactor model}\index{factor analysis!bifactor}
...
residuals(bifactorModel_cfa_fit, type = "cor")

modificationindices(bifactorModel_cfa_fit, sort = TRUE)

compRelSEM(bifactorModel_cfa_fit) # omega_total
compRelSEM(bifactorModel_cfa_fit, true = "omegaH") # omega_hierarchical
compRelSEM(bifactorModel_cfa_fit, return.total = -1, true = list(.TOTAL. = "G")) # proportion of variance in scale composite that is attributable to general factor

AVE(bifactorModel_cfa_fit)
maximalRelia(bifactorModel_cfa_fit)

BifactorIndicesCalculator::bifactorIndices(
  bifactorModel_cfa_fit
)

bifactorModel_cfa_factorScores <- lavPredict(bifactorModel_cfa_fit)
```

A path diagram of the bifactor item factor analysis is in @fig-bifactorIFA.\index{factor analysis!confirmatory}\index{item response theory!bifactor model}\index{factor analysis!bifactor}

```{r}
#| label: fig-bifactorIFA
#| fig-cap: "Item Factor Analysis Diagram of One-Parameter Logistic Model."
#| fig-alt: "Item Factor Analysis Diagram of One-Parameter Logistic Model."
...
IRT holds promise to enable the development of briefer assessments, including short forms and [adaptive assessments](#sec-cat), that have strong [reliability](#sec-reliability) and [validity](#sec-validity).\index{item response theory}\index{brief assessment}\index{adaptive testing}\index{reliability}\index{validity}
However, there are situations where IRT models may not be preferable, such as when assessing a [formative construct](#sec-formativeConstruct), when using [small sample sizes](#sec-sampleSize-irt), or when [assumptions of IRT](#sec-assumptions-irt) are violated.

## Suggested Readings {#sec-readings-irt}

If you are interested in learning more about IRT, I highly recommend the book by @Embretson2000.\index{item response theory}

## Exercises {#sec-exercises-irt}
...
#### Test Information Curve and Test Standard Error of Measurement
...
#### Item Information Curves Category Characteristic Curves
...
### Questions {#sec-exercisesQuestions-irt}
...
What do these estimates of item discrimination indicate?
    d. Provide a figure of the item information curves.
    e. Examining the item information curves, which item provides the most information at upper construct levels (2–4 standard deviations above the mean)?
Which item provides the most information at lower construct levels (2–4 standard deviations below the mean)?
    f. Provide a figure of the test information curve.
    g. Examining the test information curve, where (at what construct levels) does the measure do the best job of assessing?
Based on its information curve, describe what purposes the test would be better- or worse-suited for.
3.
...
### Answers {#sec-exercisesAnswers-irt}
...
```{r}
#| label: fig-exercise-icc
#| fig-cap: "Exercise 1a: Item Characteristic Curves."
#| fig-alt: "Exercise 1a: Item Characteristic Curves."
...
```{r}
#| label: fig-exercise-ibc
#| fig-cap: "Exercise 2b: Item Boundary Characteristic Curves."
#| fig-alt: "Exercise 2b: Item Boundary Characteristic Curves."
...
2.
    ...
    d. Below is a figure of item information curves:

```{r}
#| label: fig-exercise-iic
#| fig-cap: "Exercise 2c: Item Information Curves."
#| fig-alt: "Exercise 2c: Item Information Curves."

plot(gradedResponseModel_ex, type = "infotrace", facet_items = FALSE, main = "Item Information Curves")
```
...
```{r}
#| label: fig-exercise-tic
#| fig-cap: "Exercise 2e: Test Information Curve."
#| fig-alt: "Exercise 2e: Test Information Curve."

plot(gradedResponseModel_ex, type = "info", main = "Test Information Curve")
```
...
## Session Info {#sec-sessionInfo-irt}
