...
The model results for the three-factor model are below:\index{factor analysis!exploratory}\index{orthogonal rotation}
...
The factor loadings and summaries of the model results can be generated with the following syntax:\index{factor analysis!exploratory}\index{oblique rotation}
...
The model results for a given model can be generated using the following syntax:\index{factor analysis!exploratory}\index{oblique rotation}
...
Fit indices are generated using the syntax below and are in Table \@ref(tab:efaOrthogonal_fitIndices).\index{factor analysis!exploratory}\index{orthogonal rotation}
...
```{r efaOrthogonal_fitIndices, echo = FALSE}
...
```{r, eval = FALSE}
efaObliqueLavaan_fitIndices <- fitMeasures(
  efaObliqueLavaan_fit,
  fit.measures = c(
    "chisq", "df", "pvalue",
    "chisq.scaled", "df.scaled", "pvalue.scaled",
...
```{r}
fa3Oblique_lavaan <- lavPredict(efaFactor3ObliqueLavaan_fit)
```
...
factor.plot(fa3Orthogonal_lavaan, cut = 0.5)
```
...
factor.plot(fa3Oblique_lavaan, cut = 0.5)
```
...
pairs.panels(fa3Oblique_lavaan)
```
...
Modification indices are generated using the syntax below and are in Table \@ref(tab:modIndicesCFA).\index{structural equation modeling!modification indices}

```{r}
cfaModelFitModIndices <- modificationindices(
  cfaModelFit,
  sort. = TRUE)
```

```{r modIndicesCFA, echo = FALSE}
head(cfaModelFitModIndices) %>% 
  kable(.,
...
In this example, I use the factor loadings from the three-factor [EFA](#efa) model, as specified in Section \@ref(#oblique-efa).\index{structural equation modeling!exploratory}
