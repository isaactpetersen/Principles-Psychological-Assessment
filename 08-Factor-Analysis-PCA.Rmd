...
Now, it is fast to compute [factor analysis](#factorAnalysis) with computers (e.g., often less than 30 ms).\index{factor analysis!confirmatory}
...
- $\text{Item Intercept}$ represents the constant term that accounts for the expected value of the indicator when the latent factors are zero
...
#### Variables To Include and their Scaling {#variablesToInclude-factorAnalysis}
...
#### Method of Factor Extraction {#methodOfFactorExtraction}
...
##### Exploratory structural equation modeling {#efa-cfa-esem}
...
#### How Many Factors To Retain {#factorsToRetain}
...
- Kaiser-Guttman criterion: in [PCA](#pca), components with eigenvalues greater than 1 or, for [factor analysis](#factorAnalysis), factors with eigenvalues greater than zero\index{factor analysis!decisions}\index{principal component analysis}\index{factor analysis}\index{eigenvalue}
...
An example of a factor structure from an orthogonal rotation is shown in Figure \@ref(fig:orthogonalRotation).\index{factor analysis!decisions}\index{factor analysis}\index{principal component analysis}\index{rotation}\index{simple structure}\index{orthogonal rotation}
...
Often this is not the case, so we choose our rotation method in order to decide if the factors can be correlated (an oblique rotation) or if the factors will be uncorrelated (an orthogonal rotation).\index{factor analysis!decisions}\index{factor analysis}\index{principal component analysis}\index{rotation}\index{orthogonal rotation}\index{oblique rotation}
...
#### Model Selection and Interpretation {#modelSelectionInterpretation}
...
numColumns <- ncol(summaryTableTransposed)

digits <- rep(2, length.out = numColumns)
for (j in seq_len(numColumns)) {
  if (is.numeric(summaryTableTransposed[, j])) summaryTableTransposed[, j] = sprintf(sprintf('$%%.%sf$', digits[j]), summaryTableTransposed[, j])
}
```
...
numColumns <- ncol(corTable1)

for (j in seq_len(numColumns)) {
  # Vectorized replacement for dagger
  corTable1[, j] <- gsub("†", "^{†}", corTable1[, j])
  corTable2[, j] <- gsub("†", "^{†}", corTable2[, j])
  
  # Vectorized replacement for three asterisks
  corTable1[, j] <- gsub("\\*\\*\\*", "^{\\***}", corTable1[, j])
  corTable2[, j] <- gsub("\\*\\*\\*", "^{\\***}", corTable2[, j])
  
  # Vectorized replacement for two asterisks
  corTable1[, j] <- gsub("\\*\\*(\\b|$)", "^{\\**}", corTable1[, j], perl = TRUE)
  corTable2[, j] <- gsub("\\*\\*(\\b|$)", "^{\\**}", corTable2[, j], perl = TRUE)
  
  # Vectorized replacement for one asterisk
  corTable1[, j] <- gsub("\\*(\\b|$)", "^{\\*}", corTable1[, j], perl = TRUE)
  corTable2[, j] <- gsub("\\*(\\b|$)", "^{\\*}", corTable2[, j], perl = TRUE)
  
  corTable1[, j] <- paste("$", corTable1[, j], "$", sep = "")
  corTable2[, j] <- paste("$", corTable2[, j], "$", sep = "")
  corTable3[, j] <- paste("$", corTable3[, j], "$", sep = "")
}

corTable1[corTable1 == "$NA$"] <- "NA"
corTable2[corTable2 == "$$"] <- ""
corTable3[corTable3 == "$$"] <- ""
```
...
A pairs panel plot is shown in Figure \@ref(fig:pairsPanel).\index{correlation}

```{r pairsPanel, out.width = "100%", fig.align = "center", fig.cap = "Pairs Panel Plot."}
pairs.panels(HolzingerSwineford1939[,vars])
```

A correlation plot is depicted in Figure \@ref(fig:correlationPlot).\index{correlation}
...
### Exploratory Factor Analysis {#efaExamples}
...
A scree plot from a parallel analysis is depicted in Figure \@ref(fig:screePlotEFAparallel).\index{scree plot}\index{factor analysis!exploratory}
...
##### Very simple structure plot {#vssPlot}
...
VSS plots were generated using the `psych` package [@R-psych].\index{very simple structure plot}
...
A VSS plot is shown in Figure \@ref(fig:vssOrthogonal).\index{very simple structure plot}\index{orthogonal rotation}
...
Fit indices for the orthogonal models are extracted using the syntax below and are shown in Table \@ref(tab:efaOrthogonalFitIndices).\index{factor analysis!exploratory}\index{orthogonal rotation}
...
A pairs panel plot is illustrated in Figure \@ref(fig:pairsPlotOrthogonal).\index{factor analysis!exploratory}\index{orthogonal rotation}\index{correlation}
...
### Confirmatory Factor Analysis {#cfaExamples}
...
A path diagram of the model is shown in Figure \@ref(fig:bifactorModelPathDiagram).\index{factor analysis!bifactor}\index{structural equation modeling!path diagram}
...
#### Multitrait-Multimethod Model {#mtmmCFA}
...
### Exploratory Structural Equation Model {#esemModel}
...
```{r, include = FALSE}
esem_loadingsTable <- esem_loadings

numColumns <- ncol(esem_loadingsTable)

digits <- rep(3, length.out = numColumns)
for (j in seq_len(numColumns)) {
  if (is.numeric(esem_loadingsTable[, j])) esem_loadingsTable[, j] = sprintf(sprintf('$%%.%sf$', digits[j]), esem_loadingsTable[, j])
}
```
...
esem_loadingsTable %>% 
  kable(
    .,
    col.names = c("Item", "Latent", "Loading"),
...
A scree plot based on parallel analysis is shown in Figure \@ref(fig:screePlotPCAParallel).\index{scree plot}\index{parallel analysis}
...
A scree plot is shown in Figure \@ref(fig:screePlotPCA).\index{scree plot}\index{principal component analysis}
...
#### Very Simple Structure Plot {#pcaAnalysisVSSplot}
...
A VSS plot of PCA with orthogonal rotation is shown in Figure \@ref(fig:vssOrthogonalPCA).\index{very simple structure plot}\index{orthogonal rotation}\index{principal component analysis}
...
A biplot of the two-component orthogonal model is shown in Figure \@ref(fig:pcaBiplot).\index{principal component analysis}\index{correlation}\index{orthogonal rotation}
...
A pairs panel plot of the three-component orthogonal model is depicted in Figure \@ref(fig:pairsPlotOrthogonalPCA).\index{principal component analysis}\index{correlation}\index{orthogonal rotation}
