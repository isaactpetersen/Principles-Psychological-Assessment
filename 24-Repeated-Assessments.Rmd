...
## Overview {#overview-repeated-assessments}
...
```{r crossSectional, out.width = "10%", fig.align = "center", fig.cap = "Cross-Sectional Association.", echo = FALSE}
...
```{r laggedAssociation, out.width = "50%", fig.align = "center", fig.cap = "Lagged Association.", echo = FALSE}
...
```{r laggedAssociationControlPriorLevels, out.width = "50%", fig.align = "center", fig.cap = "Lagged Association, Controlling for Prior Levels of the Outcome.", echo = FALSE}
...
```{r crossLaggedPanelModel, out.width = "50%", fig.align = "center", fig.cap = "Lagged Association, Controlling for Prior Levels of the Outcome, Simultaneously Testing Both Directions of Effect.", echo = FALSE}
...
Or, if there are bidirectional, transactional effects, what the magnitude of each direction of effect is.\index{research design!longitudinal}\index{validity!internal}\index{cross-lagged}
...
The detailed instructions help respondents remember more accurately, so the clinician or researcher can examine how people's heavy substance use influences other processes like parenting.\index{repeated measurement}
...
To observe a person's (or group's) growth, it is recommended to use [raw scores](#rawScores) rather than [standardized scores](#standardizedScores) that are age-normed, such as *T* scores, *z* scores, standard scores, and percentile ranks [@Moeller2015].\index{data!raw}\index{data!standardized}\index{norm-referenced!age}\index{data!\textit{z} score}\index{data!\textit{T} score}\index{data!standard score}\index{data!percentile rank}
Age-normed scores prevent observing growth at the person level and at the group level (e.g., changes in means or variances over time).\index{data!raw}\index{data!standardized}\index{norm-referenced!age}
...
Stability of people's level is evaluated with the [coefficient of repeatability](#repeatability).\index{stability!types of}\index{reliability!test–retest!coefficient of repeatability}
...
Rank-order stabiliy is evaluated by examining the association of people's level at T1 with people's level at T2 (e.g., correlation coefficient or regression coefficent).\index{stability!types of}
...
For instance, we examined risk and protective factors that predict the development of externalizing problems [@Petersen2015].\index{growth curve model}
...
Here are types of research designs based on combinations of age, period, and cohort:\index{research design!types}
...
Cross-sectional designs are thus limited in their ability to describe developmental processes including change and stability over time [@Kraemer2000].\index{research design!types}\index{research design!cross-sectional}
...
However, it has been shown, for example, that cross-sectional studies overestimate age-related cognitive declines compared to following the same people over time in a longitudinal study [@Ackerman2013].\index{research design!types}\index{research design!cross-sectional}
...
There are three types of longitudinal sequence designs: [time-sequential](#timeSequential), [cross-sequential](#crossSequential), and [cohort-sequential](#cohortSequential) designs.\index{research design!types}\index{research design!longitudinal sequences}\index{research design!time-sequential}\index{research design!cross-sequential}\index{research design!cohort-sequential}
...
```{r longitudinalSequences, out.width = "60%", fig.align = "center", fig.cap = "Types of Longitudinal Sequences as a Function of Which Two Factors are Specified by the Researcher.", echo = FALSE}
...
```{r timeSequential, out.width = "60%", fig.align = "center", fig.cap = "Time-Sequential Research Design.", echo = FALSE}
...
```{r crossSequential, out.width = "60%", fig.align = "center", fig.cap = "Cross-Sequential Research Design.", echo = FALSE}
...
```{r cohortSequential, out.width = "80%", fig.align = "center", fig.cap = "Cohort-Sequential Research Design.", echo = FALSE}
...
However, age is not the only metric for development; time after an exposure (e.g., puberty onset, school entry) can also be an important time metric.\index{research design!types}\index{research design!longitudinal sequences}
...
The content of a measure includes the facets assessed by a given measure, such as individual behaviors, questionnaire items, or sub-dimensions of a broader construct.\index{heterotypic continuity!identifying}
...
Heterotypic continuity is present to the extent that the content of a construct shows cross-time changes in: people's [rank-order stability](#stability), the content's level on the construct, and how strongly the content reflects the construct.\index{heterotypic continuity!identifying}\index{stability!rank-order}
Cross-time changes in people's [rank-order stability](#stability) indicate changes in the degree of stability of individual differences in the construct across time, and they are examined with correlation of the content across time.\index{heterotypic continuity!identifying}\index{stability!rank-order}
...
How strongly the content reflects the construct, conceptually is the degree of a content's [construct validity](#constructValidity), and it is examined with [item discrimination](#itemDiscrimination) in [IRT](#irt) or with factor loadings in [SEM](#sem).\index{heterotypic continuity!identifying}\index{item response theory!item discrimination}\index{structural equation modeling!factor loading}
...
In adulthood, disobedience could serve prosocial functions, including protesting against societally unjust actions, and thus it shows weaker [construct validity](#constructValidity) for externalizing problems.\index{heterotypic continuity}\index{validity!construct}
...
If a construct changes in manifestation across time, and the measures do not align with the changing manifestation, the scores will not validly assess individuals' growth.\index{heterotypic continuity}
...
And in the case of heterotypic continuity, it assesses content that lacks [construct validity](#constructValidity), i.e., there are item intrusions.\index{repeated measurement}\index{heterotypic continuity}\index{validity!construct}
...
And in the case of heterotypic continuity, it assesses content that lacks [content validity](#contentValidity) because all important facets of the construct are not assessed—i.e., there are item gaps.\index{repeated measurement}\index{validity!content}
...
Age-norming includes [standardized scores](#standardizedScores) (e.g., *T* scores, *z* scores, standard scores) and percentile ranks.\index{norm-referenced!age}\index{data!standardized}
However, as discussed in Section \@ref(testRevision), [standardized scores](#standardizedScores) prevent observing individuals' or groups' absolute change over time.\index{norm-referenced!age}\index{data!standardized}
...
A third approach to ensure statistical equivalence of measures is developmental scaling, which is also called vertical scaling (see Figure \@ref(fig:ageDifferingContent)) and is a form of data harmonization.\index{heterotypic continuity!accounting for}\index{developmental scaling}
...
The common content sets the scale, but all content (including both age-common content and age-differing content) estimate each person's score on that scale.\index{heterotypic continuity!accounting for}\index{developmental scaling}
...
Thurstone scaling aligns the [percentile scores](#percentileRanks) of two measures based on [*z* scores](#zScores) on the common content.\index{heterotypic continuity!accounting for}\index{developmental scaling}
...
Examples of an [IRT](#irt) approach to developmental scaling are provided by @Petersen2018a, @Petersen2021, @PetersenLeBeau2021, and @Petersen2022.\index{heterotypic continuity!accounting for}\index{developmental scaling}\index{item response theory}
...
A number of pieces of evidence can provide support for the theoretical equivalence of measures.\index{heterotypic continuity!accounting for}\index{theory}
...
The content that is assessed at each age should be based on theory.\index{heterotypic continuity!accounting for}\index{theory}
The assessed content should reflect the same construct at relevant ages—i.e., the content should show [construct validity](#constructValidity) at each age it is assessed.\index{heterotypic continuity!accounting for}\index{validity!construct}
...
In addition, the measures should show [convergent](#convergentValidity) and [discriminant](#discriminantValidity) validity.\index{heterotypic continuity!accounting for}\index{validity!convergent}\index{validity!discriminant}
...
However, we would argue that a single (non-[adaptive](#adaptiveTesting)) measure would not be able to [validly](#validity) assess the same construct across development if the construct shows heterotypic continuity.\index{lifespan assessment}\index{heterotypic continuity}\index{adaptive testing}
...
Although there can be benefits to examining development in a piecewise way, relying on a fixed set of content restricts our ability to see growth over a longer time and our ability to understand key developmental transitions.
...
Use [developmental scaling](#developmentalScaling) approaches to link different measures onto the same scale.\index{heterotypic continuity}\index{developmental scaling}
...
Use [developmental scaling](#developmentalScaling) to link different measures onto the same scale.\index{heterotypic continuity!accounting for}\index{developmental scaling}
