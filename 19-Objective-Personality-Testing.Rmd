...
## Overview {#overview-objective-personality}

There are two main types of personality tests: objective personality tests and [projective personality tests](#projective).\index{personality assessment!objective}\index{personality assessment!projective}
...
In a [*projective personality test*](#projective), an ambiguous stimulus is presented to a respondent, who is asked to make an open-ended response.\index{personality assessment!projective}
Examples of [projective personality tests](#projective) include the [Rorschach Inkblot Test](#rorschach) and the [Thematic Apperception Test](#tat) (TAT).\index{personality assessment!projective}\index{Rorschach Inkblot Test}\index{Thematic Apperception Test}
[Projective tests](#projective) have largely fallen by the wayside now, but it is still helpful to think about their potential advantages.\index{personality assessment!projective}
[Projective tests](#projective) are described in further detail in Chapter \@ref(projective).\index{personality assessment!projective}

### Projective Personality Tests {#projectivePersonalityTests}

In a [projective personality test](#projective), the client's response is not limited.\index{personality assessment!projective}
[Projective personality tests](#projective) were designed from a psychodynamic perspective, and they are supposed to have limitless variability and therefore a freer access to the client's internal world.\index{personality assessment!projective}\index{psychoanalysis}
However, Card V of the [Rorschach Inkblot Test](#rorschach) looks like a moth or a bat, and around 90% of respondents likely give that response [@Wiggins1973], so [projective tests](#projective) are not completely limitless.\index{personality assessment!projective}\index{Rorschach Inkblot Test}
[Projective personality tests](#projective) are designed to have ambiguous content.\index{personality assessment!projective}
...
However, even so-called objective personality tests can have items that are ambiguous and that function similarly to a [projective test](#projective).\index{personality assessment!objective}\index{personality assessment!projective}
...
Scoring of [projective tests](#projective) does not rely on the client's insight, so [projective tests](#projective) might get past [social desirability](#methodBias) of clients' responses and potential defensiveness.\index{personality assessment!projective}\index{response style!social desirability}
Therefore, [projective tests](#projective) are potentially difficult to fake.\index{personality assessment!projective}
...
However, they and other [projective techniques](#projective) have lost considerable ground since then.\index{personality assessment!projective}

### Objective Personality Tests {#objectivePersonalityTests}

In contrast to [projective personality tests](#projective), objective personality tests, the client's responses are substantially constrained to the possible answers.\index{personality assessment!projective}\index{personality assessment!objective}
...
In addition, objective tests have more [reliable](#reliability) scoring than [projective tests](#projective).\index{personality assessment!objective}\index{personality assessment!projective}\index{personality assessment!objective!reliability of}
It takes a very long time to score the [Rorschach Inkblot Test](#rorschach), and it still has very low [reliability](#reliability).\index{personality assessment!projective}\index{Rorschach Inkblot Test}\index{personality assessment!projective!reliability of}
...
(ref:somaticSymptomCaption) Various Factors That Could Influence a Respondent's Answer to the True/False Question: "I hardly ever notice my heart pounding, and I am seldom short of breath".

```{r somaticSymptom, out.width = "100%", fig.align = "center", fig.cap = "(ref:somaticSymptomCaption)", echo = FALSE}
...
In conclusion, every objective test is partly [projective](#projective)—that is, the stimuli are interpreted in different ways by different people.\index{personality assessment!objective}\index{personality assessment!projective}
...
For example, consider the Rosenberg Self-Esteem Scale and the Spielberger State–Trait Anxiety Inventory below.\index{assessment development!deductive approach}
...
[Factor analysis](#factorAnalysisOverview) is used for the inductive approach to developing measures.\index{assessment development!inductive approach}\index{factor analysis}
...
In SPSS, you can use [principal component analysis (PCA)](#pca) for item extraction.\index{assessment development!inductive approach}\index{factor analysis!decisions}\index{principal component analysis}
Investigators often determine how many components/factors to keep based on the [criterion](#criterionValidity) of keeping components with eigenvalues greater than 1, often use orthogonal rotation of data, etc.\index{assessment development!inductive approach}\index{factor analysis!decisions}\index{eigenvalue}
[Factor analysis](#factorAnalysisOverview) and [PCA](#pca) are described in Chapter \@ref(factor-analysis-PCA).\index{factor analysis}\index{principal component analysis}
...
Ideally, you would test the items in large and heterogeneous samples that are representative of the population.\index{assessment development!best practices}\index{validity!external}
    - Examine the items in relation to external [criteria](#criterionValidity), using an [empirical approach](#externalApproach).\index{assessment development!best practices}\index{assessment development!external approach}\index{criterion}
    - Examine items in relation to each other, using an [inductive approach](#inductiveApproach).\index{assessment development!best practices}\index{assessment development!inductive approach}
...
### Response Scale {#responseScale}

Evidence suggests that there may not be benefits of having more than six response options for Likert-scale items that assess personality [@Simms2019].\index{personality assessment}\index{response scale!precision of}\index{Likert scale}
...
## Flawed Nature of Self-Assessments {#flawedSelfAssessment}

It is a common finding that people tend to overestimate their skill and abilities—most people tend to describe themselves as "above average", which is statistically impossible.\index{self-report}
People are over-confident, and they overestimate the likelihood of achieving desirable outcomes and underestimate how long it will take to complete future projects.\index{self-report}\index{over-confidence}
...
This phenomenon is called social desirability bias, which is a form of [method bias](#methodBias-types) in which people systematically adjust their responses to reflect more socially desirable attributes.\index{self-report}\index{faking!good}\index{bias!social desirability}\index{method bias}
