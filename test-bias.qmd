...
Estimates of [DIF](#dif) after resolving item 5 are in Table \@ref(tab:itemLevelDIFAfterResolving).\index{item response theory!differential item functioning}
...
The two broad categories of test bias include [predictive bias](#predictiveBias) and [test structure bias](#testStructureBias).\index{bias!predictive}\index{bias!test structure}
[Predictive bias](#predictiveBias) refers to differences between groups in the relation between the test and criterion—i.e., different intercepts and/or slopes of the regression line between the test and the criterion.\index{bias!predictive}
...
However, even if a test does not show bias, it does not mean that the test is [fair](#fairness).\index{bias}\index{fairness}
...
[Fairness](#fairness) is a complex question, so do the best you can and try to minimize any negative impact of the assessment procedures.\index{fairness}
