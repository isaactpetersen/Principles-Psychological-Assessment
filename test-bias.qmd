...
![](images/adjustment_bandingBonusPoints.pn){width=100% fig-alt="Using Banding With Bonus Points as a Scoring Adjustment."}
...
$w$ is computed as in Equation \@ref(eq:measurementNoninvarianceEffectSizeW) [@Newsom2015]:\index{structural equation modeling!measurement invariance}\index{effect size}

$$
w = \sqrt{\frac{\Delta \chi^2}{N(\Delta df)}}
$$ {#eq-measurementNoninvarianceEffectSizeW}

McDonald's centrality index ($\text{M}_\text{C}$) is computed as in Equation \@ref(eq:measurementNoninvarianceEffectSizeMc) [@Newsom2015]:\index{structural equation modeling!measurement invariance}\index{effect size}

$$
\text{M}_\text{C} = \exp\!\left[-\tfrac{1}{2}\,\left(\frac{\chi^{2} - \text{df}}{N - 1}\right)\right]
$$ {#eq-measurementNoninvarianceEffectSizeMc}

Then, $\Delta \text{M}_\text{C}$ is computed as in Equation \@ref(eq:measurementNoninvarianceEffectSizeDeltaMc) [@Newsom2015]:\index{structural equation modeling!measurement invariance}\index{effect size}
