...
## Overview of Bias {#sec-overview-bias}

There are multiple definitions of the term "bias" depending on the context.
In general, bias is a [systematic error](#sec-systematicError) [@Reynolds2012].\index{bias}\index{bias!types of}
[Mean error](#sec-meanError) is an example of [systematic error](#sec-systematicError), and is sometimes called bias.\index{bias}\index{measurement error!systematic error}\index{bias!types of}
...
[Method biases](#sec-methodBias) are a form of [systematic error](#sec-systematicError) that involve the influence of measurement on a person's score that is not due to the person's level on the construct.\index{method bias}\index{bias}\index{measurement error!systematic error}\index{bias!types of}
[Method biases](#sec-methodBias) include response biases or response styles, including acquiescence and social desirability bias.\index{method bias}\index{bias}\index{measurement error!systematic error}\index{response style}\index{bias!social desirability}\index{bias!types of}
...
Sometimes bias is used to refer in particular to [systematic error](#sec-systematicError) (in measurement, prediction, etc.) as a function of group membership, where test bias refers to the same score having different meaning for different groups.\index{bias}\index{bias!types of}
...
For example, a test is biased if there is differential [validity](#sec-validity) of test scores for groups (e.g., age, education, culture, race, sex).\index{culture}\index{bias}\index{validity}\index{bias!types of}
Test bias would exist, for instance, if a test is a less [valid](#sec-validity) predictor for racial minorities or linguistic minorities.\index{bias}\index{validity}\index{bias!types of}
...
There are some known instances of test bias, as described in @sec-biasExamples.\index{bias}
...
Furthermore, just because a test does not show bias does not mean that there are not race-, social class-, and gender-related [biases in clinical judgment](#sec-biasClinicalJudgment) during the assessment process.\index{clinical judgment!bias}\index{bias!clinical judgment}
...
1. [predictive bias](#sec-predictiveBias)\index{bias}\index{bias!types of}\index{bias!predictive}
1. [test structure bias](#sec-testStructureBias)\index{bias}\index{bias!types of}\index{bias!test structure}

[Predictive bias](#sec-predictiveBias) refers to differences between groups in the relation between the test and criterion.\index{bias}\index{bias!types of}\index{bias!predictive}\index{validity!criterion}
As with all [criterion-related validity](#sec-criterionValidity) tests, the findings depend on the strength and quality of the criterion.\index{validity!criterion}
[Test structure bias](#sec-testStructureBias) refers to differences in the internal test characteristics across groups.\index{bias}\index{bias!types of}\index{bias!test structure}

## Ways to Investigate/Detect Test Bias {#sec-detectBias}

### Predictive Bias {#sec-predictiveBias}

Predictive bias exists when differences emerge between groups in terms of [predictive validity](#sec-predictiveValidity) to a criterion.\index{bias!predictive}
...
For instance, consider a [2x2 confusion matrix](#sec-confusionMatrix) used for the standard prediction problem.\index{confusion matrix}
A [confusion matrix](#sec-confusionMatrix) for whom to select for a job is depicted in @fig-jobSelection.\index{confusion matrix}

::: {#fig-jobSelection}
![](images/JobSelection.png){width=100% fig-alt="2x2 Confusion Matrix for Job Selection. TP = true positive; TN = true negative; FP = false positive; FN = false negative."}

2x2 Confusion Matrix for Job Selection. TP = true positive; TN = true negative; FP = false positive; FN = false negative.
:::

We can also visualize the [confusion matrix](#sec-confusionMatrix) in terms of a scatterplot of the test scores (i.e., predicted job performance) and the "truth" scores (i.e., actual job performance), as depicted in @fig-jobSelection2.\index{confusion matrix}\index{bias!predictive}
...
```{r}
#| label: fig-jobSelection2
#| fig-cap: "2x2 Confusion Matrix for Job Selection in the Form of a Graph With Predicted Performance on the x-Axis and Actual Job Performance on the y-Axis. TP = true positive; TN = true negative; FP = false positive; FN = false negative."
#| fig-alt: "2x2 Confusion Matrix for Job Selection in the Form of a Graph With Predicted Performance on the x-Axis and Actual Job Performance on the y-Axis. TP = true positive; TN = true negative; FP = false positive; FN = false negative."
#| fig-scap: "2x2 Confusion Matrix for Job Selection in the Form of a Graph."
#| fig-height: 8
#| code-fold: true
...
The data points in the top right quadrant are [true positives](#sec-truePositive): people who the test predicted would do a good job and who did a good job.\index{bias!predictive}\index{true positive}
The data points in the bottom left quadrant are [true negatives](#sec-trueNegative): people who the test predicted would do a poor job and who would have done a poor job.\index{bias!predictive}\index{true negative}
The data points in the bottom right quadrant are [false positives](#sec-falsePositive): people who the test predicted would do a good job and who did a poor job.\index{bias!predictive}\index{false positive}
The data points in the top left quadrant are [false negatives](#sec-falseNegative): people who the test predicted would do a poor job and who would have done a good job.\index{bias!predictive}\index{false negative}
...
@fig-jobSelection3 depicts a strong predictor.\index{bias!predictive}
The best-fit regression line has a steep slope where there are lots of data points that are [true positives](#sec-truePositive) and [true negatives](#sec-trueNegative), with relatively few [false positives](#sec-falsePositive) and [false negatives](#sec-falseNegative).\index{bias!predictive}\index{true positive}\index{true negative}\index{false positive}\index{false negative}

```{r}
#| label: fig-jobSelection3
#| fig-cap: "Example of a Strong Predictor. TP = true positive; TN = true negative; FP = false positive; FN = false negative."
#| fig-alt: "Example of a Strong Predictor. TP = true positive; TN = true negative; FP = false positive; FN = false negative."
#| fig-scap: "Example of a Strong Predictor."
#| fig-height: 8
#| code-fold: true
...
@fig-jobSelection4 depicts a poor predictor.\index{bias!predictive}
The best-fit regression line has a shallow slope where there are just as many data points that are in the false cells ([false positives](#sec-falsePositive) and [false negatives](#sec-falseNegative)) as there are in the true cells ([true positives](#sec-truePositive) and [true negatives](#sec-trueNegative)).\index{bias!predictive}\index{true positive}\index{true negative}\index{false positive}\index{false negative}
In general, the steeper the slope, the better the predictor.\index{bias!predictive}

```{r}
#| label: fig-jobSelection4
#| fig-cap: "Example of a Poor Predictor. TP = true positive; TN = true negative; FP = false positive; FN = false negative."
#| fig-alt: "Example of a Poor Predictor. TP = true positive; TN = true negative; FP = false positive; FN = false negative."
#| fig-scap: "Example of a Poor Predictor."
#| fig-height: 8
#| code-fold: true
...
#### Types of Predictive Bias {#sec-typesPredictiveBias}

There are three types of predictive bias:\index{bias!predictive}

1. [Different slopes](#sec-differentSlopes)\index{bias!predictive!different slopes}
1. [Different intercepts](#sec-differentIntercepts)\index{bias!predictive!different intercepts}
1. [Different intercepts and slopes](#sec-differentInterceptsAndSlopes)\index{bias!predictive!different intercepts and slopes}
...
##### Different Slopes {#sec-differentSlopes}
...
Differing slopes indicate differential [predictive validity](#sec-predictiveValidity), in which the test is a more effective predictor of performance in one group over the other.\index{bias!predictive!different slopes}\index{validity!predictive}
Different slopes predictive bias is depicted in @fig-testBias1.\index{bias!predictive!different slopes}
...
::: {#fig-testBias1}
![](images/testBias-01.png){width=100% fig-alt="Test Bias: Different Slopes. TP = true positive; TN = true negative; FP = false positive; FN = false negative."}
...
If the predictor is a strong predictor in both groups but shows slight differences in the slope, [within-group norming](#sec-withinGroupNorming) could be used.\index{bias!predictive!different slopes}

##### Different Intercepts {#sec-differentIntercepts}
...
Different intercepts predictive bias is depicted in @fig-testBias2.\index{bias!predictive!different intercepts}

::: {#fig-testBias2}
![](images/testBias-02.png){width=100% fig-alt="Test Bias: Different Intercepts. TP = true positive; TN = true negative; FP = false positive; FN = false negative."}
...
@fig-testBias2 indicates that the measure systematically under-estimates the job performance of the minority group.\index{bias!predictive!different intercepts}
...
The over-prediction of lower-scoring groups is likely mostly an artifact of [measurement error](#sec-measurementError) [@Gottfredson1994].\index{bias!predictive!different intercepts}
The over-estimation of African Americans' and Hispanics' school and job performance may be due to [measurement error](#sec-measurementError) in the tests.\index{bias!predictive!different intercepts}
...
Black individuals tend to be far less disadvantaged on the noncognitive determinants of job performance than on the cognitive ones.\index{bias!predictive!different intercepts}
...
To correct for a bias in intercepts, we could add [bonus points](#sec-bonusPoints) to the scores for the minority group to correct for the amount of the [systematic error](#sec-systematicError), and to result in the same regression line.\index{bias!predictive!different intercepts}\index{measurement error!systematic error}
But if the minority group is over-predicted (as has often been the case where intercept differences have been observed), we would not want to use [score adjustment](#sec-scoreAdjustment) to lower the minority group's scores.\index{bias!predictive!different intercepts}

##### Different Intercepts and Slopes {#sec-differentInterceptsAndSlopes}
...
In cases of different intercepts and slopes, there is both differential [validity](#sec-validity) (because the regression lines have different slopes), as well as varying under- and over-estimation of groups' performance at particular scores.\index{bias!predictive!different intercepts and slopes}\index{validity}
Different intercepts and slopes predictive bias is depicted in @fig-testBias3.\index{bias!predictive!different intercepts and slopes}

::: {#fig-testBias3}
![](images/testBias-03.png){width=100% fig-alt="Test Bias: Different Intercepts and Slopes. TP = true positive; TN = true negative; FP = false positive; FN = false negative."}
...
### Test Structure Bias {#sec-testStructureBias}
...
#### Empirical Approaches to Identification {#sec-testStructureBiasEmpirical}

In general, it is important to ensure that scores are on the same number line—and are thus able to be meaningfully compared—across groups.\index{bias!test structure}
To empirically ensure that scores are on the same number line across groups, we must ensure two things:\index{bias!test structure}

- the scores have the same *scale* across groups—scale refers to how much the score changes per unit on the construct (number line)—that is, if scores have the same scale across groups, movement along the number line is equivalent.\index{bias!test structure}
- the scores have the same *location* across groups—location refers to the origin or zero point on the number line—that is, if scores have the same location across groups, the number lines have the same origin.\index{bias!test structure}

Two measures with different scales are depicted in @fig-rulerDifferentScale.

::: {#fig-rulerDifferentScale}
![](images/rulerDifferentScale.png){width=100% fig-alt="Two Measures (i.e., Rulers) With Different Scales."}

Two Measures (i.e., Rulers) With Different Scales.
:::

Two measures with different locations are depicted in @fig-rulerDifferentLocation.

::: {#fig-rulerDifferentLocation}
![](images/rulerDifferentLocation.png){width=100% fig-alt="Two Measures (i.e., Rulers) With Different Locations."}

Two Measures (i.e., Rulers) With Different Locations.
:::

The concepts of location and scale for various model types are provided in @tbl-testBiasLocationScale.\index{bias!test structure}\index{item response theory!differential item functioning}\index{item response theory!item discrimination}\index{item response theory!item difficulty}\index{structural equation modeling!measurement invariance}\index{factor analysis!confirmatory}\index{structural equation modeling!factor loading}\index{structural equation modeling!intercept}

| **Model**                    | **Location**        | **Scale**      |
|------------------------------|---------------------|----------------|
| Regression                   | intercept           | slope          |
| Confirmatory Factor Analysis | intercept/threshold | factor loading |
| Item Response Theory         | difficulty/severity | discrimination |

: Concepts of Location and Scale in Various Model Types. {#tbl-testBiasLocationScale}

##### Item $\times$ Group tests (ANOVA) {#sec-itemXgroupTests-bias}
...
##### Item Response Theory {#sec-irt-bias}

Using [item response theory](#sec-irt), we can examine [differential item functioning](#sec-dif) (DIF).\index{bias!test structure}\index{item response theory}\index{item response theory!differential item functioning}
Evidence of [DIF](#sec-dif), indicates that there are differences between group in terms of [discrimination](#sec-itemDiscrimination) and/or [difficulty/severity](#sec-itemDifficulty) of items.\index{bias!test structure}\index{item response theory!differential item functioning}\index{item response theory!item discrimination}\index{item response theory!item difficulty}
Differences between groups in terms of the [item characteristic curve](#sec-icc) (which combines the item's [discrimination](#sec-itemDiscrimination) and [severity](#sec-itemDifficulty)) would be evidence against [construct validity](#sec-constructValidity) invariance between the groups and would provide evidence of bias.\index{bias!test structure}\index{item response theory!differential item functioning}\index{item response theory!item discrimination}\index{item response theory!item difficulty}\index{item response theory!item characteristic curve}\index{validity!construct}
[DIF](#sec-dif) examines stretching and compression of different groups.\index{bias!test structure}\index{item response theory!differential item functioning}
...
The item would be expected to show a weaker [discrimination](#sec-itemDiscrimination) and higher [severity](#sec-itemDifficulty) in adults compared to children.\index{bias!test structure}\index{item response theory!differential item functioning}\index{item response theory!item discrimination}\index{item response theory!item difficulty}
[DIF](#sec-dif) is discussed in @sec-dif.\index{bias!test structure}\index{item response theory!differential item functioning}

##### Confirmatory Factor Analysis {#sec-cfa-bias}

[Confirmatory factor analysis](#sec-cfa-sem) allows tests of [measurement invariance](#sec-measurementInvariance) (also called factorial invariance).\index{bias!test structure}\index{structural equation modeling!measurement invariance}\index{factor analysis!confirmatory}
[Measurement invariance](#sec-measurementInvariance) examines whether the factor structure of the underlying latent variables in the test is consistent across groups.\index{bias!test structure}\index{structural equation modeling!measurement invariance}\index{latent variable}
...
[Measurement invariance](#sec-measurementInvariance) is discussed in @sec-measurementInvariance.\index{bias!test structure}\index{structural equation modeling!measurement invariance}
...
A different factor structure across groups is depicted in @fig-testBias4.\index{bias!test structure}\index{structural equation modeling!measurement invariance}

::: {#fig-testBias4}
![](images/testBias-04.png){width=100% fig-alt="Different Factor Structure Across Groups."}
...
- Metric ("weak factorial") invariance: items have the same factor loadings ([discrimination](#sec-itemDiscrimination)) in each group.\index{bias!test structure}\index{structural equation modeling!measurement invariance}\index{structural equation modeling!factor loading}\index{item response theory!item discrimination}
- Scalar ("strong factorial") invariance: items have the same intercepts ([difficulty/severity](#sec-itemDifficulty)) in each group.\index{bias!test structure}\index{structural equation modeling!measurement invariance}\index{structural equation modeling!intercept}\index{item response theory!item difficulty}
...
##### Structural Equation Modeling {#sec-sem-bias}

[Structural equation modeling](#sec-sem) is a [confirmatory factor analysis](#sec-cfa) (CFA) model that incorporates prediction.\index{factor analysis!confirmatory}\index{structural equation modeling}
...
##### Signal Detection Theory {#sec-sdt-bias}

[Signal detection theory](#sec-sdt) is a dynamic measure of bias.\index{bias}\index{signal detection theory}
It allows examining the overall bias in selection systems, including both accuracy and errors at various cutoffs ([sensitivity](#sec-sensitivity), [specificity](#sec-specificity), [positive predictive value](#sec-ppv), and [negative predictive value](#sec-npv)), as well as accuracy across all possible cutoffs (the [area under the receiver operating characteristic curve](#sec-auc)).\index{bias}\index{signal detection theory}\index{sensitivity}\index{specificity}\index{positive predictive value}\index{negative predictive value}\index{receiver operating characteristic curve!area under the curve}
While there may be similar [predictive validity](#sec-predictiveValidity) between groups, the type of errors we are making across groups might differ.\index{bias!predictive}\index{validity!predictive}
It is important to decide which types of error to emphasize depending on the fairness goals and examining [sensitivity](#sec-sensitivity)/[specificity](#sec-specificity) to adjust cutoffs.\index{bias!predictive}\index{signal detection theory}\index{sensitivity}\index{specificity}

##### Empirical Evidence of Test Structure Bias {#sec-evidenceTestStructureBias}

It is not uncommon to find items that show differences across groups in [severity](#sec-itemDifficulty) (intercepts) and/or [discrimination](#sec-itemDiscrimination) (factor loadings).\index{bias!test structure}\index{structural equation modeling!measurement invariance}\index{structural equation modeling!intercept}\index{structural equation modeling!factor loading}\index{item response theory!differential item functioning}\index{item response theory!item discrimination}\index{item response theory!item difficulty}
...
#### Theoretical/Judgmental Approaches to Identification {#sec-testStructureBiasTheoretical}

##### Facial Validity Bias {#sec-facialValidityBias}

[Facial validity](#sec-faceValidity) bias considers the extent to which an average person thinks that an item is biased—i.e., the item has differing [validity](#sec-validity) between minority and majority groups.\index{bias!test structure}\index{validity!face}
...
##### Content Validity Bias {#sec-contentValidityBias}

[Content validity](#sec-contentValidity) bias is determined by judgments of construct experts who look for items that do not do an adequate job assessing the construct between groups.\index{bias!test structure}\index{validity!content}
A construct may include some content facets in one group, but may include different content facets in another group, as depicted in @fig-testBias5.\index{bias!test structure}\index{validity!content}

::: {#fig-testBias5}
![](images/testBias-05.png){width=100% fig-alt="Different Content Facets in a Given Construct for Two Groups."}
...
If an item is linguistically complicated, grammatically complex or convoluted, or a double negative, it may be less [valid](#sec-validity) or predictive for rural populations and those with less education.\index{bias!test structure}\index{validity!content}\index{validity}

Also, stereotype threat may contribute to [content validity](#sec-contentValidity) bias.\index{bias!test structure}\index{validity!content}\index{stereotype threat}
...
Another example of [content validity](#sec-contentValidity) bias is when the same measure is used to assess a construct across ages even though the construct shows heterotypic continuity.\index{bias!test structure}\index{validity!content}\index{heterotypic continuity}
...
[Content validity](#sec-contentValidity) and [facial validity](#sec-faceValidity) bias judgments are often related, but not always.\index{bias!test structure}\index{validity!content}\index{validity!face}\index{heterotypic continuity}

## Examples of Bias {#sec-biasExamples}

As described in the overview in @sec-overview-bias, there is not much empirical evidence of test bias [@Brown1999; @Hall1999; @Jensen1980; @Kuncel2010a; @Reynolds2012; @Reynolds2021; @Sackett1994; @Sackett2008].\index{bias!examples}
...
For instance, test bias can occur if a subgroup is less familiar with the language, the stimulus material, or the response procedures, or if they have different [response styles](#sec-methodBias-types).\index{bias!examples}\index{response style}
In addition to test bias, there are known patterns of [bias in clinical judgment](#sec-biasClinicalJudgment), as described in @sec-biasClinicalJudgment.

## Test Fairness {#sec-fairness}
...
Consider an example of an unbiased test, as depicted in @fig-testBias6, adapted from @Gottfredson1994.\index{fairness}
...
In sum, there is no [predictive validity](#sec-predictiveValidity) bias between the two groups.\index{fairness}\index{bias!predictive}
...
Although the test is unbiased, there are differences in the *quality* of prediction: there are more [false negatives](#sec-falseNegative) in the Black group compared to the White group.\index{fairness}\index{false negative}
...
There are two kinds of errors: [false negatives](#sec-falseNegative) and [false positives](#sec-falsePositive).\index{fairness}\index{false negative}\index{false positive}
Each error type has very different implications.\index{fairness}\index{false negative}\index{false positive}
[False negatives](#sec-falseNegative) would be when the test predicts that an applicant would perform poorly and we do not give them the job even though they would have performed well.\index{fairness}\index{false negative}
[False negatives](#sec-falseNegative) have a negative effect on the applicant.\index{fairness}\index{false negative}
And, in this example, there are more [false negatives](#sec-falseNegative) in the Black group.\index{fairness}\index{false negative}
By contrast, [false positives](#sec-falsePositive) would be when we predict that an applicant would do well, and we give them the job but they perform poorly.\index{fairness}\index{false positive}
[False positives](#sec-falsePositive) are a benefit to the applicant but have a negative effect on the employer.\index{fairness}\index{false positive}
In this example, there are more [false positives](#sec-falsePositive) in the White group, which is an undeserved benefit based on the [selection ratio](#sec-selectionRatio); therefore, the White group benefits.\index{fairness}\index{false positive}\index{selection ratio}
...
### Adverse Impact {#sec-adverseImpact}

Adverse impact is defined as rejecting members of one group at a higher rate than another group.\index{adverse impact}
Adverse impact is different from test [validity](#sec-validity).\index{adverse impact}\index{validity}
According to federal guidelines, adverse impact is present if the [selection rate](#sec-selectionRatio) of one group is less than four-fifths (80%) the [selection rate](#sec-selectionRatio) of the group with the highest [selection rate](#sec-selectionRatio).\index{adverse impact}\index{selection ratio}

There is much more evidence of adverse impact than test bias.\index{adverse impact}\index{bias}
Indeed, disparate impact of tests on personnel selection across groups is the norm rather than the exception, even when using [valid](#sec-validity) tests that are unbiased, which in part reflect group-related differences in job-related skills [@Gottfredson1994].\index{adverse impact}\index{validity}
...
- personality tests, which produce higher estimates of dominance among men than women; it is unclear whether this has [predictive bias](#sec-predictiveBias).\index{adverse impact}\index{bias!predictive}

### Bias Versus Fairness {#sec-biasVsFairness}

Whether a measure is accurate or shows test bias is a scientific question.
By contrast, whether a test is fair and thus should be used for a given purpose is not just a scientific question; it is also an [ethical](#sec-ethics) question.\index{bias}\index{fairness}\index{ethics}
It involves the consideration of the potential consequences of testing in terms of social values and [consequential validity](#sec-consequentialValidity).\index{fairness}

### Operationalizing Fairness {#sec-operationalizingFairness}
...
1. Equal outcomes: the [selection rate](#sec-selectionRatio) is the same across groups.\index{fairness!operationalizing}\index{selection ratio}
1. Equal opportunity: the [sensitivity](#sec-sensitivity) ([true positive rate](#sec-sensitivity); 1 $-$ [false negative rate](#sec-falseNegativeRate)) is the same across groups.\index{fairness!operationalizing}\index{sensitivity}\index{false negative!rate}
1. Equal odds: the [sensitivity](#sec-sensitivity) is the same across groups and the [specificity](#sec-specificity) ([true negative rate](#sec-specificity); 1 $-$ [false positive rate](#sec-falsePositiveRate)) is the same across groups.\index{fairness!operationalizing}\index{sensitivity}\index{specificity}\index{false positive!rate}
...
The job selection procedure shows *equal opportunity* if, among those who show strong job performance, the proportion of classification errors ([false negatives](#sec-falseNegative)) is the same for men and women.\index{fairness!operationalizing}
[Receiver operating characteristic (ROC) curves](#sec-roc) are depicted for two groups in @fig-testBias-ROC.\index{fairness!operationalizing}\index{receiver operating characteristic curve}
A cutoff that represents equal opportunity is depicted with a horizontal line (i.e., the same [sensitivity](#sec-sensitivity)) in @fig-testBias-ROC.
The job selection procedure shows *equal odds* if (a), among those who show strong job performance, the proportion of classification errors ([false negatives](#sec-falseNegative)) is the same for men and women, and (b), among those who show poor job performance, the proportion of classification errors ([false positives](#sec-falsePositive)) is the same for men and women.\index{fairness!operationalizing}\index{sensitivity}\index{false negative!rate}\index{false positive!rate}
A cutoff that represents equal odds is depicted where the [ROC curve](#sec-roc) for Group A intersects with the [ROC curve](#sec-roc) from Group B in @fig-testBias-ROC.\index{fairness!operationalizing}
...
::: {#fig-testBias-ROC}
![](images/testBias-ROC.png){width=100% fig-alt="Receiver Operating Characteristic (ROC) Curves for Two Groups. (Figure reprinted from @Fletcher2021, Figure 2, p. 3. Fletcher, R. R., Nakeshimana, A., & Olubeko, O. (2021). Addressing fairness, bias, and appropriate use of artificial intelligence and machine learning in global health. *Frontiers in Artificial Intelligence*, *3*(116). <https://doi.org/10.3389/frai.2020.561802>)"}

Receiver Operating Characteristic (ROC) Curves for Two Groups. (Figure reprinted from @Fletcher2021, Figure 2, p. 3. Fletcher, R. R., Nakeshimana, A., & Olubeko, O. (2021). Addressing fairness, bias, and appropriate use of artificial intelligence and machine learning in global health. *Frontiers in Artificial Intelligence*, *3*(116). <https://doi.org/10.3389/frai.2020.561802>)
:::

It is not possible to meet all three types of fairness simultaneously (i.e., equal [selection rates](#sec-selectionRatio), [sensitivity](#sec-sensitivity), and [specificity](#sec-specificity) across groups) unless the [base rates](#sec-baseRate) are the same across groups or the selection is perfectly accurate [@Fletcher2021].\index{fairness!operationalizing}\index{selection ratio}\index{sensitivity}\index{specificity}\index{base rate}
...
In the 1991 Civil Rights Act, [score adjustments](#sec-scoreAdjustment) based on race, gender, and ethnicity (e.g., within-race [norming](#sec-norm) or race-conscious score adjustments) were made illegal in personnel selection [@Gottfredson1994].\index{fairness!operationalizing}
...
However, you likely will not ever really know the true [base rate](#sec-baseRate) in these situations.\index{fairness!operationalizing}\index{base rate}
...
## Correcting For Bias {#sec-correctForBias}

### What to Do When Detecting Bias {#sec-whenDetectingBias}

When examining item bias (using [differential item functioning](#sec-dif)/DIF or [measurement non-invariance](#sec-measurementInvariance)) with many items (or measures) across many groups, there can be many tests, which will make it likely that [DIF](#sec-dif)/[non-invariance](#sec-measurementInvariance) will be detected, especially with a large sample.\index{item response theory!differential item functioning}\index{structural equation modeling!measurement invariance}
Some detected [DIF](#sec-dif) may be artificial or trivial, but other [DIF](#sec-dif) may be real and important to address.\index{item response theory!differential item functioning}\index{structural equation modeling!measurement invariance}
It is important to consider how you will proceed when detecting [DIF](#sec-dif)/[non-invariance](#sec-measurementInvariance).\index{item response theory!differential item functioning}\index{structural equation modeling!measurement invariance}
Considerations of effect size and theory can be important for evaluating the [DIF](#sec-dif)/[non-invariance](#sec-measurementInvariance) and whether it is negligible or important to address.\index{item response theory!differential item functioning}\index{structural equation modeling!measurement invariance}
...
Does the bias present [adverse impact](#sec-adverseImpact) for a minority group?\index{bias!correcting for}\index{adverse impact}
...
If the effects are small, if the bias does not present [adverse impact](#sec-adverseImpact) for a minority group, and if there is no compelling theoretical reason for the bias, the bias might not be sufficient to scrap the instrument for the population.\index{bias!correcting for}
...
There are a number of [score adjustment](#sec-scoreAdjustment) and [non-score adjustment](#sec-otherBiasCorrections) approaches to correct for bias, as described in Sections [-@sec-scoreAdjustment] and [-@sec-otherBiasCorrections].\index{bias!correcting for}
If the bias occurs at the item level (e.g., [test structure bias](#sec-testStructureBias)), it is generally recommended to [remove](#sec-removeBiasedItems) or [resolve](#sec-resolveBiasedItems) items that show non-negligible bias.\index{bias!correcting for}\index{bias!test structure}
...
If the bias occurs at the test score level (e.g., [predictive bias](#sec-predictiveBias)), [score adjustments](#sec-scoreAdjustment) may be considered.\index{bias!correcting for}\index{bias!test structure}\index{bias!correcting for!score adjustment}

There are situations measurement invariance across time may not be expected (and may thus be problematic to require).\index{bias!correcting for}
For instance, if there is a treatment that occurs between two measurement occasions, the factor structure might change due to the treatment rather than due to the mere passage of time.\index{bias!correcting for}
Thus, it would be helpful to know whether the instrument shows measurement invariance across a similar timeframe when no treatment is occurring.\index{bias!correcting for}
Another situation where measurement invariance across time may not be expected is when the construct shows heterotypic continuity [@Petersen2020].\index{bias!correcting for}\index{heterotypic continuity}
If the construct changes in its behavioral manifestation with development, then measurement invariance may not be expected across wide spans of time.\index{bias!correcting for}\index{heterotypic continuity}
...
### Score Adjustment to Correct for Bias {#sec-scoreAdjustment}
...
#### Why Adjust Scores? {#sec-whyAdjustScores}
...
A third potential reason that we may want to apply score adjustment is to improve the [fairness](#sec-fairness) of a test.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{fairness}

#### Types of Score Adjustment {#sec-typesOfScoreAdjustment}
...
- [Bonus points](#sec-bonusPoints)\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!bonus points}
- [Within-group norming](#sec-withinGroupNorming)\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}
- [Separate cutoffs](#sec-separateCutoffs)\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!separate cutoffs}
- [Top-down selection from different lists](#sec-topDownSelection)\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!top-down selection from different lists}
- [Banding](#sec-banding)\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!banding}
- [Banding with bonus points](#sec-bandingBonusPoints)\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!banding with bonus points}
- [Sliding band](#sec-slidingBand)\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!sliding band}
- [Separate tests](#sec-separateTests)\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!separate tests}
- [Item elimination based on group differences](#sec-itemElimination)\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!item elimination based on group differences}

##### Bonus Points {#sec-bonusPoints}
...
Bonus points is used to correct for [predictive bias](#sec-predictiveBias) [differences in intercepts](#sec-differentIntercepts) between groups.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!bonus points}\index{bias!predictive!different intercepts}
...
An example of using bonus points as a score adjustment is depicted in @fig-bonusPoints.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!bonus points}

::: {#fig-bonusPoints}
![](images/adjustment_bonusPoints.png){width=100% fig-alt="Using Bonus Points as a Scoring Adjustment."}
...
##### Within-Group Norming {#sec-withinGroupNorming}

A [norm](#sec-norm) is the standard of performance that a person's performance can be compared to.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}\index{norm}
Within-group norming treats the person's group in the sample as the [norm](#sec-norm).\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}
Within-group norming converts an individual's score to [standardized scores](#sec-standardizedScores) (e.g., T scores) or percentiles within one's own group.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}\index{data!standardized}
Then, the people are selected based on the highest standard scores across groups.
Withing-group norming is used to correct for [predictive bias](#sec-predictiveBias) [differences in slopes](#sec-differentSlopes) between groups.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}\index{bias!predictive!different slopes}
An example of using within-group norming as a score adjustment is depicted in @fig-withinGroupNorming.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}

::: {#fig-withinGroupNorming}
![](images/adjustment_withinGroupNorming.png){width=100% fig-alt="Using Within-Group Norming as a Scoring Adjustment."}
...
First, it accounts for differences in group standard deviations and means, so it does not have the same problem as [bonus points](#sec-bonusPoints) and is generally more effective at eliminating [adverse impact](#sec-adverseImpact) compared to [bonus points](#sec-bonusPoints).\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}\index{bias!correcting for!bonus points}\index{adverse impact}
Second, some general (non-group-specific) [norms](#sec-norm) are clearly irrelevant for characterizing a person's functioning.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}
Group-specific [norms](#sec-norm) aim to describe a person's performance relative to people with a similar background, thus potentially reducing cultural [bias](#sec-bias).\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}
Third, group-specific [norms](#sec-norm) may better reflect cultural, educational, socioeconomic, and other factors that may influence a person's score [@Burlew2019].\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}
Fourth, group-specific [norms](#sec-norm) may increase [specificity](#sec-specificity), and reduce over-pathologizing by preventing giving a diagnosis to people who might not show a condition [@Manly2007].\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}\index{specificity}
...
However, percentile scores will consistently eliminate [adverse impact](#sec-adverseImpact).\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}\index{data!percentile rank}\index{adverse impact}
Second, using group-specific [norms](#sec-norm) may obscure background variables that explain underlying reasons for group-related differences in test performance [@Manly2005; @Manly2007].\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}
Third, group-specific [norms](#sec-norm) do not address the problem if the measure shows test [bias](#sec-bias) [@Burlew2019].\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}
Fourth, group-specific [norms](#sec-norm) may reduce [sensitivity](#sec-sensitivity) to detect conditions [@Manly2007].\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}\index{sensitivity}
...
Race-norming compared the Black football players cognitive test scores to group-specific [norms](#sec-norm): the cognitive test scores of Black individuals in the general population (not to common [norms](#sec-norm)).\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}\index{norm-referenced!race}
Using Black-specific [norms](#sec-norm) assumed that Black football players showed lower cognitive ability than other groups, so a low cognitive ability score for a Black player was less likely to be flagged as concerning.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}\index{norm-referenced!race}
Thus, the race-specific [norms](#sec-norm) led to lower identified rates of concussions among Black football players compared to White football players.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}\index{norm-referenced!race}
Due to the [adverse impact](#sec-adverseImpact), Black players sued the National Football League, and the league stopped the controversial practice of race-norming for identification of concussion\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}\index{adverse impact} (<https://www.washingtonpost.com/sports/2021/06/03/nfl-concussion-settlement-race-norming/>; archived at <https://perma.cc/KN3L-5Z7R>).\index{norm-referenced!race}

A common question is whether to use group-specific [norms](#sec-norm) or common [norms](#sec-norm).\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}
Group-specific [norms](#sec-norm) are a controversial practice, and the answer depends.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}
If you are interested in a person's absolute functioning (e.g., for determining whether someone is concussed or whether they are suitable to drive), recommendations are to use common [norms](#sec-norm), not group-specific [norms](#sec-norm) [@Barrash2010; @Silverberg2009].\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}
...
The question about which [norms](#sec-norm) to use are complex, and psychologists should evaluate the cost and benefit of each [norm](#sec-norm), and use the [norm](#sec-norm) with the greatest benefit and the least cost for the client [@Manly2007].\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}

##### Separate Cutoffs {#sec-separateCutoffs}
...
Using separate cutoffs functions the same as adding [bonus points](#sec-bonusPoints), but it has greater transparency—i.e., you are lowering the standard for one group compared to another group.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!separate cutoffs}\index{bias!correcting for!bonus points}
An example of using separate cutoffs as a score adjustment is depicted in @fig-separateCutoffs.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!separate cutoffs}

::: {#fig-separateCutoffs}
![](images/adjustment_separateCutoffs.png){width=100% fig-alt="Using Separate Cutoffs as a Scoring Adjustment. In this example, the cutoff for the majority group is 128; the cutoff for the minority group is 123.", fig.scap = "Using Separate Cutoffs as a Scoring Adjustment."}

Using Separate Cutoffs as a Scoring Adjustment. In this example, the cutoff for the majority group is 128; the cutoff for the minority group is 123.", fig.scap = "Using Separate Cutoffs as a Scoring Adjustment.
:::

##### Top-Down Selection from Different Lists {#sec-topDownSelection}
...
Top-down selection from different lists functions the same as [within-group norming](#sec-withinGroupNorming).\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!top-down selection from different lists}\index{bias!correcting for!within-group norming}
An example of using top-down selection from different lists as a score adjustment is depicted in @fig-topDownSelectionDifferentLists.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!top-down selection from different lists}

::: {#fig-topDownSelectionDifferentLists}
![](images/adjustment_separateLists.png){width=100% fig-alt="Using Top-Down Selection From Different Lists as a Scoring Adjustment. In this example, the top three candidates are selected from each group."}
...
An example of using banding as a score adjustment is depicted in @fig-banding.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!banding}

::: {#fig-banding}
![](images/adjustment_banding.png){width=100% fig-alt="Using Banding as a Scoring Adjustment."}

Using Banding as a Scoring Adjustment.
:::

The problem with banding is that bands are set by the [standard error of measurement](#sec-standardErrorOfMeasurement): you can select the first group from the first band, but then whom do you select after the first band?\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!banding}\index{reliability!standard error of measurement}
...
A solution to this problem with banding is to use a [sliding band](#sec-slidingBand), as described later.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!banding}\index{bias!correcting for!sliding band}

##### Banding with Bonus Points {#sec-bandingBonusPoints}

[Banding](#sec-banding) is often used with [bonus points](#sec-bonusPoints) to reduce the negative impact for minority groups.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!banding with bonus points}\index{bias!correcting for!banding}\index{bias!correcting for!bonus points}
An example of using [banding](#sec-banding) with [bonus points](#sec-bonusPoints) as a score adjustment is depicted in @fig-bandingBonusPoints.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!banding with bonus points}\index{bias!correcting for!banding}\index{bias!correcting for!bonus points}

::: {#fig-bandingBonusPoints}
![](images/adjustment_bandingBonusPoints.png){width=100% fig-alt="Using Banding With Bonus Points as a Scoring Adjustment."}

Using Banding With Bonus Points as a Scoring Adjustment.
:::

##### Sliding Band {#sec-slidingBand}

Using a sliding band is a solution to the problem of which bands to use when using [banding](#sec-banding).\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!sliding band}\index{bias!correcting for!banding}
...
Using the top band, you select all members of a minority group in the top band, then select members of the majority group with the top score of the band, then slide the band down (based on [SEM](#sec-standardErrorOfMeasurement)), and repeat.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!sliding band}
You work your way down with bands though groups that are indistinguishable based on [SEM](#sec-standardErrorOfMeasurement), until getting a cell needed to select a relevant candidate.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!sliding band}\index{reliability!standard error of measurement}

For instance, if the top score is 22 and the [SEM](#sec-standardErrorOfMeasurement) is 4 points, the first band would be: [18, 22].\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!sliding band}\index{reliability!standard error of measurement}
...
An example of using a sliding band as a score adjustment is depicted in @fig-slidingBand.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!sliding band}

::: {#fig-slidingBand}
![](images/adjustment_slidingBand.png){width=100% fig-alt="Using a Sliding Band as a Scoring Adjustment."}
...
Using a sliding band has the same effects on decisions as [bonus points](#sec-bonusPoints) that are the width of the band.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!sliding band}\index{bias!correcting for!bonus points}
For example, if the [SEM](#sec-standardErrorOfMeasurement) is 3, it has the same decisions as [bonus points](#sec-bonusPoints) of 3; therefore, any scores within 3 of the highest score are now considered equal.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!sliding band}\index{reliability!standard error of measurement}
...
However, a sliding band may not always eliminate [adverse impact](#sec-adverseImpact).\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!sliding band}\index{adverse impact}
...
##### Separate Tests {#sec-separateTests}
...
For instance, you might use one test for the majority group and a different test for the minority group, making sure that each test is [valid](#sec-validity) for the relevant group.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!separate tests}\index{validity}
Using separate tests is an extreme version of [top-down selection](#sec-topDownSelection) and [within-group norming](#sec-withinGroupNorming).\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!separate tests}\index{bias!correcting for!top-down selection from different lists}\index{bias!correcting for!separate tests}\index{bias!correcting for!within-group norming}
Using separate tests would be an option if a measure shows [different slopes](#sec-differentSlopes) [predictive bias](#sec-predictiveBias).\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!separate tests}\index{bias!predictive!different slopes}
...
Empirical keying is an example of [dustbowl empiricism](#sec-theoryEmpiricism) (i.e., relying on empiricism rather than theory).\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!separate tests}\index{radical operationalism}\index{empiricism}
...
##### Item Elimination based on Group Differences {#sec-itemElimination}
...
#### Use of Score Adjustment {#sec-useOfScoreAdjustment}
...
1. [Bonus points](#sec-bonusPoints)\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!bonus points}
1. [Within-group norming](#sec-withinGroupNorming)\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}

[Bonus points](#sec-bonusPoints) and [within-group norming](#sec-withinGroupNorming) are the techniques that are most often used in the real world.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!bonus points}\index{bias!correcting for!within-group norming}
...
### Other Ways to Correct for Bias {#sec-otherBiasCorrections}
...
#### Use Multiple Predictors {#sec-useMultiplePredictors}
...
Using multiple predictors works best when the predictors are not correlated with the assessment that has [adverse impact](#sec-adverseImpact), which is difficult to achieve.\index{bias!correcting for}\index{bias!correcting for!use multiple predictors}\index{adverse impact}
...
Include as many relevant aspects of the construct as possible for [content validity](#sec-contentValidity).\index{bias!correcting for}\index{bias!correcting for!use multiple predictors}\index{validity!content}
...
#### Change the Criterion {#sec-changeCriterion}

Another option is to change the criterion so that the [predictive validity](#sec-predictiveValidity) of tests is less skewed.\index{bias!correcting for}\index{bias!correcting for!change the criterion}\index{validity!predictive}
...
If an item is clearly [invalid](#sec-validity) in one group but [valid](#sec-validity) in another group, another option is to keep the item in one group, and to remove it in another group.\index{item response theory!differential item functioning}\index{bias!correcting for!remove biased items}\index{validity}

Be careful when removing items because removing items can lead to poorer [content validity](#sec-contentValidity)—i.e., items may no longer be a representative set of the content of the construct.\index{bias!correcting for}\index{bias!correcting for!remove biased items}\index{validity!content}
Removing items also reduces a measure's [reliability](#sec-reliability) and ability to detect individual differences [@Hagquist2017; @Hagquist2019].\index{bias!correcting for}\index{bias!correcting for!remove biased items}\index{reliability}
[DIF](#sec-dif) effects tend to be small and inconsistent; removing items showing [DIF](#sec-dif) may not have a big impact.\index{bias!correcting for}\index{bias!correcting for!remove biased items}\index{item response theory!differential item functioning}

#### Resolve Biased Items {#sec-resolveBiasedItems}

Another option, for items identified that show [differential item functioning](#sec-dif) using [IRT](#sec-irt) or [measurement non-invariance](#sec-measurementInvariance) using [CFA](#sec-cfa), is to resolve instead of [remove](#sec-removeBiasedItems) items.\index{bias!correcting for}\index{bias!correcting for!resolve biased items}\index{item response theory}\index{item response theory!differential item functioning}\index{structural equation modeling!measurement invariance}
Resolving items involves allowing an item to have a different [discrimination](#sec-itemDiscrimination)/factor loading and/or [difficulty](#sec-itemDifficulty)/intercept parameter for each group.\index{bias!correcting for}\index{bias!correcting for!resolve biased items}\index{item response theory!differential item functioning}\index{item response theory!item difficulty}\index{item response theory!item discrimination}\index{structural equation modeling!measurement invariance}\index{structural equation modeling!factor loading}\index{structural equation modeling!intercept}
Allowing item parameters to differ across groups has a very small effect on [reliability](#sec-reliability) and person separation, so it can be preferable to [removing items](#sec-removeBiasedItems) [@Hagquist2017; @Hagquist2019].\index{bias!correcting for}\index{bias!correcting for!resolve biased items}\index{reliability}

#### Use Alternative Modes of Testing {#sec-alternativeTestingModes}
...
#### Use Work Records {#sec-workRecords}
...
In some cases, use of work records can actually increase [adverse impact](#sec-adverseImpact) on under-represented groups because the primary group typically already has an idea of how to get into the relevant job or is already in the relevant job; therefore, they have a leg up.\index{bias!correcting for}\index{bias!correcting for!work records}\index{adverse impact}
...
#### Increase Time Limit {#sec-increaseTimeLimit}
...
Time limits often lead to greater [measurement error](#sec-measurementError) because scores conflate pace and quality of work.\index{bias!correcting for}\index{bias!correcting for!increase time limit}
...
#### Use Motivation Sets {#sec-motivationSets}
...
Motivation sets could include, for example, using more realistic test stimuli that are clearly applicable to the school or job requirements (i.e., that have [face validity](#sec-faceValidity)) to motivate all test takers.\index{bias!correcting for}\index{bias!correcting for!motivation sets}\index{validity!face}

#### Use Instructional Sets {#sec-instructionalSets}
...
## Getting Started {#sec-gettingStarted-bias}

### Load Libraries {#sec-loadLibraries-bias}
...
### Prepare Data {#sec-prepareData-bias}

#### Load Data {#sec-loadData-bias}
...
#### Simulate Data {#sec-simulateData-bias}
...
#### Add Missing Data {#sec-addMissingData-bias}
...
## Examples of Unbiased Tests (in Terms of Predictive Bias) {#sec-examplesUnbiasedTests}

### Unbiased test where males and females have equal means on predictor and criterion {#sec-unbiasedTestsEqualMeans}

@fig-unbiasedEqualMeans depicts an example of an unbiased test where males and females have equal means on the predictor and criterion.\index{bias!predictive!unbiased}
...
```{r}
#| label: fig-unbiasedEqualMeans
#| fig-cap: "Unbiased Test Where Males and Females Have Equal Means on Predictor and Criterion."
#| fig-alt: "Unbiased Test Where Males and Females Have Equal Means on Predictor and Criterion."
#| code-fold: true
...
### Unbiased test where females have higher means than males on predictor and criterion {#sec-unbiasedTestsFemalesHigherMeans}

@fig-unbiasedFemaleHigher depicts an example of an unbiased test where females have higher means than males on the predictor and criterion.\index{bias!predictive!unbiased}
...
```{r}
#| label: fig-unbiasedFemaleHigher
#| fig-cap: "Unbiased Test Where Females Have Higher Means Than Males on Predictor and Criterion."
#| fig-alt: "Unbiased Test Where Females Have Higher Means Than Males on Predictor and Criterion."
#| code-fold: true
...
### Unbiased test where males have higher means than females on predictor and criterion {#sec-unbiasedTestsMalesHigherMeans}

@fig-unbiasedMaleHigher depicts an example of an unbiased test where males have higher means than females on the predictor and criterion.\index{bias!predictive!unbiased}
The test is unbiased because there are no differences in the regression lines (of `predictor` predicting `criterion`) between males and females.\index{bias!predictive!unbiased}
...
```{r}
#| label: fig-unbiasedMaleHigher
#| fig-cap: "Unbiased Test Where Males Have Higher Means Than Females on Predictor and Criterion."
#| fig-alt: "Unbiased Test Where Males Have Higher Means Than Females on Predictor and Criterion."
#| code-fold: true
...
## Predictive Bias: Different Regression Lines {#sec-predictiveBiasExamples}

### Example of unbiased prediction (no differences in intercepts or slopes) {#sec-unbiasedPrediction}

@fig-unbiased2 depicts an example of an unbiased test where males and females have equal means on the predictor and criterion.\index{bias!predictive!unbiased}
...
```{r}
#| label: fig-unbiased2
#| fig-cap: "Example of Unbiased Prediction (No Differences in Intercepts or Slopes Between Males and Females)."
#| fig-alt: "Example of Unbiased Prediction (No Differences in Intercepts or Slopes Between Males and Females)."
#| code-fold: true
...
### Example of intercept bias {#sec-interceptBias}

@fig-interceptBias depicts an example of a biased test due to intercept bias.\index{bias!predictive!different intercepts}
...
```{r}
#| label: fig-interceptBias
#| fig-cap: "Example of Intercept Bias in Prediction (Different Intercepts Between Males and Females)."
#| fig-alt: "Example of Intercept Bias in Prediction (Different Intercepts Between Males and Females)."
#| code-fold: true
...
### Example of slope bias {#sec-slopeBias}

@fig-slopeBias depicts an example of a biased test due to slope bias.\index{bias!predictive!different slopes}
...
That is, scores have stronger [predictive validity](#sec-predictiveValidity) for males than females.\index{bias!predictive!different slopes}\index{validity!predictive}
...
```{r}
#| label: fig-slopeBias
#| fig-cap: "Example of Slope Bias in Prediction (Different Slopes Between Males And Females)."
#| fig-alt: "Example of Slope Bias in Prediction (Different Slopes Between Males And Females)."
#| code-fold: true
...
### Example of intercept and slope bias {#sec-interceptSlopeBias}

@fig-interceptSlopeBias depicts an example of a biased test due to intercept and slope bias.\index{bias!predictive!different intercepts and slopes}
...
That is, scores have stronger [predictive validity](#sec-predictiveValidity) for males than females.\index{bias!predictive!different intercepts and slopes}
...
```{r}
#| label: fig-interceptSlopeBias
#| fig-cap: "Example of Intercept And Slope Bias in Prediction (Different Intercepts and Slopes Between Males and Females)."
#| fig-alt: "Example of Intercept And Slope Bias in Prediction (Different Intercepts and Slopes Between Males and Females)."
#| code-fold: true
...
### Example of different measurement reliability/error across groups {#sec-differentialReliability}

In the example depicted in @fig-differentMeasurementError, there are differences in the measurement [reliability](#sec-reliability)/[error](#sec-measurementError) on the criterion between males and females: males' scores have a lower [reliability](#sec-reliability) (higher [measurement error](#sec-measurementError)) on the criterion than females' scores.\index{bias!predictive!different measurement error}\index{measurement error}\index{reliability}
...
```{r}
#| label: fig-differentMeasurementError
#| fig-cap: "Example of Different Measurement Reliability/Error Across Groups."
#| fig-alt: "Example of Different Measurement Reliability/Error Across Groups."
#| code-fold: true
...
For instance, the [severity](#sec-itemDifficulty) or [discrimination](#sec-itemDiscrimination) of an item could be higher in one group compared to another group.\index{item response theory!differential item functioning}\index{item response theory!item difficulty}\index{item response theory!item discrimination}
...
However, in adulthood, disobedience to authority could reflect prosocial functions, such as protesting against societally unjust actions, and may show weaker [construct validity](#sec-constructValidity) with respect to externalizing problems, as operationalized by a weaker [discrimination](#sec-itemDiscrimination) coefficient in adulthood than in childhood.\index{item response theory!differential item functioning}\index{validity!construct}\index{item response theory!item discrimination}
Tests of differential item functioning are equivalent to tests of [measurement invariance](#sec-measurementInvariance).\index{item response theory!differential item functioning}\index{structural equation modeling!measurement invariance}

Approaches to addressing DIF are described in @sec-whenDetectingBias.\index{item response theory!differential item functioning}
If we identify that an item shows non-negligible DIF, we have three primary options (described above): (1) drop the item for both groups, (2) drop the item for one group but keep it for the other group, or (3) freely estimate the parameters ([discrimination](#sec-itemDiscrimination) and [difficulty](#sec-itemDifficulty)) for the item across groups.\index{item response theory!differential item functioning}\index{item response theory!item difficulty}\index{item response theory!item discrimination}

Tests of DIF were conducted using the `mirt` package [@R-mirt].\index{item response theory!differential item functioning}

### Item Descriptive Statistics {#sec-descriptiveStats-bias}
...
### Unconstrained Model {#sec-difUnconstrained}

#### Fit Model {#sec-difUnconstrainedFit}
...
#### Model Summary {#sec-difUnconstrainedSummary}
...
- item 4 has higher [difficulty/severity](#sec-itemDifficulty) ($b_1$ and $b_2$ parameters) among females than males\index{item response theory!differential item functioning}\index{item response theory!item difficulty}
...
### Constrained Model {#sec-difConstrained}

Constrain item parameters to be equal across groups (to use as baseline model for identifying [DIF](#sec-dif)).\index{item response theory!differential item functioning}

#### Fit Model {#sec-difConstrainedFit}
...
#### Model Summary {#sec-difConstrainedSummary}
...
### Compare model fit of constrained model to unconstrained model {#sec-difCompareConstrainedUnconstrained}
...
### Identify DIF by iteratively removing constraints from fully constrained model {#sec-difRemoveConstraints}

One way to identify DIF is to iteratively remove constraints from a model in which all parameters are constrained to be the same across groups.\index{item response theory!differential item functioning}
Removing constraints allows individual items to have different item parameters across groups, to identify which items yield a significant improvement in model fit when allowing their [discrimination](#sec-itemDiscrimination) and/or [severity](#sec-itemDifficulty) to differ across groups.\index{item response theory!differential item functioning}
Items 1, 3, 4, 5, 6, and 7 showed [DIF](#sec-dif) in [discrimination](#sec-itemDiscrimination) and/or [severity](#sec-itemDifficulty), based on a significant chi-square difference test.\index{item response theory!differential item functioning}\index{item response theory!item discrimination}\index{item response theory!item difficulty}\index{chi-square!difference test}

#### Items that differ in discrimination and/or severity {#sec-difRemoveConstraintsDiscriminationSeverity}
...
#### Items that differ in discrimination {#sec-difRemoveConstraintsDiscrimination}

Items 1, 3, 4, and 5 showed [DIF](#sec-dif) in [discrimination](#sec-itemDiscrimination), based on a significant chi-square difference test.\index{item response theory!differential item functioning}\index{item response theory!item discrimination}\index{chi-square!difference test}
...
#### Items that differ in severity {#sec-difRemoveConstraintsSeverity}

Items 1, 3, 4, 5, and 7 showed [DIF](#sec-dif) in [difficulty/severity](#sec-itemDifficulty), based on a significant chi-square difference test.\index{item response theory!differential item functioning}\index{item response theory!item difficulty}\index{chi-square!difference test}
...
### Identify DIF by iteratively adding constraints to unconstrained model {#sec-difAddConstraints}

#### Items that differ in discrimination and/or severity {#sec-difAddConstraintsDiscriminationSeverity}

Another way to identify DIF is to iteratively add constraints to a model in which all parameters are allowed to differ across groups.\index{item response theory!differential item functioning}
Adding constraints forces individual items to have the same item parameters across groups, to identify which items yield a significant worsening in model fit when constraining their [discrimination](#sec-itemDiscrimination) and/or [severity](#sec-itemDifficulty) to be the same across groups.\index{item response theory!differential item functioning}
Items 1, 2, 3, 4, 5, and 6 showed [DIF](#sec-dif) in [discrimination](#sec-itemDiscrimination) and/or [severity](#sec-itemDifficulty), based on a significant chi-square difference test.\index{item response theory!differential item functioning}\index{item response theory!item discrimination}\index{item response theory!item difficulty}\index{chi-square!difference test}
The [DIF](#sec-dif) in [discrimination](#sec-itemDiscrimination) and/or [severity](#sec-itemDifficulty) is depicted in @fig-difAddDiscriminationSeverity.\index{item response theory!differential item functioning}\index{item response theory!item discrimination}\index{item response theory!item difficulty}

```{r}
#| label: fig-difAddDiscriminationSeverity
#| fig-cap: "Item Probability Functions for Examining Differential Item Functioning in Terms of Discrimination and/or Severity."
#| fig-alt: "Item Probability Functions for Examining Differential Item Functioning in Terms of Discrimination and/or Severity."
#| fig-show: "hold"
...
#### Items that differ in discrimination {#sec-difAddConstraintsDiscrimination}

Items 6 and 7 showed [DIF](#sec-dif) in [discrimination](#sec-itemDiscrimination), based on a significant chi-square difference test.\index{item response theory!differential item functioning}\index{item response theory!item discrimination}\index{chi-square!difference test}
The [DIF](#sec-dif) in [discrimination](#sec-itemDiscrimination) is depicted in @fig-difAddDiscrimination.\index{item response theory!differential item functioning}\index{item response theory!item discrimination}

```{r}
#| label: fig-difAddDiscrimination
#| fig-cap: "Item Probability Functions for Examining Differential Item Functioning in Terms of Discrimination."
#| fig-alt: "Item Probability Functions for Examining Differential Item Functioning in Terms of Discrimination."
#| fig-show: "hold"
...
#### Items that differ in severity {#sec-difAddConstraintsSeverity}

Items 1, 2, 3, 4, 5, and 6 showed [DIF](#sec-dif) in [difficulty/severity](#sec-itemDifficulty), based on a significant chi-square difference test.\index{item response theory!differential item functioning}\index{item response theory!item difficulty}\index{chi-square!difference test}
The [DIF](#sec-dif) in [difficulty/severity](#sec-itemDifficulty) is depicted in @fig-difAddSeverity\index{item response theory!differential item functioning}\index{item response theory!item difficulty}

```{r}
#| label: fig-difAddSeverity
#| fig-cap: "Item Probability Functions for Examining Differential Item Functioning in Terms of Severity."
#| fig-alt: "Item Probability Functions for Examining Differential Item Functioning in Terms of Severity."
#| fig-show: "hold"
...
### Compute effect size of DIF {#sec-difEffectSize}

Effect size measures of [DIF](#sec-dif) were computed based on expected scores [@Meade2010] using the `mirt` package [@R-mirt].\index{item response theory!differential item functioning}\index{effect size}
Some researchers recommend using a simulation-based procedure to examine the impact of [DIF](#sec-dif) on screening accuracy [@Gonzalez2021].\index{item response theory!differential item functioning}\index{effect size}

#### Test-level DIF {#sec-difTestLevel}

In addition to consideration of [differential test functioning](#sec-dif), we can also consider whether the test as a whole (i.e., the collection of items) differs in its functioning by group—called differential test functioning.
Differential test functioning is depicted in @fig-dtf.\index{item response theory!differential item functioning}\index{effect size}
Estimates of differential test functioning are in @tbl-testLevelDIF.\index{item response theory!differential item functioning}

```{r}
#| label: tbl-testLevelDIF
#| tbl-cap: "Estimates of Differential Test Functioning."
...
In general, the measure showed greater [difficulty](#sec-itemDifficulty) for females than for males.\index{item response theory!differential item functioning}\index{effect size}\index{item response theory!item difficulty}
...
```{r}
#| label: fig-dtf
#| fig-cap: "Differential Test Functioning by Sex."
#| fig-alt: "Differential Test Functioning by Sex."
...
#### Item-level DIF {#sec-difItemLevel}

[Differential item functioning](#sec-dif) is depicted in @fig-dif.\index{item response theory!differential item functioning}\index{effect size}
Estimates of [differential item functioning](#sec-dif) are in @tbl-itemLevelDIF.\index{item response theory!differential item functioning}

```{r}
#| label: tbl-itemLevelDIF
#| tbl-cap: "Estimates of Differential Item Functioning."
...
The measure-level differences in functioning appear to be largely driven by greater [difficulty](#sec-itemDifficulty) of items 4 and 5 for females than males.\index{item response theory!differential item functioning}\index{effect size}\index{item response theory!item difficulty}
...
```{r}
#| label: fig-dif
#| fig-cap: "Differential Item Functioning by Sex."
#| fig-alt: "Differential Item Functioning by Sex."
...
### Item plots {#sec-difItemPlots}

Plots of item response category characteristic curves by sex are in Figures [-@fig-ircSex1]–[-@fig-ircSex7] below.\index{item response theory!differential item functioning}\index{item response theory!item response category characteristic curve}

```{r}
#| label: fig-ircSex1
#| fig-cap: "Item Response Category Characteristic Curves by Sex: Item 1."
#| fig-alt: "Item Response Category Characteristic Curves by Sex: Item 1."
...
```{r}
#| label: fig-ircSex2
#| fig-cap: "Item Response Category Characteristic Curves by Sex: Item 2."
#| fig-alt: "Item Response Category Characteristic Curves by Sex: Item 2."
...
```{r}
#| label: fig-ircSex3
#| fig-cap: "Item Response Category Characteristic Curves by Sex: Item 3."
#| fig-alt: "Item Response Category Characteristic Curves by Sex: Item 3."
...
```{r}
#| label: fig-ircSex4
#| fig-cap: "Item Response Category Characteristic Curves by Sex: Item 4."
#| fig-alt: "Item Response Category Characteristic Curves by Sex: Item 4."
...
```{r}
#| label: fig-ircSex5
#| fig-cap: "Item Response Category Characteristic Curves by Sex: Item 5."
#| fig-alt: "Item Response Category Characteristic Curves by Sex: Item 5."
...
```{r}
#| label: fig-ircSex6
#| fig-cap: "Item Response Category Characteristic Curves by Sex: Item 6."
#| fig-alt: "Item Response Category Characteristic Curves by Sex: Item 6."
...
```{r}
#| label: fig-ircSex7
#| fig-cap: "Item Response Category Characteristic Curves by Sex: Item 7."
#| fig-alt: "Item Response Category Characteristic Curves by Sex: Item 7."
...
Plots of item [information](#sec-irtReliability) functions by sex are in Figures [-@fig-itemInfo1]–[-@fig-itemInfo7] below.\index{item response theory!differential item functioning}\index{item response theory!item information curve}
Items 1, 5, and 6 showed greater [information](#sec-irtReliability) for females than for males.\index{item response theory!differential item functioning}\index{item response theory!item information curve}
Items 2, 4, and 7 showed greater [information](#sec-irtReliability) for females than for males.\index{item response theory!differential item functioning}\index{item response theory!item information curve}

```{r}
#| label: fig-itemInfo1
#| fig-cap: "Item Information Curves by Sex: Item 1."
#| fig-alt: "Item Information Curves by Sex: Item 1."
...
```{r}
#| label: fig-itemInfo2
#| fig-cap: "Item Information Curves by Sex: Item 2."
#| fig-alt: "Item Information Curves by Sex: Item 2."
...
```{r}
#| label: fig-itemInfo3
#| fig-cap: "Item Information Curves by Sex: Item 3."
#| fig-alt: "Item Information Curves by Sex: Item 3."
...
```{r}
#| label: fig-itemInfo4
#| fig-cap: "Item Information Curves by Sex: Item 4."
#| fig-alt: "Item Information Curves by Sex: Item 4."
...
```{r}
#| label: fig-itemInfo5
#| fig-cap: "Item Information Curves by Sex: Item 5."
#| fig-alt: "Item Information Curves by Sex: Item 5."
...
```{r}
#| label: fig-itemInfo6
#| fig-cap: "Item Information Curves by Sex: Item 6."
#| fig-alt: "Item Information Curves by Sex: Item 6."
...
```{r}
#| label: fig-itemInfo7
#| fig-cap: "Item Information Curves by Sex: Item 7."
#| fig-alt: "Item Information Curves by Sex: Item 7."
...
Plots of expected item scores by sex are are in Figures [-@fig-itemScore1]–[-@fig-itemScore7] below.\index{item response theory!differential item functioning}

```{r}
#| label: fig-itemScore1
#| fig-cap: "Expected Item Score by Sex: Item 1."
#| fig-alt: "Expected Item Score by Sex: Item 1."
...
```{r}
#| label: fig-itemScore2
#| fig-cap: "Expected Item Score by Sex: Item 2."
#| fig-alt: "Expected Item Score by Sex: Item 2."
...
```{r}
#| label: fig-itemScore3
#| fig-cap: "Expected Item Score by Sex: Item 3."
#| fig-alt: "Expected Item Score by Sex: Item 3."
...
```{r}
#| label: fig-itemScore4
#| fig-cap: "Expected Item Score by Sex: Item 4."
#| fig-alt: "Expected Item Score by Sex: Item 4."
...
```{r}
#| label: fig-itemScore5
#| fig-cap: "Expected Item Score by Sex: Item 5."
#| fig-alt: "Expected Item Score by Sex: Item 5."
...
```{r}
#| label: fig-itemScore6
#| fig-cap: "Expected Item Score by Sex: Item 6."
#| fig-alt: "Expected Item Score by Sex: Item 6."
...
```{r}
#| label: fig-itemScore7
#| fig-cap: "Expected Item Score by Sex: Item 7."
#| fig-alt: "Expected Item Score by Sex: Item 7."
...
### Addressing DIF {#sec-addressingDIF}

Based on the analyses above, item 5 shows the largest magnitude of [DIF](#sec-dif), followed by item 4.
Specifically, item 5 has a stronger [discrimination](#sec-itemDiscrimination) parameter for women than men.\index{item response theory!differential item functioning}\index{item response theory!item discrimination}
So, we should handle item 5 first.\index{item response theory!differential item functioning}
If we deem the [DIF](#sec-dif) for item 5 to be non-negligible, we have three primary options: (1) drop item 5 for both men and women, (2) drop item 5 for men but keep it for women, or (3) freely estimate the parameters ([discrimination](#sec-itemDiscrimination) and [difficulty](#sec-itemDifficulty)) for item 5 across groups.\index{item response theory!differential item functioning}\index{item response theory!item discrimination}\index{bias!correcting for!resolve biased items}\index{bias!correcting for!remove biased items}

#### Drop item for both groups {#sec-dropItemForBothGroups}

The first option is to drop item 5 for both men and women groups.\index{item response theory!differential item functioning}\index{bias!correcting for!remove biased items}
...
However, this can lead to lower [reliability](#sec-reliability) and weaker ability to detect individual differences.\index{item response theory!differential item functioning}\index{bias!correcting for!remove biased items}\index{reliability}
...
#### Drop item for one group but not another group {#sec-dropItemForOneGroup}
...
You might do this if the item is [invalid](#sec-validity) for men, but still [valid](#sec-validity) for women.\index{item response theory!differential item functioning}\index{bias!correcting for!remove biased items}\index{validity}
...
#### Freely estimate item to have different parameters across groups {#sec-resolveItemAcrossGroups}

Alternatively, we can resolve [DIF](#sec-dif) by allowing the item to have different parameters ([discrimination](#sec-itemDiscrimination) and [difficulty](#sec-itemDifficulty)) across both groups.\index{item response theory!differential item functioning}\index{bias!correcting for!resolve biased items}\index{item response theory!item difficulty}\index{item response theory!item discrimination}
You might do this if the item is [valid](#sec-validity) for both men and women, and it has importantly different item parameters nonetheless.\index{item response theory!differential item functioning}\index{bias!correcting for!resolve biased items}\index{validity}
...
#### Compare model fit {#sec-difResolveItemsCompareModels}
...
#### Next steps {#sec-difNextSteps}

Whichever of these approaches we select, we would then identify and handle the remaining non-negligent [DIF](#sec-dif) sequentially by magnitude.\index{item response theory!differential item functioning}\index{bias!correcting for!resolve biased items}\index{bias!correcting for!remove biased items}
For instance, if we chose to resolve [DIF](#sec-dif) in item 5 by allowing the item to have different parameters across both groups, we would then iteratively drop constraints to see which items continue to show non-negligible [DIF](#sec-dif).\index{item response theory!differential item functioning}\index{bias!correcting for!resolve biased items}
...
```{r}
#| label: tbl-itemLevelDIFAfterResolving
#| tbl-cap: "Differential Item Functioning After Resolving DIF in Item 5."
#| echo: false
...
Estimates of [DIF](#sec-dif) after resolving item 5 are in @tbl-itemLevelDIFAfterResolving.\index{item response theory!differential item functioning}
Of the remaining [DIF](#sec-dif), item 4 appears to have the largest [DIF](#sec-dif).\index{item response theory!differential item functioning}\index{bias!correcting for!resolve biased items}\index{bias!correcting for!remove biased items}
If we deem item 4 to show non-negligible [DIF](#sec-dif), we would address it using one of the three approaches above and then see which items continue to show the largest [DIF](#sec-dif), address it if necessary, then identify the remaining [DIF](#sec-dif), etc.\index{item response theory!differential item functioning}\index{bias!correcting for!resolve biased items}\index{bias!correcting for!remove biased items}

## Measurement/Factorial Invariance {#sec-measurementInvariance}
...
Tests of measurement invariance are equivalent to tests of [differential item functioning](#sec-dif).\index{structural equation modeling!measurement invariance}\index{item response theory!differential item functioning}
...
Measurement invariance can be tested using many methods, including [confirmatory factor analysis](#sec-cfa) and [IRT](#sec-irt).\index{structural equation modeling!measurement invariance}\index{factor analysis!confirmatory}\index{item response theory}
[CFA](#sec-cfa) approaches to testing measurement invariance include multi-group [CFA](#sec-cfa) (MGCFA), multiple-indicator, multiple-causes (MIMIC) models, moderated nonlinear factor analysis (MNLFA), score-based tests [@Wang2014a], and the alignment method [@Lai2021].\index{structural equation modeling!measurement invariance}\index{factor analysis!confirmatory}
The [IRT](#sec-irt) approach to testing measurement (non-)invariance is to examine whether items show [differential item functioning](#sec-dif).\index{structural equation modeling!measurement invariance}\index{item response theory}\index{item response theory!differential item functioning}

The tests of measurement invariance in [confirmatory factor analysis](#sec-cfa) (CFA) models were fit in the `lavaan` package [@R-lavaan].\index{structural equation modeling!measurement invariance}\index{factor analysis!confirmatory}
The examples were adapted from `lavaan` documentation:\index{structural equation modeling!measurement invariance}\index{factor analysis!confirmatory} <https://lavaan.ugent.be/tutorial/groups.html> (archived at <https://perma.cc/2FBK-RSAH>).
...
Although measurement invariance is important to test, it is also worth noting that tests of measurement invariance and [DIF](#sec-dif) rest on various fundamentally untestable assumptions related to scale setting [@Raykov2020].\index{structural equation modeling!measurement invariance}\index{item response theory!differential item functioning}
...
Approaches to addressing measurement non-invariance are described in @sec-whenDetectingBias.
...
### Effect Size of Measurement Non-Invariance {#sec-measurementNonInvarianceEffectSize}

A number of studies describe how to test the effect size of measurement invariance [e.g., @Liu2017a; @Gunn2020; @Newsom2015; @Nye2019] or differential item functioning [e.g., @Meade2010; @Gonzalez2021].\index{structural equation modeling!measurement invariance}\index{effect size}\index{item response theory!differential item functioning}
Indices of effect size of measurement noninvariance include $d_{\text{MACS}}$ [@Nye2019], $w$ [@Newsom2015], and $\Delta \text{M}_\text{C}$ [@Newsom2015].\index{structural equation modeling!measurement invariance}\index{effect size}

$w$ is interpreted such that .1, .3, and .5 indicate a small, medium, and large effect size, respectively.
$w$ is computed as in @eq-measurementNoninvarianceEffectSizeW [@Newsom2015]:\index{structural equation modeling!measurement invariance}\index{effect size}

$$
w = \sqrt{\frac{\Delta \chi^2}{N(\Delta df)}}
$$ {#eq-measurementNoninvarianceEffectSizeW}

McDonald's centrality index ($\text{M}_\text{C}$) is computed as in @eq-measurementNoninvarianceEffectSizeMc [@Newsom2015]:\index{structural equation modeling!measurement invariance}\index{effect size}

$$
\text{M}_\text{C} = \exp\!\left[-\tfrac{1}{2}\,\left(\frac{\chi^{2} - \text{df}}{N - 1}\right)\right]
$$ {#eq-measurementNoninvarianceEffectSizeMc}

Then, $\Delta \text{M}_\text{C}$ is computed as in @eq-measurementNoninvarianceEffectSizeDeltaMc [@Newsom2015]:\index{structural equation modeling!measurement invariance}\index{effect size}

$$
\Delta \text{M}_\text{C} = \text{M}_{\text{C},\text{unconstrained}} - \text{M}_{\text{C},\text{constrained}}
$$ {#eq-measurementNoninvarianceEffectSizeDeltaMc}

### Specify Models {#sec-measurementInvarianceModels}

#### Null Model {#sec-measurementInvarianceNullModelSyntax}
...
#### CFA Model {#sec-measurementInvarianceCFAsyntax}
...
#### Configural Invariance {#sec-measurementInvarianceConfiguralSyntax}
...
```{r}
#| label: configuralInvarianceModel
...
#### Metric ("Weak Factorial") Invariance {#sec-measurementInvarianceMetricSyntax}
...
#### Scalar ("Strong Factorial") Invariance {#sec-measurementInvarianceScalarSyntax}
...
#### Residual ("Strict Factorial") Invariance {#sec-measurementInvarianceResidualSyntax}
...
### Specify the Fit Indices of Interest {#sec-measurementInvarianceFitIndices}
...
### Null Model {#sec-measurementInvarianceNullModel}

#### Fit the Model {#sec-measurementInvarianceNullModelFit}
...
#### Model Summary {#sec-measurementInvarianceNullModelSummary}
...
### Configural Invariance {#sec-measurementInvarianceConfigural}

Specify the same number of factors in each group, and which indicators load on which factors are the same in each group.\index{structural equation modeling!measurement invariance!configural}

#### Model Syntax {#sec-measurementInvarianceConfiguralModelSyntax}

```{r}
#| label: configuralInvarianceModelSyntax1

configuralInvarianceModel <- measEq.syntax(
  configural.model = cfaModel,
  data = HolzingerSwineford1939,
  ID.fac = "std.lv", # may be deprecated to snake_case (std_lv)
  group = "school")
```
...
```{r}
#| label: configuralInvarianceModelSyntax2

configuralInvarianceSyntax <- as.character(configuralInvarianceModel)
```

##### Summary of Model Features {#sec-measurementInvarianceConfiguralModelFeatures}

```{r}
summary(configuralInvarianceModel)
```

##### Model Syntax in Table Form {#sec-measurementInvarianceConfiguralModelTabular}

```{r}
lavaanify(cfaModel, ngroups = 2)
```

#### Fit the Model {#sec-measurementInvarianceConfiguralModelFit}

```{r}
#| label: configuralInvarianceModelFit
...
```{r}
#| label: configuralInvarianceModelFitFull
...
#### Model Summary {#sec-measurementInvarianceConfiguralModelSummary}
...
#### Model Fit {#sec-measurementInvarianceConfiguralModelFitCriteria}
...
#### Effect Size of Non-Invariancee {#sec-measurementInvarianceConfiguralModelEffectSize}
...
The $w$ and $\Delta \text{M}_\text{C}$ indices of the effect size of measurement non-invariance, as described by @Newsom2015, were calculated using the `measurementNoninvarianceEffectSize()` function of the [`petersenlab`](https://github.com/DevPsyLab/petersenlab) package [@R-petersenlab].

```{r}
petersenlab::measurementNoninvarianceEffectSize(
  model_unconstrained = nullModelFit,
  model_constrained = configuralInvarianceModel_fit
)
```

#### Equivalence Test {#sec-measurementInvarianceConfiguralModelEquivalenceTest}
...
#### Permutation Test {#sec-measurementInvarianceConfiguralModelPermutationTest}
...
A non-significant *p*-value indicates that the model does not fit significantly worse than the best-possible fitting model.\index{structural equation modeling!measurement invariance!configural}
...
```{r}
#| label: configuralInvarianceModelPermutation
#| output: false
...
The *p*-values are non-significant, indicating that the model does not fit significantly worse than the best-possible fitting model.\index{structural equation modeling!measurement invariance!configural}
In other words, configural invariance held.\index{structural equation modeling!measurement invariance!configural}

#### Internal Consistency Reliability {#sec-measurementInvarianceReliability}
...
maximalRelia(configuralInvarianceModel_fit)
```

#### Path Diagram {#sec-measurementInvariancePathDiagram}

A path diagram of the model generated using the `semPlot` package [@R-semPlot] is in the figures below ("1" = group 1; "2" = group 2).

```{r}
#| label: fig-measurementInvariancePathDiagram
#| fig-cap: "Configural Invariance Model in Confirmatory Factor Analysis (1 = Group 1; 2 = Group 2)."
#| fig-alt: "Configural Invariance Model in Confirmatory Factor Analysis (1 = Group 1; 2 = Group 2)."
...
### Metric ("Weak Factorial") Invariance Model {#sec-measurementInvarianceMetricModel}

Specify invariance of factor loadings across groups.\index{structural equation modeling!measurement invariance!metric}\index{structural equation modeling!factor loading}

#### Model Syntax {#sec-measurementInvarianceMetricModelSyntax}

```{r}
#| label: metricInvarianceModelSyntax1
#| output: false
...
```{r}
#| label: metricInvarianceModelSyntax2

metricInvarianceSyntax <- as.character(metricInvarianceModel)
```

##### Summary of Model Features {#sec-measurementInvarianceMetricModelFeatures}

```{r}
summary(metricInvarianceModel)
```

#### Model Syntax in Table Form {#sec-measurementInvarianceMetricModelTabular}
...
#### Fit the Model {#sec-measurementInvarianceMetricModelFit}

```{r}
#| label: metricInvarianceModelFit
...
```{r}
#| label: metricInvarianceModelFitFull
...
#### Model Summary {#sec-measurementInvarianceMetricModelOutput}
...
#### Model Fit {#sec-measurementInvarianceMetricModelFitCriteria}
...
#### Compare Model Fit {#sec-measurementInvarianceMetricModelCompare}

##### Nested Model ($\chi^2$) Difference Test {#sec-measurementInvarianceMetricModelNestedModelComparison}
...
##### Compare Other Fit Criteria {#sec-measurementInvarianceMetricModelCompareFitCriteria}

```{r}
round(metricInvarianceModelFitIndices - configuralInvarianceModelFitIndices, digits = 3)
```

##### Score-Based Test {#sec-measurementInvarianceMetricModelScoreBasedTest}
...
##### Effect Size of Non-Invariance {#sec-measurementInvarianceMetricModelEffectSize}

The effect size of measurement non-invariance, as described by @Nye2019, was calculated using the `dmacs` package [@R-dmacs].\index{structural equation modeling!measurement invariance!configural}\index{effect size}

```{r}
lavaan_dmacs(metricInvarianceModel_fit)
```
...
```{r}
petersenlab::measurementNoninvarianceEffectSize(
  model_unconstrained = configuralInvarianceModel_fit,
  model_constrained = metricInvarianceModel_fit
)
```

##### Equivalence Test {#sec-measurementInvarianceMetricModelEquivalenceTest}
...
##### Permutation Test {#sec-measurementInvarianceMetricModelPermutationTest}
...
```{r}
#| label: metricInvarianceModelPermutation
#| output: false
...
The *p*-values are non-significant, indicating that the model does not fit significantly worse than the configural invariance model.\index{structural equation modeling!measurement invariance!metric}
In other words, metric invariance held.\index{structural equation modeling!measurement invariance!metric}

#### Internal Consistency Reliability {#sec-measurementInvarianceMetricModelReliability}
...
maximalRelia(metricInvarianceModel_fit)
```

#### Path Diagram {#sec-measurementInvarianceMetricModelPathDiagram}

A path diagram of the model generated using the `semPlot` package [@R-semPlot] is below.

```{r}
#| label: fig-measurementInvarianceMetricModelPathDiagram
#| fig-cap: "Metric Invariance Model in Confirmatory Factor Analysis."
#| fig-alt: "Metric Invariance Model in Confirmatory Factor Analysis."
...
### Scalar ("Strong Factorial") Invariance Model {#sec-measurementInvarianceScalarModel}

Specify invariance of factor loadings and intercepts across groups.\index{structural equation modeling!measurement invariance!scalar}\index{structural equation modeling!factor loading}\index{structural equation modeling!intercept}

#### Model Syntax {#sec-measurementInvarianceScalarModelSyntax}

```{r}
#| label: scalarInvarianceModelSyntax1
...
```{r}
#| label: scalarInvarianceModelSyntax2

scalarInvarianceSyntax <- as.character(scalarInvarianceModel)
```

##### Summary of Model Features {#sec-measurementInvarianceScalarModelFeatures}

```{r}
summary(scalarInvarianceModel)
```

#### Model Syntax in Table Form {#sec-measurementInvarianceScalarModelTabular}
...
#### Fit the Model {#sec-measurementInvarianceScalarModelFit}

```{r}
#| label: scalarInvarianceModelFit
...
```{r}
#| label: scalarInvarianceModelFitFull
...
#### Model Summary {#sec-measurementInvarianceScalarModelOutput}
...
#### Model Fit {#sec-measurementInvarianceScalarModelFitCriteria}
...
#### Compare Model Fit {#sec-measurementInvarianceScalarModelCompare}

##### Nested Model ($\chi^2$) Difference Test {#sec-measurementInvarianceScalarModelNestedModelComparison}
...
##### Compare Other Fit Criteria {#sec-measurementInvarianceScalarModelCompareFitCriteria}
...
##### Score-Based Test {#sec-measurementInvarianceScalarScoreBasedTest}
...
##### Effect Size of Non-Invariance {#sec-measurementInvarianceScalarModelEffectSize}

The effect size of measurement non-invariance, as described by @Nye2019, was calculated using the `dmacs` package [@R-dmacs].\index{structural equation modeling!measurement invariance!configural}\index{effect size}

```{r}
lavaan_dmacs(scalarInvarianceModel_fit)
```
...
```{r}
petersenlab::measurementNoninvarianceEffectSize(
  model_unconstrained = metricInvarianceModel_fit,
  model_constrained = scalarInvarianceModel_fit
)
```

##### Equivalence Test {#sec-measurementInvarianceScalarModelEquivalenceTest}
...
##### Permutation Test {#sec-measurementInvarianceScalarModelPermutationTest}
...
```{r}
#| label: scalarInvarianceModelPermutation
#| output: false
...
The *p*-values are significant, indicating that the model fit significantly worse than the metric invariance model.\index{structural equation modeling!measurement invariance!scalar}
In other words, scalar invariance failed.\index{structural equation modeling!measurement invariance!scalar}

#### Internal Consistency Reliability {#sec-measurementInvarianceScalarModelReliability}
...
maximalRelia(scalarInvarianceModel_fit)
```

#### Path Diagram {#sec-measurementInvarianceScalarModelPathDiagram}

A path diagram of the model generated using the `semPlot` package [@R-semPlot] is below.

```{r}
#| label: fig-measurementInvarianceScalarModelPathDiagram
#| fig-cap: "Scalar Invariance Model in Confirmatory Factor Analysis."
#| fig-alt: "Scalar Invariance Model in Confirmatory Factor Analysis."
...
### Residual ("Strict Factorial") Invariance Model {#sec-measurementInvarianceResidualModel}

Specify invariance of factor loadings, intercepts, and residuals across groups.\index{structural equation modeling!measurement invariance!residual}\index{structural equation modeling!factor loading}\index{structural equation modeling!intercept}\index{structural equation modeling!residual}

#### Model Syntax {#sec-measurementInvarianceResidualModelSyntax}

```{r}
#| label: residualInvarianceModelSyntax1
...
```{r}
#| label: residualInvarianceModelSyntax2

residualInvarianceSyntax <- as.character(residualInvarianceModel)
```

##### Summary of Model Features {#sec-measurementInvarianceResidualModelFeatures}

```{r}
summary(residualInvarianceModel)
```

#### Model Syntax in Table Form {#sec-measurementInvarianceResidualModelTabular}
...
#### Fit the Model {#sec-measurementInvarianceResidualModelFit}

```{r}
#| label: residualInvarianceModelFit
...
```{r}
#| label: residualInvarianceModelFitFull
...
#### Model Summary {#sec-measurementInvarianceResidualModelOutput}
...
#### Model Fit {#sec-measurementInvarianceResidualModelFitCriteria}
...
#### Compare Model Fit {#sec-measurementInvarianceResidualModelCompare}

##### Nested Model ($\chi^2$) Difference Test {#sec-measurementInvarianceResidualModelNestedModelComparison}
...
##### Compare Other Fit Criteria {#sec-measurementInvarianceResidualModelCompareFitCriteria}

```{r}
round(
  residualInvarianceModelFitIndices - scalarInvarianceModelFitIndices,
  digits = 3)

round(cbind(
  null = inspect(nullModelFit, "fit_measures"),
  configural = inspect(configuralInvarianceModel_fit, "fit_measures"), 
  weak = inspect(metricInvarianceModel_fit, "fit_measures"),
  strong = inspect(scalarInvarianceModel_fit, "fit_measures"),
  strict = inspect(residualInvarianceModel_fit, "fit_measures")), 3)
```

##### Score-Based Test {#sec-measurementInvarianceResidualModelScoreBasedTest}
...
##### Effect Size of Non-Invariance {#sec-measurementInvarianceResidualModelEffectSize}

The effect size of measurement non-invariance, as described by @Nye2019, was calculated using the `dmacs` package [@R-dmacs].\index{structural equation modeling!measurement invariance!configural}\index{effect size}

```{r}
lavaan_dmacs(residualInvarianceModel_fit)
```
...
```{r}
petersenlab::measurementNoninvarianceEffectSize(
  model_unconstrained = scalarInvarianceModel_fit,
  model_constrained = residualInvarianceModel_fit
)
```

##### Equivalence Test {#sec-measurementInvarianceResidualModelEquivalenceTest}
...
##### Permutation Test {#sec-measurementInvarianceResidualModelPermutationTest}
...
```{r}
#| label: residualInvarianceModelPermutation
#| output: false
...
The *p*-values are non-significant, indicating that the model did not fit significantly worse than the scalar invariance model.\index{structural equation modeling!measurement invariance!residual}\index{chi-square!equivalence test}

#### Internal Consistency Reliability {#sec-measurementInvarianceResidualModelReliability}
...
maximalRelia(residualInvarianceModel_fit)
```

#### Path Diagram {#sec-measurementInvarianceResidualModelPathDiagram}

A path diagram of the model generated using the `semPlot` package [@R-semPlot] is below.

```{r}
#| label: fig-measurementInvarianceResidualModelPathDiagram
#| fig-cap: "Residual Invariance Model in Confirmatory Factor Analysis."
#| fig-alt: "Residual Invariance Model in Confirmatory Factor Analysis."
...
### Addressing Measurement Non-Invariance {#sec-addressingMeasurementNonInvariance}
...
#### Iteratively add constraints to identify measurement non-invariance {#sec-measurementInvarianceAddConstraints}
...
##### Variable 1 {#sec-measurementInvarianceAddConstraintsVariable1}
...
##### Variable 2 {#sec-measurementInvarianceAddConstraintsVariable2}
...
##### Variable 3 {#sec-measurementInvarianceAddConstraintsVariable3}
...
##### Variable 4 {#sec-measurementInvarianceAddConstraintsVariable4}
...
##### Variable 5 {#sec-measurementInvarianceAddConstraintsVariable5}
...
##### Variable 6 {#sec-measurementInvarianceAddConstraintsVariable6}
...
##### Variable 7 {#sec-measurementInvarianceAddConstraintsVariable7}
...
##### Variable 8 {#sec-measurementInvarianceAddConstraintsVariable8}
...
##### Variable 9 {#sec-measurementInvarianceAddConstraintsVariable9}
...
##### Summary {#sec-measurementInvarianceAddConstraintsSummary}
...
#### Iteratively drop constraints to identify measurement non-invariance {#sec-measurementInvarianceDropConstraints}
...
So, we will use the scalar invariance model as the baseline model (in which item intercepts are constrained to be the same across groups), and we will iteratively drop constraints to identify the items for which measurement invariance becomes established (relative to the metric invariance model which allows item intercepts to differ across groups), and therefore, identifies which item(s) show non-invariant intercepts across groups.\index{structural equation modeling!measurement invariance}\index{structural equation modeling!measurement invariance!scalar}\index{structural equation modeling!intercept}

##### Variable 1 {#sec-measurementInvarianceDropConstraintsVariable1}
...
##### Variable 2 {#sec-measurementInvarianceDropConstraintsVariable2}
...
##### Variable 3 {#sec-measurementInvarianceDropConstraintsVariable3}
...
##### Variable 4 {#sec-measurementInvarianceDropConstraintsVariable4}
...
##### Variable 5 {#sec-measurementInvarianceDropConstraintsVariable5}
...
##### Variable 6 {#sec-measurementInvarianceDropConstraintsVariable6}
...
##### Variable 7 {#sec-measurementInvarianceDropConstraintsVariable7}
...
##### Variable 8 {#sec-measurementInvarianceDropConstraintsVariable8}
...
##### Variable 9 {#sec-measurementInvarianceDropConstraintsVariable9}
...
##### Summary {#sec-measurementInvarianceDropConstraintsSummary}
...
#### Next Steps {#sec-measurementInvarianceNextSteps}
...
Then, we would test subsequent measurement invariance models (e.g., residual invariance) while keeping the partially freed constaints from the partial scalar invariance model.\index{structural equation modeling!measurement invariance}
...
## Conclusion {#sec-conclusion-bias}

Bias is a [systematic error](#sec-systematicError).\index{bias}\index{measurement error!systematic error}
Test bias refers to a [systematic error](#sec-systematicError) (in measurement, prediction, etc.) as a function of group membership.\index{bias}\index{measurement error!systematic error}
The two broad categories of test bias include [predictive bias](#sec-predictiveBias) and [test structure bias](#sec-testStructureBias).\index{bias!predictive}\index{bias!test structure}
[Predictive bias](#sec-predictiveBias) refers to differences between groups in the relation between the test and criterion—i.e., different intercepts and/or slopes of the regression line between the test and the criterion.\index{bias!predictive}
[Test structure bias](#sec-testStructureBias) refers to differences in the internal test characteristics across groups, and can be evaluated with approaches such as [measurement invariance](#sec-measurementInvariance) and [differential item function](#sec-dif).\index{bias!test structure}\index{structural equation modeling!measurement invariance}\index{item response theory!differential item functioning}
When detecting test bias, it is important to address it, and there are a number of ways of [correcting for bias](#sec-correctForBias), including [score adjustment](#sec-scoreAdjustment) and [other approaches](#sec-otherBiasCorrections) that do not involve score adjustment.\index{bias!correcting for}\index{bias!correcting for!score adjustment}
However, even if a test does not show bias, it does not mean that the test is [fair](#sec-fairness).\index{bias}\index{fairness}
[Fairness](#sec-fairness) is not a scientific question but rather a moral, societal, and [ethical](#sec-ethics) question; there are many different ways of [operationalizing fairness](#sec-operationalizingFairness).\index{fairness}\index{ethics}\index{fairness!operationalizing}
[Fairness](#sec-fairness) is a complex question, so do the best you can and try to minimize any negative impact of the assessment procedures.\index{fairness}

## Suggested Readings {#sec-readings-bias}

@Putnick2016; @Cole1981; @Fernandez2018; @Reynolds2012; @Sackett1994; @Jonson2022

## Exercises {#sec-exercises-bias}
...
### Questions {#sec-exercisesQuestions-bias}
...
4. Fit a multi-group [item response theory](#sec-irt) model to the Antisocial subscale at time 2, with the participant's sex as a grouping factor.
...
### Answers {#sec-exercisesAnswers-bias}
...
2. No, the Antisocial scale does not show equal outcomes in terms of [selection rate](#sec-selectionRatio).
...
## Session Info {#sec-sessionInfo-bias}
