...
As an example, our clients complete a brief measure of behavioral issues on a tablet when they arrive and are in the waiting room.\index{computerized assessment}
...
A test characteristic curve of the measure is shown in Figure \@ref(fig:tcc-cat).\index{adaptive testing!computerized}\index{item response theory!test characteristic curve}
...
Item characteristic curves and information curves for Item 30 are shown in Figure \@ref(fig:catICCItem30).\index{adaptive testing!computerized}\index{item response theory!item characteristic curve}\index{item response theory!item information curve}
...
### Run Computerized Adaptive Test {#runCAT}
...
plot(result, SE = 1, main = "")
```

#### CAT 95% Confidence Interval {#cat95CI}

Ninety-Five percent confidence intervals of a person's estimated construct level (theta; $\theta$) as a function of the item (presented in the order that items were administered as part of the CAT) are shown in Figure \@ref(fig:cat95PctCI).\index{adaptive testing!computerized}

(ref:cat95PctCICaption) Computerized Adaptive Test 95% Confidence Interval of Theta.

```{r cat95PctCI, out.width = "100%", fig.align = "center", fig.cap = "(ref:cat95PctCICaption)"}
plot(result, SE = qnorm(.975), main = "")
```
