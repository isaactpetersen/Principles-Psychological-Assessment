...
As noted in Section \@ref(reliabilityVsValidity), [reliability](#reliability) sets the upper bound for [validity](#validity), so unreliable judgments cannot be valid.\index{actuarial}\index{clinical judgment}
...
One situation where clinical judgment would be important is when no actuarial method exists for the judgment or [prediction](#prediction).\index{clinical judgment}\index{actuarial}
...
Findings consistently show that actuarial methods are as [accurate](#validity) or more [accurate](#validity) than clinical [prediction](#prediction) methods [@Dawes1989; @Aegisdottir2006; @Baird2000; @Grove1996; @Grove2000].\index{clinical judgment}\index{actuarial}
...
Thus, actuarial methods have [incremental validity](#incrementalValidity) over clinical judgment.\index{clinical judgment}\index{actuarial}
...
Actuarial methods are especially important to use in low-[validity](#validity) environments (like clinical psychology) in which there is considerable uncertainty and unpredictability [@Kahneman2011].\index{actuarial}
By contrast, for [norm-referenced](#norm) assessment tasks, different approaches may be preferred over actuarial methods and statistical [prediction](#prediction) rules.
For instance, for [norm-referenced](#norm) assessment tasks such as describing personality and psychopathology [@Garb2019], where the criterion is a latent construct (derived from the internal pattern of associations among the items) rather than a specific event or outcome, psychometric methods of scale construction such as [factor analysis](#factorAnalysis) may be preferred [@Garb2019]—i.e., the [inductive approach](#inductiveApproach) to scale construction.\index{factor analysis}\index{norm-referenced}
...
Although LASSO models showed the highest [discrimination accuracy](#discrimination) in the internal sample, they showed the largest shrinkage in the external sample.\index{least absolute shrinkage and selection option}\index{discrimination}\index{shrinkage}
...
Multiple regression and/or prior literature can be used to identify the weights of various [predictors](#prediction).\index{multiple regression}
Even unit-weighted formulas (formulas whose [predictors](#prediction) are equally weighted with a weight of one) can sometimes generalize better to other samples than complex weightings [@Garb2019; @Wainer1976].
...
## Examples of Using Actuarial Methods {#actuarialExamples}
...
Another reason why clinical methods are more widely used than actuarial methods is the over-confidence in clinicians' [predictions](#prediction)—clinicians think they are more accurate than they actually are.\index{clinical judgment}\index{actuarial}\index{over-confidence}
...
Note, however, that it is not just clinicians who are over-confident; humans in general tend to be over-confident in their [predictions](#prediction).\index{clinical judgment}\index{over-confidence}
