...
At the individual level, it is common to infer that a person's level changed from T1 from T2 if the magnitude of their change is greater than the [smallest real difference](#repeatability).\index{change!inferring}\index{reliability!test–retest!coefficient of repeatability}
...
Thus, it is also important to consider the [effect size](#effectSize) of the change and whether the change is greater than the minimal clinically important difference (MCID).\index{change!inferring}\index{effect size}
