...
# Repeated Assessments Across Time {#sec-repeated-assessments}

## Overview of Repeated Measurement {#sec-overview-repeated-assessments}
...
As described in @sec-correlationCausation, there are three primary reasons a correlation between X and Y does not necessarily mean that X causes Y: First, the association could reflect the opposite direction of effect, where Y actually causes X.\index{repeated measurement}\index{correlation!causation}
...
[Mixed-effects modeling](#sec-mixedModels) (i.e., mixed modeling, multilevel modeling, hierarchical linear modeling) and other analyses, such as [structural equation modeling](#sec-sem), can handle the longitudinal dependency or nesting of repeated measures data.\index{repeated measurement}\index{mixed model}\index{structural equation modeling}
...
### Comparison of Approaches {#sec-approaches-repeated-assessments}

Research designs can be compared in terms of their [internal validity](#sec-internalValidity)—the extent to which we can be confident about causal inferences.\index{validity!internal}
A cross-sectional association is depicted in @fig-crossSectional:\index{research design!cross-sectional}

::: {#fig-crossSectional}
![](images/Longitudinal-01.png){width=20% fig-alt="Cross-Sectional Association."}

Cross-Sectional Association.
:::

Among observational/correlational research designs, cross-sectional designs tend to have the weakest [internal validity](#sec-internalValidity).\index{research design!cross-sectional}\index{validity!internal}
...
![](images/Longitudinal-02.png){width=100% fig-alt="Lagged Association."}
...
![](images/Longitudinal-03.png){width=100% fig-alt="Lagged Association, Controlling for Prior Levels of the Outcome."}

Lagged Association, Controlling for Prior Levels of the Outcome.
:::

A lagged association controlling for prior levels of the outcome has better [internal validity](#sec-internalValidity) than a lagged association that does not control for prior levels of the outcome.\index{research design!longitudinal}\index{validity!internal}\index{cross-lagged}
...
![](images/Longitudinal-04.png){width=100% fig-alt="Lagged Association, Controlling for Prior Levels of the Outcome, Simultaneously Testing Both Directions Of Effect."}

Lagged Association, Controlling for Prior Levels of the Outcome, Simultaneously Testing Both Directions Of Effect.
:::

Lagged associations that control for prior levels of the outcome and that simultaneously test each direction of effect provide the strongest [internal validity](#sec-internalValidity) among correlational/observational designs, especially when examining whether within-person changes in the predictor predict later within-person changes in the outcome (and vice versa).\index{research design!longitudinal}\index{validity!internal}\index{cross-lagged}
...
The [cross-lagged panel model](#sec-clpm) and [dual latent change score model](#sec-lcsm) are examples of models that include lagged associations that control for prior levels of the outcome and that simultaneously test each direction of effect.\index{research design!longitudinal}\index{validity!internal}\index{cross-lagged}\index{cross-lagged!cross-lagged model}\index{latent change score model}
...
::: {#fig-crossLaggedPanelModelRandomIntercepts}
![](images/Longitudinal-05.png){width=100% fig-alt="Lagged Association, Controlling for Prior Levels of the Outcome and Random Intercepts, Simultaneously Testing Both Directions Of Effect."}

Lagged Association, Controlling for Prior Levels of the Outcome and Random Intercepts, Simultaneously Testing Both Directions Of Effect.
:::
...
## Examples of Repeated Measurement {#sec-examples-repeated-assessments}

Below are some examples of repeated measurement.\index{repeated measurement}

### Alcohol Timeline Followback {#sec-alcoholTimelineFollowback}
...
The measure provides many summary statistics and [norms](#sec-norm) are available.\index{repeated measurement}\index{norm}
...
Because the measure provides detailed information with [norms](#sec-norm), it can be more useful than global estimates of drinking derived from less structured clinical interviews.\index{repeated measurement}\index{norm}
...
### Ambulatory Assessment {#sec-ambulatory-repeated-assessments}

We discussed [ambulatory assessment](#sec-ambulatory) in @sec-ambulatory.
[Ambulatory assessment](#sec-ambulatory) includes an array of methods used to study people in their natural environment.\index{ambulatory assessment}
There is a proliferation of smartphone apps for [ambulatory assessment](#sec-ambulatory).\index{ambulatory assessment}
And the [ambulatory assessments](#sec-ambulatory) often, because of their temporal resolution, provide repeated measurements.\index{ambulatory assessment}\index{repeated measurement}
...
@Matthews2016 provided an example of how they developed an [ambulatory assessment](#sec-ambulatory) using a smartphone app to assess the dynamic process of bipolar disorder.\index{ambulatory assessment}\index{repeated measurement}
...
## Test Revisions {#sec-testRevision}
...
If a measure was revised to improve its [validity](#sec-validity) or if a measure is not [valid](#sec-validity) at particular ages, it may make sense to use different measures across ages or across time.\index{test revision}\index{validity}
You would not want to use a measure at a given age if it is not [valid](#sec-validity) at that age.\index{test revision}\index{validity}
Likewise, it does not make sense to use an [invalid](#sec-validity) measure at a later time point if a more [valid](#sec-validity) version becomes available, even if it was used at an earlier time point.\index{test revision}\index{validity}
...
If different measures are used across ages or across time, there may be ways to link scores from the different measures to be on the same scale, as described in @sec-developmentalScaling.\index{test revision}\index{linking}
...
## Change and Stability {#sec-changeAndStability}
...
Stability of people's level is evaluated with the [coefficient of repeatability](#sec-repeatability).\index{stability!types of}\index{reliability!test–retest!coefficient of repeatability}
...
Stability of structure is examined using longitudinal [factorial invariance](#sec-measurementInvariance).\index{stability!types of}\index{structural equation modeling!measurement invariance}
...
## Assessing Change {#sec-assessingChange}
...
If a person's score differs between two time points, how do you know that the differences across time reflect a person's true change in their level on the construct rather than [measurement error](#sec-measurementError), such as regression to the mean?\index{change!assessing}\index{measurement error}
...
### Inferring Change {#sec-inferringChange}
...
b. the [measurement error](#sec-measurementError) ([unreliability](#sec-reliability)) at both time points is small\index{change!inferring}\index{measurement error}
c. the measure has the same meaning and is on a comparable scale at each time point (i.e., the measure's scores show [measurement invariance](#sec-measurementInvariance) across time)\index{change!inferring}\index{structural equation modeling!measurement invariance}
d. evidence suggests that the differences across time are not likely to be due to [potential confounds of change](#sec-potentialConfoundsOfChange) such as [practice effects](#sec-practiceEffects), [cohort effects](#sec-cohortEffects), and [time-of-measurement effects](#sec-timeOfMeasurementEffects).\index{change!inferring}\index{change!confounds of}\index{practice effect}\index{cohort effect}\index{time-of-measurement effect}

[Measurement error](#sec-measurementError) can be reduced by combining multiple measures of a construct into a latent variable, such as in [structural equation modeling](#sec-sem) (SEM) or [item response theory](#sec-irt) (IRT).\index{change!inferring}\index{measurement error}\index{latent variable}\index{structural equation modeling}\index{item response theory}\index{aggregation}
...
At the individual level, it is common to infer that a person's level changed from T1 from T2 if the magnitude of their change is greater than the [smallest real difference](#sec-repeatability).\index{change!inferring}\index{reliability!test–retest!coefficient of repeatability}
However, just because a person (or a group) changed in their level on the construct does not necessarily mean that the change is meaningful.\index{change!inferring}
Thus, it is also important to consider the [effect size](#sec-effectSize) of the change and whether the change is greater than the minimal clinically important difference (MCID).\index{change!inferring}\index{effect size}

### Potential Confounds of Change {#sec-potentialConfoundsOfChange}

Potential confounds of change include [practice effects](#sec-practiceEffects), [cohort effects](#sec-cohortEffects), and [time-of-measurement effects](#sec-timeOfMeasurementEffects).\index{change!inferring}\index{change!confounds of}\index{practice effect}\index{cohort effect}\index{time-of-measurement effect}

#### Practice Effects {#sec-practiceEffects}
...
#### Cohort Effects {#sec-cohortEffects}
...
#### Time-of-Measurement Effects {#sec-timeOfMeasurementEffects}
...
### When and How Frequently to Assess {#sec-whenAndHowFrequentlyToAssess}
...
If the measurement intervals are too frequent, differences might reflect only [measurement error](#sec-measurementError).\index{change!inferring}\index{measurement error}
...
### Difference Scores {#sec-changeScores}

As described in @sec-differenceScore-reliability, [difference scores](#sec-differenceScores) (also called change scores) can have limitations because they can be less [reliable](#sec-reliability) than each of the individual measures that compose the difference score.\index{difference score!reliability}
The [reliability](#sec-reliability) of individual differences in [change scores](#sec-differenceScores) depends on the extent to which: (a) the measures used to calculate the [change scores](#sec-differenceScores) are reliable, and (b) the variability of true individual differences in change is large [@Rogosa1983]—that is, there are true individual differences in change.\index{difference score!reliability}

#### Superior Approaches to Difference Scores {#sec-superiorApproachesToDifferenceScores}

There are several options that are often better than traditional [difference scores](#sec-differenceScores) or change scores.\index{difference score}
Instead of difference scores, use [residualized change](#sec-residualizedChange) (e.g., [autoregressive cross-lagged models](#sec-clpm)), [latent change score models](#sec-lcsm), or [growth curve models](#sec-gcm).\index{difference score}\index{cross-lagged!cross-lagged model}\index{growth curve model}\index{latent change score model}

#### Residualized Change {#sec-residualizedChange}

One alternative to using difference scores is to examine residualized change.
Residualized change means regressing the score at timepoint 2 (i.e., the outcome variable) on the score at timepoint 1 (i.e., the predictor variable) and saving the residual.
Examining residualized change is equivalent to examining difference scores when the unstandardized regression coefficient of the score at timepoint 1 in predicting the score at timepoint 2 is 1.0—i.e., for every one-unit increase in the score at timepoint 1, there is a one-unit increase in the score at timepoint 2.
An example of residualized change is the [autoregressive cross-lagged panel model](#sec-clpm).

##### Autoregressive Cross-Lagged Models {#sec-clpm}

Autoregressive cross-lagged models (aka cross-lagged panel models) can be fit in [path analysis](#sec-pathAnalysis-sem) or [SEM](#sec-sem).\index{cross-lagged!cross-lagged model}\index{path analysis}\index{structural equation modeling}
In a cross-lagged panel model, the predictor at timepoint 1 is allowed to predict the outcome at timepoint 2 while controlling for prior levels of the outcome at timepoint 1.\index{cross-lagged!cross-lagged model}
...
The traditional cross-lagged panel model tests whether individuals with low levels of X *relative to others* experience subsequent *rank-order increases* in Y [@Orth2021; @Evans2023].\index{cross-lagged!cross-lagged model}
To disaggregate within-person from between-person effects, it is recommended to include the estimation of random intercepts, a so-called random-intercepts cross-lagged panel model [@Hamaker2015].\index{cross-lagged!cross-lagged model}
The random-intercepts cross-lagged panel model tests whether, when individuals experience *lower-than-their-own usual levels* of X, they experience subsequent increases in Y *relative to their usual level* [@Orth2021; @Evans2023].\index{cross-lagged!cross-lagged model}
...
For tests of absolute change, models such as [change score models](#sec-lcsm) or [growth curve models](#sec-gcm) are necessary.\index{cross-lagged!cross-lagged model}\index{latent change score model}\index{growth curve model}

##### Latent Change Score Models {#sec-lcsm}

Latent change score models can be fit in [SEM](#sec-sem).\index{latent change score model}
...
Latent variables are theoretically error-free (at least of random error), so latent change scores can be perfectly [reliable](#sec-reliability), unlike traditional difference scores, whose [reliability](#sec-reliability) is strongly influenced by the [measurement unreliability](#sec-reliability) of the specific measures that compose them.\index{latent change score model}\index{latent variable}\index{measurement error}\index{measurement error!random error}
Latent change score models are described by @Kievit2018.\index{latent change score model}

##### Growth Curve Models {#sec-gcm}

Growth curve models can be fit in [mixed-effects modeling](#sec-mixedModels) or [SEM](#sec-sem).\index{growth curve model}\index{structural equation modeling}\index{mixed model}
Growth curve models require three or more time points to be able to account for [measurement error](#sec-measurementError) in terms of the shape of a linear trajectory.\index{growth curve model}\index{measurement error}
...
Growth curve models correct for [measurement error](#sec-measurementError) in the shape of the person's trajectory.\index{growth curve model}\index{measurement error}
...
### Nomothetic Versus Idiographic {#sec-nomotheticIdiographicApproaches}
...
### Structural Equation Modeling {#sec-sem-repeated-measures}

[Structural equation modeling](#sec-sem) (SEM) allows multiple dependent variables.\index{structural equation modeling}
...
If examining changes in two constructs in the context of [SEM](#sec-sem), you could examine for instance whether one precedes the other in prediction, to better understand which might cause which and to help tease apart the chicken and the egg.\index{structural equation modeling}\index{cross-lagged}
...
### Attrition and Systematic Missingness {#sec-attrition-repeated-measures}
...
It can be helpful to use full-information maximum likelihood (FIML) in [SEM](#sec-sem) to make use of all available data.\index{full information maximum likelihood}\index{missing data}
...
You can help account for [practice effects](#sec-practiceEffects) with planned missingness designs in which participants are randomly assigned to receive or not receive a given measurement occasion.\index{practice effect}\index{missing data!planned}

### Measurement Invariance {#sec-measurementInvariance-repeated-measures}

Just like [measurement or factorial invariance](#sec-measurementInvariance) can be tested and established across genders, races, ethnicities, cultures, etc. to ensure measurement equivalence, it can also be tested across ages or times.\index{culture}\index{structural equation modeling!measurement invariance}
The goal is to establish longitudinal [measurement invariance](#sec-measurementInvariance), i.e., [measurement invariance](#sec-measurementInvariance) across ages, so that you know that you are assessing the same construct over time in a comparable way so that you know that differences across time reflect true change.\index{structural equation modeling!measurement invariance}\index{change!inferring}
To examine longitudinal [measurement invariance](#sec-measurementInvariance), you can use [SEM](#sec-sem) or [IRT](#sec-irt).\index{structural equation modeling!measurement invariance}\index{item response theory!differential item functioning}
In [SEM](#sec-sem), you could examine whether measures have the same intercepts, factor loadings, and residuals across ages.\index{structural equation modeling!measurement invariance}\index{structural equation modeling!intercept}\index{structural equation modeling!factor loading}\index{structural equation modeling!residual}
In [IRT](#sec-irt), you could examine whether items show [differential item functioning](#sec-dif), versus whether they show the same [difficulty](#sec-itemDifficulty) and [discrimination](#sec-itemDiscrimination) across ages.\index{item response theory!differential item functioning}\index{item response theory!item difficulty}\index{item response theory!item discrimination}

But there are times when we would expect a construct would, theoretically, fail to show longitudinal [factorial invariance](#sec-measurementInvariance), as in the case of heterotypic continuity, described in @sec-heterotypicContinuity.\index{structural equation modeling!measurement invariance}\index{heterotypic continuity}

## Types of Research Designs {#sec-researchDesigns}
...
- [cross-sectional](#sec-crossSectional)\index{research design!types}\index{research design!cross-sectional}
- [cross-sectional sequences](#sec-crossSectionalSequences)\index{research design!types}\index{research design!cross-sectional sequences}
- [time-lag](#sec-timeLag)\index{research design!types}\index{research design!time-lag}
- [longitudinal](#sec-longitudinal)\index{research design!types}\index{research design!longitudinal}
- [longitudinal sequences](#sec-longitudinalSequences)\index{research design!types}\index{research design!longitudinal sequences}
    - [time-sequential](#sec-timeSequential)\index{research design!types}\index{research design!time-sequential}\index{research design!longitudinal sequences}
    - [cross-sequential](#sec-crossSequential)\index{research design!types}\index{research design!cross-sequential}\index{research design!longitudinal sequences}
    - [cohort-sequential](#sec-cohortSequential)\index{research design!types}\index{research design!cohort-sequential}\index{research design!longitudinal sequences}

These research designs are depicted by age and cohort in @fig-longitudinalDesigns1 and by time of measurement and cohort in @fig-longitudinalDesigns2.\index{research design!types}\index{research design!cross-sectional}\index{research design!cross-sectional sequences}\index{research design!time-lag}\index{research design!longitudinal}\index{research design!longitudinal sequences}\index{research design!time-sequential}\index{research design!cross-sequential}\index{research design!cohort-sequential}
...
::: {#fig-longitudinalDesigns1}
![](images/longitudinalDesigns_1.png){width=100% fig-alt="Research Designs by Age and Cohort. Values in the cells are the times (years) of measurement. Dashed line indicates different participants were assessed at each time of measurement."}

Research Designs by Age and Cohort. Values in the cells are the times (years) of measurement. Dashed line indicates different participants were assessed at each time of measurement.
:::

::: {#fig-longitudinalDesigns2}
![](images/longitudinalDesigns_2.png){width=100% fig-alt="Research Designs by Time Of Measurement and Cohort. Values in the cells are ages of the participants. Dashed line indicates different participants were assessed at each time of measurement."}

Research Designs by Time Of Measurement and Cohort. Values in the cells are ages of the participants. Dashed line indicates different participants were assessed at each time of measurement.
:::

### Cross-Sectional Design {#sec-crossSectional}
...
Thus, observed age-related differences could reflect [cohort differences](#sec-cohortEffects) rather than true age differences.\index{research design!types}\index{research design!cross-sectional}\index{cohort effect}
...
Any age-related differences are confounded with [cohort differences](#sec-cohortEffects) and are strongly influenced by between age-group sampling variability.\index{research design!types}\index{research design!cross-sectional}\index{cohort effect}
Cross-sectional studies are useful for preliminary data on [validity](#sec-validity) of measures for the age groups, and for examining whether the age differences are in the expected direction.\index{research design!types}\index{research design!cross-sectional}\index{validity}
...
### Cross-Sectional Sequences Design {#sec-crossSectionalSequences}
...
Thus, observed age-related differences could reflect [cohort differences](#sec-cohortEffects) or [time-of-measurement differences](#sec-timeOfMeasurementEffects) rather than true age differences.\index{research design!types}\index{research design!cross-sectional sequences}\index{cohort effect}\index{time-of-measurement effect}
This poses challenges for using cross-sectional sequences to estimate age-related changes due to [time-related effects](#sec-timeOfMeasurementEffects) such as the Flynn effect.\index{research design!types}\index{research design!cross-sectional sequences}

### Time-Lag Design {#sec-timeLag}
...
Thus, any cohort differences could reflect [different times of measurement](#sec-timeOfMeasurementEffects).\index{research design!types}\index{research design!time-lag}\index{time-of-measurement effect}
...
Thus, any observed changes with age could be due to [time-of-measurement effects](#sec-timeOfMeasurementEffects) or [cohort effects](#sec-cohortEffects) (instead of people's true change).\index{research design!types}\index{research design!longitudinal}\index{time-of-measurement effect}\index{cohort effect}

### Longitudinal Sequences Design {#sec-longitudinalSequences}
...
There are three types of longitudinal sequences designs: [time-sequential](#sec-timeSequential), [cross-sequential](#sec-crossSequential), and [cohort-sequential](#sec-cohortSequential) designs.\index{research design!types}\index{research design!longitudinal sequences}\index{research design!time-sequential}\index{research design!cross-sequential}\index{research design!cohort-sequential}
The three types of longitudinal sequences are depicted in @fig-longitudinalSequences as a function of which two factors are specified by the researcher.\index{research design!types}\index{research design!longitudinal sequences}\index{research design!time-sequential}\index{research design!cross-sequential}\index{research design!cohort-sequential}

::: {#fig-longitudinalSequences}
![](images/longitudinalSequences.png){width=100% fig-alt="Types of Longitudinal Sequences as a Function of Which Two Factors are Specified by the Researcher."}

Types of Longitudinal Sequences as a Function of Which Two Factors are Specified by the Researcher.
:::

#### Time-Sequential Design {#sec-timeSequential}

A *time-sequential* design is depicted in @fig-timeSequential.\index{research design!types}\index{research design!longitudinal sequences}\index{research design!time-sequential}
...
A time-sequential design can be helpful for identifying age differences while controlling for [time-of-measurement differences](#sec-timeOfMeasurementEffects), or for identifying [time-of-measurement differences](#sec-timeOfMeasurementEffects) while controlling for age differences.\index{research design!types}\index{research design!longitudinal sequences}\index{research design!time-sequential}
However, [cohort effects](#sec-cohortEffects) are confounded with the interaction of age and time of measurement [@Whitbourne2019].\index{research design!types}\index{research design!longitudinal sequences}\index{research design!time-sequential}\index{cohort effect}\index{time-of-measurement effect}
Thus, observed differences as a function of age or time of measurement could reflect [cohort effects](#sec-cohortEffects).\index{research design!types}\index{research design!longitudinal sequences}\index{research design!time-sequential}\index{cohort effect}

::: {#fig-timeSequential}
![](images/timeSequential.png){width=100% fig-alt="Time-Sequential Research Design."}

Time-Sequential Research Design.
:::

#### Cross-Sequential Design {#sec-crossSequential}

A *cross-sequential* design is depicted in @fig-crossSequential.\index{research design!types}\index{research design!longitudinal sequences}\index{research design!cross-sequential}
...
A cross-sequential design can be helpful for identifying [cohort differences](#sec-cohortEffects) while controlling for [time-of-measurement differences](#sec-timeOfMeasurementEffects), or for identifying [time-of-measurement differences](#sec-timeOfMeasurementEffects) while controlling for [cohort differences](#sec-cohortEffects).\index{research design!types}\index{research design!longitudinal sequences}\index{research design!cross-sequential}\index{cohort effect}\index{time-of-measurement effect}
...
::: {#fig-crossSequential}
![](images/crossSequential.png){width=100% fig-alt="Cross-Sequential Research Design."}

Cross-Sequential Research Design.
:::

#### Cohort-Sequential Design {#sec-cohortSequential}

A *cohort-sequential* design is depicted in @fig-cohortSequential.\index{research design!types}\index{research design!longitudinal sequences}\index{research design!cohort-sequential}
...
A cohort-sequential design can be helpful for identifying age differences while controlling for [cohort differences](#sec-cohortEffects), or for identifying [cohort differences](#sec-cohortEffects) while controlling for age differences.\index{research design!types}\index{research design!longitudinal sequences}\index{research design!cohort-sequential}\index{cohort effect}
However, [time-of-measurement effects](#sec-timeOfMeasurementEffects) are confounded with the interaction of age and cohort [@Whitbourne2019].\index{research design!types}\index{research design!longitudinal sequences}\index{research design!cohort-sequential}\index{time-of-measurement effect}
Thus, observed differences as a function of age or cohort could reflect [time-of-measurement effects](#sec-timeOfMeasurementEffects).\index{research design!types}\index{research design!longitudinal sequences}\index{research design!cohort-sequential}\index{time-of-measurement effect}

::: {#fig-cohortSequential}
![](images/cohortSequential.png){width=100% fig-alt="Cohort-Sequential Research Design."}

Cohort-Sequential Research Design.
:::

## Using Sequential Designs To Make Developmental Inferences {#sec-usingSequentialDesigns}
...
However, age is not the only metric for development; time after a transition (e.g., school entry, quitting smoking) or an exposure (e.g., puberty onset) can also be an important time metric.\index{research design!types}\index{research design!longitudinal sequences}
...
However, that is not often practical.
...
To the extent that the age-related effects in the time-sequential and cohort-sequential designs are stronger than the [cohort-](#sec-cohortEffects) and [time-of-measurement](#sec-timeOfMeasurementEffects) effects in the cross-sequential design, we have confidence that the observed age-related differences reflect development [@Whitbourne2019].\index{research design!types}\index{research design!longitudinal sequences}\index{cohort effect}\index{time-of-measurement effect}
In addition, there may be ways in a given study of accounting for period or cohort effects.
For instance, in an accelerated longitudinal study (cross-sequential design), it would be valuable to evaluate the extent to which the differing cohorts are comparable—i.e., that they show similar scores at the same ages.
...
@Rohrer2025 provides additional suggestions for considering age, period, and cohort effects.\index{research design!types}\index{research design!longitudinal sequences}

## Stability Versus Continuity {#sec-stabilityVsContinuity}

Continuity addresses the trajectory of development *within* individuals.
By contrast, stability refers to the maintenance of *between*-individual or group characteristics [@Petersen2024b; @Schulenberg2006; @Schulenberg2014].
When describing stability or continuity, it is important to specify which aspects of stability (e.g., of mean, variance, or rank order) and continuity (e.g., of causes, courses, forms, functions, or environments) one is referring to.
For example, @fig-meanVsRankOrderStability depicts the distinction between mean-level stability and rank-order stability; @fig-courseContinuityVsRankOrderStability depicts the distinction between continuity in course and rank-order stability.

::: {#fig-meanVsRankOrderStability}
![](images/meanVsRankOrderStability.png){width=100% fig-alt="Mean-Level Stability Versus Rank-Order Stability."}

Mean-Level Stability Versus Rank-Order Stability. C1, C2, and C3 are three individual children assessed at two timepoints: T1 and T2. The gray arrow depicts the mean (average) trajectory. (Figure reprinted from @Petersen2024b, Figure 1, Petersen, I. T. (2024). Reexamining developmental continuity and discontinuity in the 21st century: Better aligning behaviors, functions, and mechanisms. *Developmental Psychology*, *60*(11), 1992–2007. <https://doi.org/10.1037/dev0001657> Copyright (c) American Psychological Association. Used with permission.)
:::

::: {#fig-courseContinuityVsRankOrderStability}
![](images/courseContinuityVsRankOrderStability.png){width=100% fig-alt="Continuity in Course Versus Rank-Order Stability. C1, C2, and C3 are three individual children assessed at two timepoints: T1 and T2. Continuity in course is sometimes called "stability in level." (Figure reprinted from @Petersen2024b, Figure 2, Petersen, I. T. (2024). Reexamining developmental continuity and discontinuity in the 21st century: Better aligning behaviors, functions, and mechanisms. *Developmental Psychology*, *60*(11), 1992–2007. <https://doi.org/10.1037/dev0001657> Copyright (c) American Psychological Association. Used with permission.)"}

Continuity in Course Versus Rank-Order Stability. C1, C2, and C3 are three individual children assessed at two timepoints: T1 and T2. Continuity in course is sometimes called "stability in level." (Figure reprinted from @Petersen2024b, Figure 2, Petersen, I. T. (2024). Reexamining developmental continuity and discontinuity in the 21st century: Better aligning behaviors, functions, and mechanisms. *Developmental Psychology*, *60*(11), 1992–2007. <https://doi.org/10.1037/dev0001657> Copyright (c) American Psychological Association. Used with permission.)
:::

The various types of continuity, including homotypic continuity, heterotypic continuity, and phenotypic continuity, in addition to discontinuity are depicted in Figures [-@fig-continuityLatinSquare] and [-@fig-heterotypicContinuity].\index{heterotypic continuity}

::: {#fig-continuityLatinSquare}
![](images/continuityLatinSquare.png){width=100% fig-alt="The Latin square depicts Kagan's -@Kagan1969 typology of continuity as a function of behavior form and function. However, it is also important to consider continuity versus discontinuity of the mechanisms underlying a given behavior–function pair. (Figure reprinted from @Petersen2024b, Figure 3, Petersen, I. T. (2024). Reexamining developmental continuity and discontinuity in the 21st century: Better aligning behaviors, functions, and mechanisms. *Developmental Psychology*, *60*(11), 1992–2007. <https://doi.org/10.1037/dev0001657> Copyright (c) American Psychological Association. Used with permission.)"}

The Latin square depicts Kagan's -@Kagan1969 typology of continuity as a function of behavior form and function. However, it is also important to consider continuity versus discontinuity of the mechanisms underlying a given behavior–function pair. (Figure reprinted from @Petersen2024b, Figure 3, Petersen, I. T. (2024). Reexamining developmental continuity and discontinuity in the 21st century: Better aligning behaviors, functions, and mechanisms. *Developmental Psychology*, *60*(11), 1992–2007. <https://doi.org/10.1037/dev0001657> Copyright (c) American Psychological Association. Used with permission.)
:::

::: {#fig-heterotypicContinuity}
![](images/heterotypicContinuity.png){width=100% fig-alt="Three Types of Continuity in Addition to Discontinuity in the Form of a 2 (Behavioral Manifestation, Underlying Processes) x 2 (Same Versus Different Across Time) Latin Square. 'T1,' 'T2,' and 'T3' reflect time points 1, 2, and 3, respectively. The illustrations above the lines are buildings, representing the surface structure (i.e., behavioral manifestation). The illustrations below the lines depict the underlying processes supporting the buildings at each time point. The squares on the buildings are windows. The black windows represent content facets that are active across all time points (i.e., age-common content). The windows that contain X's represent content facets that are active at some but not all time points (i.e., age-unique content). The white windows represent content facets that are inactive, and therefore are not part of the construct at that time point. The increasing size of the buildings at later time points reflects growth with development. The top row of the Latin square involves the same underlying processes across time, whereas the bottom row involves different underlying proccesses across time. The left column of the Latin square involves the same behavioral manifestation across time, whereas the right column involves a different behavioral manifestation across time. Homotypic continuity (top left) describes the same behaviorial manifestation with the same underlying process (i.e., construct) across development. Heterotypic continuity (top right) describes the same underlying process with a different behaviorial manifestation across development. Phenotypic continuity (or functional discontinuity) (bottom left) describes the same behavior with different underlying processes across development. Discontinuity (bottom right) describes different behavioral manifestations with different underlying processes across development. Thus, in both homotypic continuity and heterotypic continuity, the active content facets reflect the same construct or underlying process across time, whereas in phenotypic continuity and discontinuity, the active content facets do not reflect the same construct across time. (Figure reprinted from @Petersen2020, Figure 1, p. 2. Petersen, I. T., Choe, D. E., & LeBeau, B. (2020). Studying a moving target in development: The challenge and opportunity of heterotypic continuity. *Developmental Review*, *58*, 100935. <https://doi.org/10.1016/j.dr.2020.100935> Copyright (c) Elsevier. Used with permission.)"}

Three Types of Continuity in Addition to Discontinuity in the Form of a 2 (Behavioral Manifestation, Underlying Processes) x 2 (Same Versus Different Across Time) Latin Square. 'T1,' 'T2,' and 'T3' reflect time points 1, 2, and 3, respectively. The illustrations above the lines are buildings, representing the surface structure (i.e., behavioral manifestation). The illustrations below the lines depict the underlying processes supporting the buildings at each time point. The squares on the buildings are windows. The black windows represent content facets that are active across all time points (i.e., age-common content). The windows that contain X's represent content facets that are active at some but not all time points (i.e., age-unique content). The white windows represent content facets that are inactive, and therefore are not part of the construct at that time point. The increasing size of the buildings at later time points reflects growth with development. The top row of the Latin square involves the same underlying processes across time, whereas the bottom row involves different underlying proccesses across time. The left column of the Latin square involves the same behavioral manifestation across time, whereas the right column involves a different behavioral manifestation across time. Homotypic continuity (top left) describes the same behaviorial manifestation with the same underlying process (i.e., construct) across development. Heterotypic continuity (top right) describes the same underlying process with a different behaviorial manifestation across development. Phenotypic continuity (or functional discontinuity) (bottom left) describes the same behavior with different underlying processes across development. Discontinuity (bottom right) describes different behavioral manifestations with different underlying processes across development. Thus, in both homotypic continuity and heterotypic continuity, the active content facets reflect the same construct or underlying process across time, whereas in phenotypic continuity and discontinuity, the active content facets do not reflect the same construct across time. (Figure reprinted from @Petersen2020, Figure 1, p. 2. Petersen, I. T., Choe, D. E., & LeBeau, B. (2020). Studying a moving target in development: The challenge and opportunity of heterotypic continuity. *Developmental Review*, *58*, 100935. <https://doi.org/10.1016/j.dr.2020.100935> Copyright (c) Elsevier. Used with permission.)
:::

## Heterotypic Continuity {#sec-heterotypicContinuity}

Heterotypic continuity is the persistence of an underlying construct with behavioral manifestations that change across development [@Caspi2006a; @Cicchetti2002; @Petersen2020; @Petersen2024b].\index{heterotypic continuity}
...
### How to Identify Heterotypic Continuity {#sec-identifyingHeterotypicContinuity}
...
The content of the measure can be compared to content of a construct to evaluate the [content validity](#sec-contentValidity) of the measure.\index{heterotypic continuity!identifying}\index{validity!content}

Heterotypic continuity is present to the extent that the content of a construct show cross-time changes in: people's [rank-order stability](#sec-stability), the content's level on the construct, and how strongly the content reflect the construct.\index{heterotypic continuity!identifying}\index{stability!rank-order}
Cross-time changes in people's [rank-order stability](#sec-stability) indicate changes in the degree of stability of individual differences in the construct across time, and are examined with correlation of the content across time.\index{heterotypic continuity!identifying}\index{stability!rank-order}

The content's level on the construct is similar to the inverse of frequency, prevalence, or [base rate](#sec-baseRate) of the content.\index{heterotypic continuity!identifying}\index{base rate}
The content's level on the construct is examined with [item difficulty](#sec-itemDifficulty) (severity) in [IRT](#sec-irt) or with intercepts in [SEM](#sec-sem).\index{heterotypic continuity!identifying}\index{base rate}\index{item response theory!item difficulty}\index{structural equation modeling!intercept}
...
How strongly the content reflect the construct, conceptually is the degree of a content's [construct validity](#sec-constructValidity), and is examined with [item discrimination](#sec-itemDiscrimination) in [IRT](#sec-irt) or with factor loadings in [SEM](#sec-sem).\index{heterotypic continuity!identifying}\index{item response theory!item discrimination}\index{structural equation modeling!factor loading}
For example, the item "hits others" has a higher [item discrimination](#sec-itemDiscrimination) than "eats ice cream" for the construct of externalizing problems because hitting others is more strongly related to the construct (compared to eating ice cream).\index{heterotypic continuity!identifying}\index{item response theory!item discrimination}\index{structural equation modeling!factor loading}
...
For example, inattention shows increases in the magnitude of [rank-order stability](#sec-stability) with age [@Arnett2012].\index{heterotypic continuity}\index{stability!rank-order}
Drinking alcohol becomes more frequent in adolescence compared to childhood, so its [item severity](#sec-itemDiscrimination) decreases.\index{heterotypic continuity}\index{item response theory!item difficulty}
...
In adulthood, disobedience could serve prosocial functions, including protesting against societally unjust actions, and thus shows weaker [construct validity](#sec-constructValidity) for externalizing problems.\index{heterotypic continuity}\index{validity!construct}
...
In sum, many constructs likely show heterotypic continuity.\index{heterotypic continuity}

However, it is important rule out potential alternatives to heterotypic continuity, as depicted in @fig-alternativesToHeterotypicContinuity.

::: {#fig-alternativesToHeterotypicContinuity}
![](images/alternativesToHeterotypicContinuity.png){width=100% fig-alt="Heterotypic Continuity Versus Alternative Developmental Processes. Arrows reflect causal influence. Dotted lines reflect persistence (i.e., continuity) of the same behavior or construct across time. "T1" and "T2" reflect time points 1 and 2, respectively. (Figure reprinted from @Petersen2024b, Figure 6, Petersen, I. T. (2024). Reexamining developmental continuity and discontinuity in the 21st century: Better aligning behaviors, functions, and mechanisms. *Developmental Psychology*, *60*(11), 1992–2007. <https://doi.org/10.1037/dev0001657> Copyright (c) American Psychological Association. Used with permission.)"}

Heterotypic Continuity Versus Alternative Developmental Processes. Arrows reflect causal influence. Dotted lines reflect persistence (i.e., continuity) of the same behavior or construct across time. "T1" and "T2" reflect time points 1 and 2, respectively. (Figure reprinted from @Petersen2024b, Figure 6, Petersen, I. T. (2024). Reexamining developmental continuity and discontinuity in the 21st century: Better aligning behaviors, functions, and mechanisms. *Developmental Psychology*, *60*(11), 1992–2007. <https://doi.org/10.1037/dev0001657> Copyright (c) American Psychological Association. Used with permission.)
:::

### The Problem {#sec-theChallengeOfHeterotypicContinuity}
...
Failing to account for heterotypic continuity leads to misidentified trajectories of people's growth [@Chen2015; @Petersen2026b] and to trajectories that are less able to detect people's change [@Petersen2018a; @Petersen2021].\index{heterotypic continuity}
...
### Ways to Assess a Construct Across Development {#sec-assessingConstructsAcrossDevelopment}
...
A depiction of assessing only the construct-valid content is in @fig-timeDifferingContent.\index{repeated measurement}

::: {#fig-timeDifferingContent}
![](images/timeDifferingContent.png){width=50% fig-alt="Using Only the Construct-Valid Content at Each Age. Content set A corresponds to content that is construct-valid at only T1. Content set B corresponds to content that is construct-valid at both T1 and T2. Content set C corresponds to content that is construct-valid at only T2. The *common content* (content set B) is highlighted in gray. The three approaches to assessing a construct over time are as follows: (1) using *all possible content* across all ages: ABC at T1 and T2, (2) using *only common content* across all ages: B at T1 and T2, or (3) using *only construct-valid content* at each age: AB at T1 and BC at T2. (Figure reprinted from @Petersen2021, Figure 1, p. e3. Petersen, I. T., LeBeau, B., & Choe, D.E. (2021). Creating a developmental scale to account for heterotypic continuity in development: A simulation study. *Child Development*, *92*(1), e1–e19. <https://doi.org/10.1111/cdev.13433> Copyright (c) John Wiley and Sons. Used with permission.)"}

Using Only the Construct-Valid Content at Each Age. Content set A corresponds to content that is construct-valid at only T1. Content set B corresponds to content that is construct-valid at both T1 and T2. Content set C corresponds to content that is construct-valid at only T2. The *common content* (content set B) is highlighted in gray. The three approaches to assessing a construct over time are as follows: (1) using *all possible content* across all ages: ABC at T1 and T2, (2) using *only common content* across all ages: B at T1 and T2, or (3) using *only construct-valid content* at each age: AB at T1 and BC at T2. (Figure reprinted from @Petersen2021, Figure 1, p. e3. Petersen, I. T., LeBeau, B., & Choe, D.E. (2021). Creating a developmental scale to account for heterotypic continuity in development: A simulation study. *Child Development*, *92*(1), e1–e19. <https://doi.org/10.1111/cdev.13433> Copyright (c) John Wiley and Sons. Used with permission.)
:::

#### All Possible Content {#sec-allPossibleContent}
...
And in the case of heterotypic continuity, it assesses content that lack [construct validity](#sec-constructValidity), i.e., there are item intrusions.\index{repeated measurement}\index{heterotypic continuity}\index{validity!construct}

#### Only the Common Content {#sec-onlyCommonContent}
...
However, it also has key disadvantages.\index{repeated measurement}
First, it results in fewer items and a loss of [information](#sec-irtReliability).\index{repeated measurement}\index{item response theory!information}
...
Third, and due to the loss of [information](#sec-irtReliability), it may be less sensitive to developmental change.\index{repeated measurement}\index{item response theory!information}\index{change!sensitivity to}
And in the case of heterotypic continuity, it assesses content that lack [content validity](#sec-contentValidity) because all important facets of the construct are not assessed—i.e., there are item gaps.\index{repeated measurement}\index{validity!content}

#### Only the Construct-Valid Content {#sec-onlyConstructValidContent}

There are disadvantages to using only the [construct-valid](#sec-constructValidity) content.\index{repeated measurement}\index{validity!construct}
...
Second, it retains [content validity](#sec-contentValidity) and [construct](#sec-constructValidity) [validity](#sec-validity).
...
Use of the same measure across ages does not ensure [measurement equivalence](#sec-measurementInvariance).\index{repeated measurement}\index{structural equation modeling!measurement invariance}
The problem is that even if the same measure is used across ages, the measure may lack [construct validity](#sec-constructValidity) at some ages.\index{repeated measurement}\index{validity!construct}
Thus heterotypic continuity may require different measures at different ages to retain [construct validity](#sec-constructValidity).\index{repeated measurement}\index{validity!construct}
...
### Accounting for Heterotypic Continuity {#sec-accountingForHeterotypicContinuity}
...
Specifically, it is recommended to use (only) the [construct-valid](#sec-constructValidity) content at each age when the content is valid for assessing the construct.\index{heterotypic continuity!accounting for}\index{validity!construct}
...
#### Ensuring Statistical Equivalence {#sec-ensuringStatisticalEquivalence}
...
However, as discussed in @sec-testRevision, standardized scores prevent observing individuals' or groups' absolute change over time.\index{norm-referenced!age}\index{data!standardized}
...
##### Average or Proportion Scores {#sec-averageOrProportionScores}
...
However, average and proportion scores assume that measures do not differ in [difficulty](#sec-itemDifficulty) or [discrimination](#sec-itemDiscrimination).\index{heterotypic continuity!accounting for}\index{item response theory!item difficulty}\index{item response theory!item discrimination}
...
##### Developmental Scaling {#sec-developmentalScaling}

A third approach to ensure statistical equivalence of measures is developmental scaling, which is also called vertical scaling (see @fig-ageDifferingContent).\index{heterotypic continuity!accounting for}\index{developmental scaling}
In developmental scaling, measures that differ in [difficulty](#sec-itemDifficulty) and [discrimination](#sec-itemDiscrimination) are placed on the same scale using the common content.\index{heterotypic continuity!accounting for}\index{developmental scaling}
...
The goal is for the measures to have [construct validity](#sec-constructValidity) invariance—the measure should assess the same construct across time.\index{heterotypic continuity!accounting for}\index{validity!construct}
...
The assessed content should reflect the same construct at relevant ages—i.e., the content should show [construct validity](#sec-constructValidity) at each age they are assessed.\index{heterotypic continuity!accounting for}\index{validity!construct}
Second, the content should adequately sample all aspects of construct—i.e., the content should show [content validity](#sec-constructValidity).\index{heterotypic continuity!accounting for}\index{validity!content}
Second, assuming the construct is thought to be relatively stable, the measures should show [test–retest reliability](#sec-testRetest-reliability), at least in the short term.\index{heterotypic continuity!accounting for}\index{reliability!test–retest}
In addition, the measures should show [convergent](#sec-convergentValidity) and [discriminant](#sec-discriminantValidity) validity.\index{heterotypic continuity!accounting for}\index{validity!convergent}\index{validity!discriminant}
Furthermore, the measures assessed should show a similar factor structure across time, based on [factor analysis](#sec-factorAnalysisOverview).\index{heterotypic continuity!accounting for}\index{factor analysis}
In addition, the measures should have high [internal consistency reliability](#sec-internalConsistency-reliability).\index{heterotypic continuity!accounting for}\index{reliability!internal consistency}

### Are There Measures that Can Span the Lifespan? {#sec-measuresThatSpanLifetime}
...
However, I would argue that a single (non-[adaptive](#sec-adaptiveTesting)) measure would not be able to [validly](#sec-validity) assess the same construct across development if the construct shows heterotypic continuity.\index{lifespan assessment}\index{heterotypic continuity}\index{adaptive testing}
...
[Adaptive testing](#sec-adaptiveTesting) may help to achieve this goal.\index{lifespan assessment}\index{adaptive testing}
...
### Why not Just Study Development in a Piecewise Way? {#sec-piecewiseStudyOfDevelopment}
...
Longitudinal studies allow understanding antecedent-consequent effects, better estimation of change than difference scores derived from two time points, and separating age-related from [cohort effects](#sec-cohortEffects).\index{cohort effect}\index{research design!longitudinal}

### Why not Just Study Individual Behaviors? {#sec-studyIndividualBehaviors}
...
Theoretical constructs have better psychometric properties, including better [reliability](#sec-reliability) than individual behaviors due to the principle of aggregation [@Rushton1983].\index{reliability}\index{aggregation}
...
### Summary {#sec-heterotypicContinuitySummary}
...
Use [developmental scaling](#sec-developmentalScaling) approaches to link different measures on the same scale.\index{heterotypic continuity}\index{developmental scaling}
...
## Conclusion {#sec-conclusion-repeated-measures}
...
The inference of change is strengthened to the extent that (a) the magnitude of the difference between the scores at the two time points is large, (b) the [measurement error](#sec-measurementError) at both time points is small, (c) the measure has the same meaning and is on a comparable scale at each time point, (d) the differences across time are not likely to be due to [potential confounds of change](#sec-potentialConfoundsOfChange) such as [practice effects](#sec-practiceEffects), [cohort effects](#sec-cohortEffects), and [time-of-measurement effects](#sec-timeOfMeasurementEffects).\index{repeated measurement}\index{change!inferring}\index{measurement error}\index{change}\index{change!confounds of}\index{practice effect}\index{cohort effect}\index{time-of-measurement effect}
Difference scores, though widely used, can have limitations because they can be less [reliable](#sec-reliability) than each of the individual measures that compose the difference score.\index{repeated measurement}\index{difference score}\index{difference score!reliability}
...
There are a variety of [types of research designs](#sec-researchDesigns) based on combinations of age, period, and cohort: [cross-sectional](#sec-crossSectional), [cross-sectional sequences](#sec-crossSectionalSequences), [time-lag](#sec-timeLag), [longitudinal](#sec-longitudinal), and [longitudinal sequences](#sec-longitudinalSequences), such as [time-sequential](#sec-timeSequential), [cross-sequential](#sec-crossSequential), and [cohort-sequential](#sec-cohortSequential) designs.\index{research design!types}\index{research design!cross-sectional}\index{research design!cross-sectional sequences}\index{research design!time-lag}\index{research design!longitudinal}\index{research design!longitudinal sequences}\index{research design!time-sequential}\index{research design!cross-sequential}\index{research design!cohort-sequential}
...
When considering people's change, it is important to consider changes in the behavioral manifestation of constructs, known as [heterotypic continuity](#sec-heterotypicContinuity).\index{heterotypic continuity}
[Heterotypic continuity](#sec-heterotypicContinuity) can be identified as changes in the factor structure (i.e., content) of a construct across time.\index{heterotypic continuity}
...
Use [developmental scaling](#sec-developmentalScaling) to link different measures on the same scale.\index{heterotypic continuity!accounting for}\index{developmental scaling}
...
## Suggested Readings {#sec-readings-repeated-measures}
