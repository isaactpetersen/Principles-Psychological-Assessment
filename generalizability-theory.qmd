...
Up to this point, we have discussed [reliability](#sec-reliability) from the perspective of classical test theory (CTT).\index{reliability}\index{classical test theory}
However, as we discussed, CTT makes several assumptions that are unrealistic.\index{classical test theory}
For example, CTT assumes that all error is [random](#sec-randomError).\index{classical test theory}\index{measurement error!random error}
...
There are other measurement theories that conceptualize [reliability](#sec-reliability) differently than the way that CTT conceptualizes [reliability](#sec-reliability).\index{classical test theory}\index{reliability}
...
G-theory is also discussed in the chapters on [reliability](#sec-reliability) (@sec-reliability, @sec-gTheoryReliability), [validity](#sec-validity) (@sec-validity, @sec-gTheoryValidity) and [structural equation modeling](#sec-sem) (@sec-sem, @sec-generalizability-SEM).\index{generalizability theory}\index{reliability}\index{validity}\index{structural equation modeling}

## Overview of Generalizability Theory (G-Theory) {#sec-overview-gTheory}
...
### The Universe of Generalizability {#sec-universeGeneralizability}
...
Measures with strong [reliability](#sec-reliability) show a high ratio of variance as a function of the person relative to the variance as a function of other facets or factors.\index{generalizability theory}\index{reliability}
To the extent that variance in scores is attributable to different settings, observers, instruments, occasions, attributes, or other facets, the [reliability](#sec-reliability) of the measure is weakened.\index{generalizability theory}\index{reliability}

### Universe Score {#sec-universeScore}
...
G-theory assumes that the true score does not vary across levels of a facet (e.g., across occasions, or across raters, or across items).

### G-Theory Perspective on Reliability {#sec-reliability-gTheory}

G-theory asserts that the [reliability](#sec-reliability) of a test does not reside within the test itself; a test's [reliability](#sec-reliability) depends on the circumstances under which it is developed, administered, and interpreted.\index{generalizability theory}\index{reliability}
...
By assessing a person in multiple facets of the universe, this allows us to estimate and recognize the magnitude of multiple sources of [measurement error](#sec-measurementError).\index{generalizability theory}\index{generalizability theory!facet}\index{measurement error}
Such [measurement error](#sec-measurementError) includes:\index{measurement error}

- day-to-day variation in performance (stability of the construct, [test–retest reliability](#sec-testRetest-reliability))\index{generalizability theory}\index{generalizability theory!facet}\index{measurement error}\index{reliability!test–retest}
- variance in the item sampling (coefficient of [internal consistency](#sec-internalConsistency-reliability))\index{generalizability theory}\index{generalizability theory!facet}\index{measurement error}\index{reliability!internal consistency}
- variance due to both day-to-day and item sampling (coefficient of equivalence from [parallel-forms reliability](#sec-parallelForms-reliability), or [convergent validity](#sec-convergentValidity), as discussed in @sec-gTheoryValidity)\index{generalizability theory}\index{generalizability theory!facet}\index{measurement error}\index{reliability!parallel forms}\index{validity!convergent}

In G-theory, all sources of [measurement error](#sec-measurementError) (facets) are considered simultaneously—something CTT cannot achieve [@Shavelson1989].\index{generalizability theory}\index{generalizability theory!facet}\index{measurement error}\index{measurement error}
...
This specification allows us to take into consideration variance due to occasion effects, item effects, and occasion $\times$ item effects (i.e., main effects of the facets in addition to their interaction), as in @tbl-gTheoryTable and @fig-gtheoryVennDiagram.\index{generalizability theory}\index{measurement error}\index{measurement error}\index{generalizability theory!facet}

::: {#fig-gtheoryVennDiagram}
![](images/gTheory.png){width=75% fig-alt="Venn Diagram of Variance Components from a Generalizability Theory Model with Person, Item, and Occasion Facets. e refers to the residual. Adapted from <https://wmmurrah.github.io/AdvancedMeasurementTheoryNotebook/generalizability.html> (archived at <https://perma.cc/2EXS-R9MN>)."}

Venn Diagram of Variance Components from a Generalizability Theory Model with Person, Item, and Occasion Facets. e refers to the residual. Adapted from <https://wmmurrah.github.io/AdvancedMeasurementTheoryNotebook/generalizability.html> (archived at <https://perma.cc/2EXS-R9MN>).
:::
...
```{r}
#| label: tbl-gTheoryTable
#| tbl-cap: "Generalizability Theory Table. Percent of Variance from Different Sources in Generalizability Theory Model With Three Facets: Person, Item, and Occasion (and Their Interactions). (Adapted from @Webb2005, Table 1, p. 2. Webb, N. M., & Shavelson, R. J. (2005). Generalizability theory: Overview. In B. S. Everitt & D. C. Howell (Eds.), *Encyclopedia of statistics in behavioral science* (Vol. 2, pp. 717–719). John Wiley & Sons, Ltd. <https://doi.org/10.1002/0470013192.bsa703)>"
#| echo: false
...
The G-Theory equivalent of the CTT [reliability](#sec-reliability) coefficient of a measure is the generalizability coefficient or dependability coefficient.\index{generalizability theory}\index{generalizability theory!generalizability coefficient}\index{generalizability theory!dependability coefficient}

### G-Theory Perspective on Validity {#sec-validity-gTheory}

G-theory can simultaneously consider multiple aspects of [reliability](#sec-reliability) and [validity](#sec-validity) in the same model.\index{generalizability theory}\index{reliability}\index{validity}
For instance, [internal consistency reliability](#sec-internalConsistency-reliability), [test–retest reliability](#sec-testRetest-reliability), [inter-rater reliability](#sec-interrater-reliability), [parallel-forms reliability](#sec-parallelForms-reliability), and [convergent validity](#sec-convergentValidity) [in the @Campbell1959 sense of the same construct assessed by a different method] can all be incorporated into a G-theory model.\index{generalizability theory}\index{reliability!internal consistency}\index{reliability!test–retest}\index{reliability!parallel forms}\index{validity!convergent}
...
- time: e.g., T1 and T2 ([test–retest reliability](#sec-testRetest-reliability))\index{generalizability theory}\index{generalizability theory!facet}\index{reliability!test–retest}
- items: e.g., questions within the same instrument ([internal consistency reliability](#sec-internalConsistency-reliability)) and questions across different instruments ([parallel-forms reliability](#sec-parallelForms-reliability))\index{generalizability theory}\index{generalizability theory!facet}\index{reliability!internal consistency}\index{reliability!parallel forms}
- rater: e.g., self-report and other-report ([inter-rater reliability](#sec-interrater-reliability))\index{generalizability theory}\index{generalizability theory!facet}\index{reliability!inter-rater}
- method: e.g., questionnaire and observation ([convergent validity](#sec-convergentValidity))\index{generalizability theory}\index{generalizability theory!facet}\index{validity!convergent}
...
Measures with strong [convergent validity](#sec-convergentValidity) show a high ratio of variance as a function of the person relative to the variance as a function of measurement method.\index{generalizability theory}\index{validity!convergent}
To the extent that variance in scores is attributable to different measurement methods, [convergent validity](#sec-convergentValidity) is weakened.\index{generalizability theory}\index{validity!convergent}

An example data structure that could leverage G-theory to partition the variance in scores as a function of different facets (person, time, item, rater, method) and their interactions is in @tbl-gtheoryDataStructure.\index{generalizability theory}\index{generalizability theory!facet}
...
: Example Data Structure for Generalizability Theory With the Following Facets: Person, Time, Item, Rater, Method. {#tbl-gtheoryDataStructure}

In sum, G-theory can be a useful way of estimating the degree of [reliability](#sec-reliability) and [validity](#sec-validity) of a measure's scores in the same model.\index{generalizability theory}\index{reliability}\index{validity}

### Generalizability Study {#sec-generalizabilityStudy}

In G-theory, the goal is to conduct a *generalizability study* (G study) and a [*decision study*](#sec-decisionStudy) (D study).\index{generalizability theory}\index{generalizability theory!generalizability study}\index{generalizability theory!decision study}
...
The researcher must specify and define the universe (set of conditions) to which they would like to generalize their observations and in which they would like to study the [reliability](#sec-reliability) of the measure.\index{generalizability theory}\index{generalizability theory!generalizability study}\index{generalizability theory!universe}\index{reliability}
...
### Decision Study {#sec-decisionStudy}

After conducting a [generalizability study](#sec-generalizabilityStudy), one can then use the estimates of the extent of variance in scores that are attributable to various facets (estimated from the [generalizability study](#sec-generalizabilityStudy)) to conduct a decision study (D study).\index{generalizability theory}\index{generalizability theory!generalizability study}\index{generalizability theory!decision study}
...
In G-theory, [reliability](#sec-reliability) is estimated with the [*generalizability coefficient*](#sec-generalizabilityCoefficient) and the [*dependability coefficient*](#sec-dependabilityCoefficient).\index{generalizability theory}\index{generalizability theory!generalizability coefficient}\index{generalizability theory!dependability coefficient}\index{reliability}

### Analysis Approach {#sec-analysis-gTheory}

Traditionally, a generalizability theory approach would test the generalizability study and decision study using a factorial analysis of variance [ANOVA; @Brennan1992], as exemplified in @sec-gTheoryANOVA, (as opposed to simple ANOVA in CTT).\index{generalizability theory}\index{generalizability theory!generalizability study}\index{generalizability theory!decision study}\index{ANOVA}\index{classical test theory}
...
So, it is better to fit G-theory models in a mixed model framework, as exemplified in @sec-gTheoryMixedModel.\index{generalizability theory}\index{mixed model}

### Practical Challenges {#sec-challenges-gTheory}

G-theory is strong theoretically, but it has not been widely implemented.\index{generalizability theory}
G-theory can be challenging because the researcher must specify, define, and assess the universe to which they would like to generalize their observations and to understand the [reliability](#sec-reliability) of the measure.\index{generalizability theory}\index{reliability}
Moreover, G-theory has had limited impact because of its omission of a [latent variable framework](#sec-latentVariableModeling) [@Dumenci2024] (such as in [factor analysis](#sec-factorAnalysis), [structural equation modeling](#sec-sem), and [item response theory](#sec-irt)), which is particularly useful in psychology because the most widely studied constructs in psychology tend to be latent and not directly observable.\index{generalizability theory}\index{latent variable}\index{factor analysis}\index{structural equation modeling}\index{item response theory}
Nevertheless, there are extensions of G-theory to latent variables in [structural equation modeling](#sec-sem), as described in @sec-generalizability-SEM.

## Getting Started {#sec-gettingStarted-gTheory}

### Load Libraries {#sec-loadLibraries-gTheory}
...
### Prepare Data {#sec-prepareData-gTheory}

#### Generate Data {#sec-generateData-gTheory}
...
#### Add Missing Data {#sec-addMissingData-gTheory}
...
#### Combine Data into Dataframe {#sec-combineData-gTheory}
...
### Universe Score for Each Person {#sec-universeScore-example}

Universe scores for each person are generated using the following syntax and are presented in @tbl-universeScores.\index{generalizability theory!universe score}

```{r}
#| label: tbl-universeScores
#| tbl-cap: "Universe Scores."
#| code-fold: true
...
### Generalizability (G) Study {#sec-gStudy-example}

[Generalizability studies](#sec-generalizabilityStudy) can be conducted in an [ANOVA](#sec-gTheoryANOVA) or [mixed model](#sec-gTheoryMixedModel) framework.\index{generalizability theory}\index{generalizability theory!generalizability study}
Below, we fit a [generalizability study](#sec-generalizabilityStudy) model in each framework.\index{generalizability theory}\index{generalizability theory!generalizability study}
...
#### ANOVA Framework {#sec-gTheoryANOVA}
...
#### Mixed Model Framework {#sec-gTheoryMixedModel}
...
### Decision (D) Study {#sec-dStudy-example}

The [decision (D) study](#sec-decisionStudy) and [generalizability (G) study](#sec-generalizabilityStudy), from which the [generalizability](#sec-generalizabilityCoefficient) and [dependability](#sec-dependabilityCoefficient) coefficients can be estimated, were analyzed using the `gtheory` package [@R-gtheory].\index{generalizability theory}\index{generalizability theory!generalizability study}\index{generalizability theory!decision study}\index{generalizability theory!generalizability coefficient}\index{generalizability theory!dependability coefficient}
...
### Generalizability Coefficient {#sec-generalizabilityCoefficient}

The generalizability coefficient is analogous to the [reliability](#sec-reliability) coefficient in CTT.\index{generalizability theory}\index{generalizability theory!generalizability coefficient}\index{reliability}
...
In other words, variance in a [reliable](#sec-reliability) measure should mostly be due to person variance rather than variance as a function of items, occasion, raters, methods, or other factors.\index{generalizability theory}\index{generalizability theory!facet}\index{reliability}
The generalizability coefficient uses relative error variance, so it characterizes the similarity in the relative standing of individuals, similar to CTT-based estimates of relative reliability, such as [Cronbach's alpha](#sec-cronbachAlpha).\index{generalizability theory}\index{generalizability theory!generalizability coefficient}
...
## Conclusion {#sec-conclusion-gTheory}

G-theory provides an important reminder that [reliability](#sec-reliability) is not one thing.\index{generalizability theory}\index{reliability}
You cannot just say that a test "is reliable"; it is important to specify the facets across which the [reliability](#sec-reliability) and [validity](#sec-validity) of a measure have been established (e.g., times, raters, items, groups, instruments).\index{generalizability theory}\index{reliability}\index{validity}\index{generalizability theory!facet}
Generalizability theory can be a useful way of estimating multiple aspects of [reliability](#sec-reliability) and [validity](#sec-validity) of measures in the same model.\index{generalizability theory}\index{reliability}\index{validity}

## Suggested Readings {#sec-readings-gTheory}

@Brennan2001

## Exercises {#sec-exercises-gTheory}
...
### Questions {#sec-exercisesQuestions-gTheory}
...
The data from your study are in @tbl-evaluatingGeneralizabilityTable below:
...
```{r}
#| label: tbl-evaluatingGeneralizabilityTable
#| tbl-cap: "Exercise 12: Table of Example Data for Evaluating Generalizability of Scores Across Participants, Occasions, and Raters."
#| echo: false
...
### Answers {#sec-exercisesAnswers-gTheory}
...
## Session Info {#sec-sessionInfo-gTheory}
