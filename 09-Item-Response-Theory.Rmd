...
```{r}
# Item difficulty
item.diff <- -1 * coef(summary(raschModel_mixed_logit))[, 1] # Regression coefficients * -1
item.diff[1] <- -1 * sum(item.diff[2:5]) # Difficulty of the first item is the sum of all others
...
# Factor Scores (Theta)
raschModel_mixed_logit_theta <- ranef(raschModel_mixed_logit)$ID[, 1]
```
