...
This can be helpful for going back and finding data without losing it in a file cabinet; it is also helpful for compiling data to examine in research.\index{computerized assessment}
...
- Computer-administered and online assessment allow precise timing of stimulus presentation, which is important for many domains including tests of memory.\index{computerized assessment}
...
- When safety issues arise, it can be a liability and ethical problem to have collected data regarding risk/safety (e.g., suicidality/homicidality/abuse/neglect) without promptly having reviewed the responses and taken the necessary steps to address.\index{computerized assessment!ethical challenges}
...
- It may be important to correct for a person's level of experience with computers when using computerized tasks for clinical decision-making [@LeeMeeuwKjoe2021].\index{computerized assessment!best practices}
...
Adaptive testing has also been used with observational assessments [e.g., @Granziol2022].
...
However, that is changing; there are now multidimensional [IRT](#sec-irt) approaches that allow simultaneously estimating people's scores on multiple dimensions, as described in @sec-irt-twoPLmultidimensional.\index{adaptive testing!computerized}\index{item response theory}\index{unidimensional}\index{multidimensional}
...
load(file = "./data/gradedResponseModel.RData")
```
...
## Session Info {#sec-sessionInfo-cat}
