...
## Overview of Validity {#sec-overview-validity}
...
We summarized [reliability](#sec-reliability) with three words: repeatability, consistency, and precision.\index{reliability}\index{reliability!repeatability}\index{reliability!consistency}\index{reliability!precision}
A summary of validity in three words is accuracy, utility, and meaningfulness.\index{validity!overview}\index{validity!accuracy}\index{validity!utility}\index{validity!meaningfulness}
As with [reliability](#sec-reliability), validity is not a property of the measure itself—the validity of a measure's scores reflects an interaction of the properties of the measure with the population for whom it is designed and the sample and context in which it is administered.\index{validity!overview}\index{reliability}
Moreover, validity is tied to the *interpretation* of a measure's scores for the *proposed uses*, not (just) to the measure itself.\index{validity!overview}
...
Thus, as *the Standards* indicate, it is incorrect to use the unqualified phrase "the validity of the measure" or "the measure is (in)valid," because these phrases do not specify which scores were used from the test, what the use is (e.g., predicting whether a person will succeed in a given job), and what interpretation was made of the test scores for this purpose.^[For shorthand and to avoid repetition, we sometimes refer to the validity of the measure (or research design), but in such instances, we are actually referring to the validity of the interpretation of a measure's scores for a given use (or of the inferences from a particular research design).]\index{validity!overview}\index{Standards for Educational and Psychological Testing}
For these reasons, it is insufficient to cite evidence for the validity of the measure in other work; it is also important to evaluate the validity of the measure's scores for the intended use in your particular sample.\index{validity!overview}
...
## Getting Started {#sec-gettingStarted-validity}

### Load Libraries {#sec-loadLibraries-validity}
...
### Prepare Data {#sec-prepareData-validity}

#### Simulate Data {#sec-simulateData-validity}
...
#### Add Missing Data {#sec-addMissingData-validity}
...
## Types of Validity {#sec-ValidityTypes}

Like [reliability](#sec-typesOfReliability), validity is not one thing.\index{validity}\index{reliability}
There are many types of validity.\index{validity}\index{validity!types of}
In this book, we discuss the following types of validity:\index{validity}\index{validity!types of}

- [face validity](#sec-faceValidity)\index{validity!face}
- [content validity](#sec-contentValidity)\index{validity!content}
- [criterion-related validity](#sec-criterionValidity)\index{validity!criterion}
- [concurrent validity](#sec-concurrentValidity)\index{validity!concurrent}
- [predictive validity](#sec-predictiveValidity)\index{validity!predictive}
- [construct validity](#sec-constructValidity)\index{validity!construct}
- [convergent validity](#sec-convergentValidity)\index{validity!convergent}
- [discriminant (divergent) validity](#sec-discriminantValidity)\index{validity!discriminant}
- [incremental validity](#sec-incrementalValidity)\index{validity!incremental}
- [treatment utility of assessment](#sec-treatmentUtility)\index{validity!treatment utility of assessment}
- [discriminative validity](#sec-discriminativeValidity)\index{validity!discriminative}
- [elaborative validity](#sec-elaborativeValidity)\index{validity!elaborative}
- [consequential validity](#sec-consequentialValidity)\index{validity!consequential}
- [representational validity](#sec-representationalValidity)\index{validity!representational}
- [factorial (structural) validity](#sec-factorialValidity)\index{validity!factorial}\index{validity!structural}
- [ecological validity](#sec-ecologicalValidity)\index{validity!ecological}
- [process-focused validity](#sec-processFocusedValidity)\index{validity!process focused}
- [diagnostic validity](#sec-diagnosticValidity)\index{validity!diagnostic}
- [social validity](#sec-socialValidity)\index{validity!social}
- [cultural validity](#sec-culturalValidity)\index{validity!cultural}
- [internal validity](#sec-internalValidity)\index{validity!internal}
- [external validity](#sec-externalValidity)\index{validity!external}
- [(statistical) conclusion validity](#sec-conclusionValidity)\index{validity!conclusion}\index{validity!statistical conclusion}

We arrange these types of validity into two broader categories: [measurement validity](#sec-measurementValidity) and [research design validity](#sec-researchDesignValidity).\index{validity!measurement}\index{validity!research design}
...
Fourth, a face valid measure may be too simple because anybody can understand the questions and what the questions are intended to assess, so (presumably) respondents can easily fake responses to achieve their goals.\index{validity!face}
...
There is also controversy about whether face validity is a true form of validity; many researchers have argued that it is not a true psychometric form of validity, because the *appearance* of validity is not validity [@Royal2016].\index{validity!face}
...
Content validity differs from [face validity](#sec-faceValidity) in that, for [face validity](#sec-faceValidity), a *layperson* determines whether or not the measure seems to assess the construct of interest.\index{validity!content}\index{validity!face}
...
For instance, the construct of depression has many facets, such as sleep disturbances, weight/appetite changes, low mood, suicidality, etc., as depicted in @fig-contentValidity.\index{validity!content}

::: {#fig-contentValidity}
![](images/contentValidity.png){width=100% fig-alt="Content Facets of the Construct of Depression."}

Content Facets of the Construct of Depression.
:::
...
Constructs can manifest differently at different points in development, known as [heterotypic continuity](#sec-heterotypicContinuity), which is discussed in @sec-heterotypicContinuity of @sec-repeated-assessments on [repeated assessments across time](#sec-repeated-assessments).\index{validity!content}\index{heterotypic continuity}
When considering the different dimensions of your population, it can be helpful to remember the acronym [*ADDRESSING*](#sec-addressing), which is described in @sec-addressing of @sec-diversity on [cultural and individual diversity](#sec-diversity).\index{validity!content}\index{diversity}\index{diversity!ADDRESSING acronym}

However, like [face validity](#sec-faceValidity), content validity is based on subjective judgment, which can be inaccurate.\index{validity!content}\index{validity!face}\index{judgment}\index{clinical judgment}
...
Below, we provide an example of criterion-related validity by examining the [Pearson correlation](#sec-correlationConsiderations) between a predictor and a criterion.\index{validity!criterion}
...
##### Concurrent Validity {#sec-concurrentValidity}
...
###### Sensitivity to Change (Responsiveness) {#sec-sensitivityToChange}

An important aspect of validity that is related to [concurrent validity](#sec-concurrentValidity) is the degree to which a measure's scores show *sensitivity to change* [also known as *responsiveness*\; @Fok2015].\index{validity!concurrent}\index{change!sensitivity to}\index{responsiveness!zzzzz@\igobble|seealso{sensitivity}}
If a researcher conducts an intervention that improves people's mood, it is important to have measures that are sensitive enough to detect the improvements in people's mood.\index{change!sensitivity to}

###### Nonreactivity {#sec-nonreactivity}

As noted in @sec-reactivity, the *reactivity* of a measure is how a measure's scores change in response to being measured.\index{observation!reactivity}
In an intervention, reactivity can be valuable because having the person self-monitor their behavior can lead to behavior improvement, as described in @sec-selfMonitoring.\index{observation!reactivity}\index{self-monitoring}
However, when the goal is to measure change over time, reactivity can be a problem because it can lead to changes in the measure's scores that are not due to people's changes in the construct.\index{observation!reactivity}
Thus, when merely monitoring changes over time, it is important to use measures that show nonreactivity [@Myers2002].\index{observation!reactivity}

##### Predictive Validity {#sec-predictiveValidity}
...
@Revelle2024 argues that insufficient attention is paid to [predictive validity](#sec-predictiveValidity) due to excessive focus on [reflective latent variables](#sec-reflectiveConstruct) and [internal consistency](#sec-internalConsistency-reliability) as a means to satisfy considerations of [construct validity](#sec-constructValidity) at the expense of the [usefulness](#sec-validity) of the measure.

##### Empiricism and Theory {#sec-theoryEmpiricism}
...
An example of dustbowl empiricism is the approach that was used to develop the first version of the [Minnesota Multiphasic Personality Inventory](#sec-mmpi) (MMPI).\index{radical operationalism}\index{empiricism}\index{Minnesota Multiphasic Personality Inventory}
The [MMPI](#sec-mmpi) was developed using an approach known as empirical-criterion keying, where items were selected for the scale for no reason other than the items demonstrate an association with the criterion.\index{radical operationalism}\index{empiricism}\index{Minnesota Multiphasic Personality Inventory}\index{validity!criterion}
...
Using this method with hundreds of items (and thousands of inter-item correlations), the [MMPI](#sec-mmpi) developed 10 clinical scales, which involved operational rules based on previously collected empirical evidence.\index{radical operationalism}\index{empiricism}\index{Minnesota Multiphasic Personality Inventory}\index{validity!criterion}
...
The developers originally numbered the [MMPI](#sec-mmpi) clinical scales from 1 to 10.\index{radical operationalism}\index{empiricism}\index{Minnesota Multiphasic Personality Inventory}\index{validity!criterion}
...
The [MMPI](#sec-mmpi) is discussed in further detail in @sec-objective-personality on [objective personality testing](#sec-objective-personality).\index{radical operationalism}\index{empiricism}\index{Minnesota Multiphasic Personality Inventory}\index{validity!criterion}

The idea of dustbowl empiricism was to develop a strong empirical base that would provide a strong foundation to help build up to a broad understanding that was integrated, coherent, and systematic.\index{radical operationalism}\index{empiricism}\index{validity!criterion}
However, this process was unclear when there was only a table of correlations.\index{radical operationalism}\index{empiricism}\index{validity!criterion}
Radical operationalists were opposed to [content validity](#sec-contentValidity) because it allows intrusion of our flawed thinking.\index{radical operationalism}\index{empiricism}\index{validity!criterion}\index{validity!content}
...
Consider that you find an association between variables $X$ and $Y$, consistent with your hypothesis, as depicted in @fig-commonCause1.\index{association!causation}\index{causality}

::: {#fig-commonCause1}
![](images/commonCause_1.png){width=100% fig-alt="Hypothesized Causal Effect Based on an Observed Association Between $X$ and $Y$, Such That $X$ Causes $Y$."}
...
First, the association could reflect the opposite direction of effect, where $Y$ actually causes $X$, as depicted in @fig-commonCause2.\index{association!causation}\index{direction of effect}\index{causality}

::: {#fig-commonCause2}
![](images/commonCause_2.png){width=100% fig-alt="Reverse (Opposite) Direction of Effect From the Hypothesized Effect, Where $Y$ Causes $X$."}
...
A third variable confound that is a common cause of both $X$ and $Y$ is depicted in @fig-commonCause3.\index{association!causation}\index{confound}\index{causality}

::: {#fig-commonCause3}
![](images/commonCause_3.png){width=100% fig-alt="Confounded Association Between $X$ and $Y$ due to a Common Cause, $Z$."}
...
###### Understanding the Causal System {#sec-causalSystem}
...
For instance, we have made great strides in the last decades when it comes to more accurate weather forecasts (see [here](https://www.npr.org/sections/money/2023/07/11/1186458991/should-we-invest-more-in-weather-forecasting-it-may-save-your-life); archived at <https://perma.cc/PF8P-BT3D> and [here](https://www.bbc.com/news/articles/cwy1epz58pyo); archived at <https://perma.cc/248N-9CW5>), including extreme weather events like hurricanes.\index{causality}
...
###### Model Over-Fitting {#sec-overfitting}
...
In @fig-overfittingModel, the blue line represents the true distribution of the data, and the red line is an over-fitting model:\index{over-fitting}

```{r}
#| label: fig-overfittingModel
#| fig-cap: "Over-fitting Model in Red Relative to the True Distribution of the Data in Blue."
#| fig-alt: "Over-fitting Model in Red Relative to the True Distribution of the Data in Blue."
...
###### Criterion Contamination {#sec-criterionContamination}
...
###### Using Theory as a Guide {#sec-theoryAsAGuide}
...
In clinical psychology, for example, we are often predicting a diagnosis, which is not that much more valid compared to our measure/predictor.\index{theory}\index{empiricism}\index{criterion}\index{diagnosis}
...
In psychology, our "theories" are more like conceptual frameworks [@Stallworthy2025].\index{theory}
...
###### Psychoanalysis Versus Empiricism {#sec-psychoanalysisVsEmpiricism}
...
If we consider this idea that someone might be able to extrapolate a simulation of the universe from a single piece of cake, we find similarities to how psychoanalysts connect everything to everything else through grand theories.\index{psychoanalysis}\index{theory}
...
@fig-empiricism presents a depiction of empiricism.\index{empiricism}\index{radical operationalism}
...
::: {#fig-empiricism}
![](images/empiricism.png){width=100% fig-alt="Conceptual Depiction of Empiricism."}

Conceptual Depiction of Empiricism.
:::

Contrast the empirical approach with psychoanalysis, as depicted in @fig-psychoanalysis.

::: {#fig-psychoanalysis}
![](images/psychoanalysis.png){width=100% fig-alt="Conceptual Depiction of Psychoanalysis."}

Conceptual Depiction of Psychoanalysis.
:::
...
There is a classic and hilarious [Monty Python video](https://osf.io/gc79d) (see @fig-montyPython) that is an absurd example akin to radical operationalism taken to the extreme [@MontyPythonPenguinResearch]:\index{empiricism}\index{radical operationalism}

::: {#fig-montyPython}

{{< video images/montyPythonPenguinResearch.mp4 >}}
...
[@MontyPythonPenguinResearch] provides a classic and hilarious [Monty Python video](https://osf.io/gc79d) that is an absurd example akin to radical operationalism taken to the extreme.\index{empiricism}\index{radical operationalism}
...
By contrast, [criterion-related validity](#sec-criterionValidity) deals with an observable criterion.\index{validity!criterion}\index{criterion}

Construct validity encompasses all other types of [measurement validity](#sec-measurementValidity) (e.g., [content](#sec-contentValidity) and [criterion-related](#sec-criterionValidity) validity), in addition to\index{validity!construct}\index{validity!measurement}\index{validity!content}\index{validity!criterion}

- scores on the measure show homogeneity—i.e., scores on the measure assess a single construct\index{validity!construct}
- scores on the measure show theoretically expected developmental changes (i.e., [criterion-related](#sec-criterionValidity) validity with respect to age or development)\index{validity!construct}\index{validity!criterion}
- scores on the measure show theoretically expected group differences (i.e., [criterion-related](#sec-criterionValidity) validity with respect to some group status)—this is also called known groups validity (when the groups are known to differ in their level on the construct)\index{validity!construct}\index{validity!criterion}
- scores on the measure show theoretically expected intervention effects (i.e., [criterion-related](#sec-criterionValidity) validity with respect to intervention)\index{validity!construct}\index{validity!criterion}
- establishing the *nomological network* of a construct\index{validity!construct}\index{nomological network}

Threats to construct validity are described by @Shadish2002 (see Table 3.1).\index{validity!conclusion}\index{validity!statistical conclusion}

##### Nomological Network {#sec-nomologicalNetwork}
...
An example of a nomological network is depicted in @fig-nomologicalNetwork.\index{validity!construct}\index{nomological network}

::: {#fig-nomologicalNetwork}
![](images/nomologicalNetwork.png){width=100% fig-alt="Example of a Nomological Network. O.M. = Observable Manifestation."}

Example of a Nomological Network. O.M. = Observable Manifestation.
:::
...
Approaches such as [network analysis](#sec-networkAnalysis) may be useful toward this aim.\index{network analysis}
...
Therefore, it is a low-quality claim that counting backward is part of the phenomenology of these disorders because it lacks differential deficit or [discriminant validity](#sec-discriminantValidity) [@Campbell1959].\index{validity!construct}\index{nomological network}\index{validity!discriminant}
...
##### Multitrait-Multimethod Matrix (MTMM) {#sec-MTMM}
...
The MTMM allows you to split the variance of measures' scores into variance that is due to the method (i.e., [method variance or method bias](#sec-methodBias)) and variance that is due to the construct (trait) of interest (i.e., construct variance).\index{multitrait-multimethod matrix}\index{validity!construct}\index{method variance}\index{construct!construct variance}
...
For an example of an MTMM, see @fig-campbellFiske.\index{multitrait-multimethod matrix}\index{validity!construct}
Several aspects of psychometrics can be evaluated with an MTMM, including [reliability](#sec-reliability), [convergent validity](#sec-convergentValidity), and [discriminant validity](#sec-discriminantValidity).\index{multitrait-multimethod matrix}\index{validity!construct}\index{reliability}\index{validity!convergent}\index{validity!discriminant}
The [reliability](#sec-reliability) diagonal of an MTMM is the correlation of a variable with itself, i.e., the [test–retest reliability](#sec-testRetest-reliability) or monotrait-monomethod correlations.\index{multitrait-multimethod matrix}\index{validity!construct}\index{reliability}\index{reliability!test–retest}
The [reliability](#sec-reliability) coefficients should be the highest values in the matrix because each measure should be more correlated with itself than with anything else.\index{multitrait-multimethod matrix}\index{validity!construct}\index{reliability}\index{reliability!test–retest}

::: {#fig-campbellFiske}
![](images/campbellFiske.png){width=100% fig-alt="Multitrait-Multimethod Matrix. (Figure reprinted from Campbell and Fiske [-@Campbell1959], Table 1, p. 82. Campbell, D. T., & Fiske, D. W. (1959). Convergent and discriminant validation by the multitrait-multimethod matrix. *Psychological Bulletin*, *56*, 81–105. <https://doi.org/10.1037/h0046016>)"}

Multitrait-Multimethod Matrix. (Figure reprinted from Campbell and Fiske [-@Campbell1959], Table 1, p. 82. Campbell, D. T., & Fiske, D. W. (1959). Convergent and discriminant validation by the multitrait-multimethod matrix. *Psychological Bulletin*, *56*, 81–105. <https://doi.org/10.1037/h0046016>)
:::
...
An MTMM organized by method then by construct is depicted in @fig-mtmmMethodConstruct.\index{multitrait-multimethod matrix}\index{validity!construct}

::: {#fig-mtmmMethodConstruct}
![](images/MTMM-02.png){width=100% fig-alt="Multitrait-Multimethod Matrix Organized by Method Then by Construct."}

Multitrait-Multimethod Matrix Organized by Method Then by Construct.
:::

An MTMM organized by construct then by method is depicted in @fig-mtmmConstructMethod.\index{multitrait-multimethod matrix}\index{validity!construct}

::: {#fig-mtmmConstructMethod}
![](images/MTMM-03.png){width=100% fig-alt="Multitrait-Multimethod Matrix Organized by Construct Then by Method."}

Multitrait-Multimethod Matrix Organized by Construct Then by Method.
:::
...
###### Discriminant (Divergent) Validity {#sec-discriminantValidity}
...
Discriminant validity is the extent to which a measure is *not* associated (or less strongly associated) with measures of different constructs that are not theoretically expected to be related (compared to associations with measures of the same construct).\index{multitrait-multimethod matrix}\index{validity!construct}\index{validity!discriminant}
In an MTMM, discriminant validity determines the extent to which a measure does not correlate with measures that share a method but assess different constructs.\index{multitrait-multimethod matrix}\index{validity!construct}\index{validity!discriminant}
For strong discriminant validity, we would expect the discriminant correlations (heterotrait monomethod) to be low.\index{multitrait-multimethod matrix}\index{validity!construct}\index{validity!discriminant}
...
For example, if extraversion and anxiety are moderately correlated with each other but uncorrelated with achievement, we would expect this pattern of interrelations between constructs would hold, regardless of which method was used to assess the construct.\index{multitrait-multimethod matrix}\index{validity!construct}\index{validity!discriminant}
...
As described [here (https://www.henseler.com/htmt.html)](https://www.henseler.com/htmt.html) (archived at <https://perma.cc/A9DU-WZWQ>) based on evidence from @Voorhees2016,\index{multitrait-multimethod matrix}\index{validity!construct}\index{validity!discriminant}
...
The authors updated the HTMT to use the geometric mean of the correlations rather than the arithmetic mean of the correlations to relax the assumption of [tau-equivalence](#sec-cttTauEquivalent) [@Roemer2021].\index{multitrait-multimethod matrix}\index{validity!construct}\index{validity!discriminant}
...
Unlike HTMT, HTMT2 can be used only if all inter-variable correlations are positive.\index{multitrait-multimethod matrix}\index{validity!construct}\index{validity!discriminant}
...
Other indexes of discriminant validity include the $\text{CI}_\text{CFA}\text{(sys)}$ [@Ronkko2020], $\chi^2\text{(sys)}$ [@Ronkko2020], and the Fornell-Larcker Ratio [FLR\; @Fornell1981].\index{multitrait-multimethod matrix}\index{validity!construct}\index{validity!discriminant}
The FLR is based on the idea that a construct should be more strongly associated with its indicators than with other constructs, and is computed as the ratio of (a) the highest squared inter-correlation the construct has with other constructs to (b) the [average variance extracted](#sec-averageVarianceExtracted) [AVE] by the construct.\index{multitrait-multimethod matrix}\index{validity!construct}\index{validity!discriminant}\index{reliability!internal consistency!average variance extracted}
A criterion of FLR < 1 evaluates whether the [AVE](#sec-averageVarianceExtracted) of a factor is greater than the highest squared inter-correlation between the factor and other factors in the model [@Fornell1981].\index{multitrait-multimethod matrix}\index{validity!construct}\index{validity!discriminant}\index{reliability!internal consistency!average variance extracted}
...
This would be a poor MTMM for two reasons: (1) there are trivial differences in the methods, which would lead to obvious convergence, and (2) the differences between the constructs are not important—they are obviously discriminant.\index{multitrait-multimethod matrix}\index{validity!construct}\index{validity!discriminant}
...
###### Multitrait-Multimethod Correlation Matrix {#sec-mtmmCorrelationMatrix}
...
```{r}
#| label: tbl-mtmmCorrelationHTML
#| tbl-cap: "Multitrait-Multimethod Correlation Matrix."
...
###### Construct Validity Beyond Campbell and Fiske {#sec-constructValidity-beyondCampbellFiske}
...
Triangulation is conceptually depicted in @fig-triangulation.\index{validity!construct}\index{triangulation}
...
::: {#fig-triangulation}
![](images/Triangulation.png){width=100% fig-alt="Using Triangulation to Arrive at a Closer Estimate of the Construct Using Multiple Measures and/or Methods."}

Using Triangulation to Arrive at a Closer Estimate of the Construct Using Multiple Measures and/or Methods.
:::

A contemporary approach to multitrait-multimethod modeling uses [confirmatory factor analysis](#sec-cfa), as described below.\index{multitrait-multimethod matrix}\index{multitrait-multimethod matrix!confirmatory factor analysis}\index{validity!construct}\index{factor analysis!confirmatory}

###### MTMM in Confirmatory Factor Analysis (CFA) {#sec-mtmmCFA-validity}
...
For instance, you can use [structural equation modeling](#sec-sem) (SEM) or [confirmatory factor analysis](#sec-cfa) (CFA) to derive a latent variable of a construct from multiple methods to be free from method-related error variance to generate purer assessments of the construct to see how it relates to other constructs.\index{multitrait-multimethod matrix}\index{multitrait-multimethod matrix!confirmatory factor analysis}\index{validity!construct}\index{factor analysis!confirmatory}\index{latent variable}\index{structural equation modeling}\index{method variance}\index{measurement error}\index{methods!multiple}
For an example of an MTMM in [confirmatory factor analysis](#sec-cfa), see @fig-mtmmCFA and @sec-mtmmCFA in @sec-factor-analysis-PCA on [factor analysis](#sec-factor-analysis-PCA).\index{multitrait-multimethod matrix}\index{multitrait-multimethod matrix!confirmatory factor analysis}\index{validity!construct}\index{factor analysis!confirmatory}
Guidelines for MTMM in [CFA](#sec-cfa) are provided by @Marsh2026.\index{multitrait-multimethod matrix}\index{multitrait-multimethod matrix!confirmatory factor analysis}\index{validity!construct}\index{factor analysis!confirmatory}

::: {#fig-mtmmCFA}
![](images/mtmmCFA.png){width=100% fig-alt="Multitrait-Multimethod Model in Confirmatory Factor Analysis With Three Constructs (Internalizing, Externalizing, and Thought-Disordered Problems) and Three Methods (Mother-, Father-, and Teacher-report)."}
...
A path diagram of the model is depicted in @fig-mtmmCFAFigure using the `semPlot` package [@R-semPlot].\index{multitrait-multimethod matrix}\index{multitrait-multimethod matrix!confirmatory factor analysis}\index{validity!construct}\index{factor analysis!confirmatory}

```{r}
#| label: fig-mtmmCFAFigure
...
It is not enough to show that the measure is better than chance, and the measure should not just be capitalizing on shared method variance with the criterion or on increased [reliability](#sec-reliability) of the measure.\index{validity!incremental}\index{prediction}\index{method variance}\index{reliability}
...
Incremental validity is a specific kind of [criterion-related validity](#sec-criterionValidity): significantly increased $R^2$ in hierarchical regression.\index{validity!incremental}\index{prediction}\index{validity!criterion}
...
#### Treatment Utility of Assessment {#sec-treatmentUtility}

@Hayes1987 argued that it is not enough for measures to be [reliable](#sec-reliability) and valid.\index{reliability}\index{validity}\index{useful}\index{validity!treatment utility of assessment}
...
The treatment utility of assessment is a specific kind of [criterion-related validity](#sec-criterionValidity), with the idea that "all criteria are not created equal."\index{validity!treatment utility of assessment}\index{validity!criterion}
...
Manipulated assessment and manipulated use are depicted in @fig-treatmentUtility.\index{validity!treatment utility of assessment}\index{validity!treatment utility of assessment!manipulated assessment}\index{validity!treatment utility of assessment!manipulated use}

::: {#fig-treatmentUtility}
![](images/treatmentUtility.png){width=100% fig-alt="Research Designs That Evaluate the Treatment Utility of Assessment."}

Research Designs That Evaluate the Treatment Utility of Assessment.
:::
...
Making the assessment data available but not assigning a certain treatment based on the assessment outcomes better simulates a realistic clinical environment.\index{validity!treatment utility of assessment}\index{validity!treatment utility of assessment!manipulated assessment}
...
Disadvantages of manipulated use include that (a) it is not as naturally realistic in terms of how intervention occurs in the real world and (b) you cannot remove the assessment information from clinicians in Group 2 (the group for whom the assessment information is not used for treatment selection) once they have it.

Relatively few measures show evidence of treatment utility of assessment.\index{validity!treatment utility of assessment}
For instance, a clinical trial using the manipulated assessment approach determined that the Development and Well-Being Assessment (DAWBA) did not show treatment utility as a diagnostic assessment for children and adolescents [@Sayal2025].\index{validity!treatment utility of assessment}
...
#### Discriminative Validity {#sec-discriminativeValidity}
...
Discriminative validity is not to be confused with [discriminant (divergent) validity](#sec-discriminantValidity) and discriminant analysis.\index{validity!discriminative}\index{validity!discriminant}
...
#### Elaborative Validity {#sec-elaborativeValidity}
...
Elaborative validity is a type of [incremental](#sec-incrementalValidity) theoretical validity.\index{validity!elaborative}\index{validity!incremental}\index{theory}
It is a combination of [criterion-related validity](#sec-criterionValidity) and [construct validity](#sec-constructValidity) that examines how much a given measure increases our understanding of a construct's [nomological network](#sec-nomologicalNetwork).\index{validity!elaborative}\index{validity!criterion}\index{validity!construct}\index{nomological network}
...
#### Consequential Validity {#sec-consequentialValidity}
...
Another example of consequential validity is when the validity of a measure changes over time due to changes in people's or society's response, as depicted in @fig-consequentialValidity.\index{validity!consequential}
...
::: {#fig-consequentialValidity}
![](images/consequentialValidity.png){width=100% fig-alt="Invalidation of a Measure Due to Society's Response to the Use of the Measure."}

Invalidation of a Measure Due to Society's Response to the Use of the Measure.
:::
...
#### Representational Validity {#sec-representationalValidity}
...
For instance, representational validity is a type of [elaborative validity](#sec-elaborativeValidity) and [content validity](#sec-contentValidity).\index{validity!representational}\index{validity!elaborative}\index{validity!content}

#### Factorial (Structural) Validity {#sec-factorialValidity}

Factorial validity is examined in @sec-factorAnalysis on factor analysis.\index{validity!factorial}\index{validity!structural}
...
It is a type of [construct validity](#sec-constructValidity).\index{validity!factorial}\index{validity!structural}\index{validity!construct}
...
#### Process-Focused Validity {#sec-processFocusedValidity}
...
Process-focused validity is a type of [elaborative validity](#sec-elaborativeValidity) and [construct validity](#sec-constructValidity).\index{validity!process focused}\index{validity!elaborative}\index{validity!construct}
...
#### Diagnostic Validity {#sec-diagnosticValidity}
...
It is a form of [construct validity](#sec-constructValidity) for diagnoses.\index{validity!diagnostic}

#### Social Validity {#sec-socialValidity}
...
#### Cultural Validity {#sec-culturalValidity}
...
### Research Design (Experimental) Validity {#sec-researchDesignValidity}

Aspects of *research design validity*, also called experimental validity, involve the validity—i.e., the approximate truth of—an inference from a particular research design and approach [@Shadish2002].\index{validity!research design}
Research design validity includes [internal validity](#sec-internalValidity), [external validity](#sec-externalValidity), and [conclusion validity](#sec-conclusionValidity).\index{validity!research design}\index{validity!internal}\index{validity!external}\index{validity!conclusion}\index{validity!statistical conclusion}
...
- temporal precedence—does the (purported) cause occur before the (purported) effect?\index{validity!research design}\index{validity!internal}
...
- no plausible alternative explanations—such as third variables that could influence both variables and explain their covariation (i.e., confounds)\index{validity!research design}\index{validity!internal}

Even though a cause and its (direct) effect must be related (i.e., covary), it is not necessarily the case—for multiple reasons—that you would detect an association between a cause and an effect.\index{validity!internal}
First, the association may be nonlinear.\index{validity!internal}
If you are trying to detect an association using Pearson correlation, which assumes a linear association, the (nonlinear) association may go undetected.\index{validity!internal}
Second, from a cause to a downstream effect, multiple causal pathways with opposing signs may cancel out in the aggregate.\index{validity!internal}
A cause may have both beneficial and harmful effects on the outcome, and the mediating mechanisms can "cancel each other out".\index{validity!internal}
For instance, the cause may help via mechanism `A`, but may hurt via mechanism `B`.\index{validity!internal}
That is, consider if the cause, `X`, influences the effect, `Y`, via two mechanisms, `A` and `B` (i.e., mediation)—and that `X` has a *positive* influence on `A` (i.e., the association is positive), whereas `X` has a *negative* influence on `B` (i.e., the association is negative).\index{validity!internal}
In that case, the effects of the mediating mechanisms can cancel each other out, leaving no observable bivariate association between `X` and `Y`.\index{validity!internal}
Alternatively, a null association between `X` and `Y` can also arise if a [confound](#sec-causalDiagramConfounding) influences both `X` and `Y` in an opposing direction to the effect of `X` on `Y`.\index{validity!internal}
For example, `X` may (positively) influence `Y` via mediator `A`, whereas `B` is a confound with a negative effect on `Y`.\index{validity!internal}
In that case, the positive effect through `A` and the negative effect from `B` can cancel, so no association may be observed unless the confound is controlled for; failing to control for the confound is an example of omitted variable bias.\index{validity!internal}
So, a null association does not imply no causal effect.\index{validity!internal}
Wrongly inferring that a causal effect of one variable on another is not causal is an example of [statistical conclusion invalidity](#sec-conclusionValidity) arising from a failure to account for the underlying causal mechanisms of `X` on `Y` and/or omitted confounds.\index{validity!internal}\index{validity!statistical conclusion}
...
Threats to internal validity are other factors that may explain why the outcome could have occurred even if the hypothesized causal effect is noncausal.\index{validity!research design}\index{validity!internal}
Examples of potential threats to internal validity include history, maturation, testing, instrumentation, regression, selection, attrition, and an interaction of threats [@Slack2001; @Shadish2002, Table 2.4].\index{validity!research design}\index{validity!internal}

Research designs differ in the extent to which resulting causal inferences show internal validity versus [external validity](#sec-externalValidity).\index{validity!research design}\index{validity!internal}\index{validity!external}
...
They may be able to statistically account for potential confounds using covariates or for the reverse direction of effect using longitudinal designs.\index{validity!research design}\index{validity!internal}
Nevertheless, we can have greater confidence about whether a variable influences another variable in an experiment.\index{validity!research design}\index{validity!internal}
Thus, causal inferences from experiments tend to have higher internal validity than causal inferences from than correlational designs.\index{validity!research design}\index{validity!internal}

#### External Validity {#sec-externalValidity}

*External validity* is the extent to which the findings can be generalized across person, place, and time—e.g., to the broader population and the real world.\index{validity!research design}\index{validity!external}\index{external validity!zzzzz@\igobble|seealso{validity}}
...
For instance, [norm-referenced](#sec-norm) assessments attempt to identify the distribution of scores for a given population from a sample within that population.\index{validity!research design}\index{validity!external}\index{norm-referenced}
The validity of the [norms](#sec-norm) of a [norm-referenced](#sec-norm) assessment are important to consider [@Achenbach2001c].\index{validity!research design}\index{validity!external}\index{norm-referenced}
...
Some measures are known to have poor norms, including the Exner Comprehensive System [@Exner1974; @Exner2005] for administering and scoring the [Rorschach Inkblot Test](#sec-rorschach), which has been known to over-pathologize [@Wood2001a; @Wood2001b].\index{validity!research design}\index{validity!external}\index{norm}\index{Rorschach Inkblot Test}
The [Rorschach](#sec-rorschach) is discussed in greater detail in @sec-projective.\index{Rorschach Inkblot Test}
External validity can also depend on whether there is systematic missingness or attrition in the predictor or outcome variables of interest.
For instance, if particular subgroups are more likely to drop out or fail to respond, the remaining sample may differ systematically from the intended population, biasing estimates and reducing generalizability.
Threats to external validity are described by @Shadish2002 (see Table 3.2).\index{validity!conclusion}\index{validity!statistical conclusion}

##### Tradeoffs of Internal Validity and External Validity {#sec-internalExternalValidityTradeoffs}

It is important to note that there is a tradeoff between [internal](#sec-internalValidity) and [external](#sec-externalValidity) validity—a single research design does not always yield inferences with both high [internal](#sec-internalValidity) and high [external](#sec-externalValidity) validity.\index{validity!research design}\index{validity!internal}\index{validity!external}
...
Thus, experiments tend to yield inferences with higher [internal validity](#sec-internalValidity) than other research designs.\index{validity!research design}\index{validity!internal}\index{validity!external}\index{research design!experimental}
...
Inferences from observational designs have greater capacity for [external validity](#sec-externalValidity) than experimental designs because people can be observed in their natural environments to see how variables are related in the real world.\index{validity!research design}\index{validity!internal}\index{validity!external}\index{research design!observational}
However, the greater [external validity](#sec-externalValidity) comes at a cost of lower [internal validity](#sec-internalValidity).\index{validity!research design}\index{validity!internal}\index{validity!external}\index{research design!observational}
Observational designs are not well-positioned to make causal inferences because they have multiple threats to [internal validity](#sec-internalValidity), including issues of temporal precedence in cross-sectional observational designs, and there are numerous potential third variables (i.e., confounds) that could act as a common cause of both the predictor and outcome variables.\index{validity!research design}\index{validity!internal}\index{validity!external}\index{research design!observational}\index{correlation!causation}
...
As the [internal validity](#sec-internalValidity) of inferences from a study's design increases, the [external validity](#sec-externalValidity) of its inferences tend to decrease.\index{validity!research design}\index{validity!external}\index{validity!internal}
...
Because no single research design always yields inferences with both high [internal](#sec-internalValidity) and [external](#sec-externalValidity) validity, scientific inquiry needs a combination of many different research designs so we can be more confident in our inferences—experimental designs for making causal inferences and observational designs for making inferences that are more likely to reflect the real world.\index{validity!research design}\index{validity!internal}\index{validity!external}\index{research design!experimental}\index{research design!observational}

Case studies, because they have smaller sample sizes, tend to have lower [external validity](#sec-externalValidity) than both experimental and observational studies.\index{validity!research design}\index{validity!internal}\index{validity!external}\index{research design!experimental}\index{research design!observational}\index{research design!case study}
Case studies also tend to yield inferences with lower [internal validity](#sec-internalValidity) because they have less potential to control for threats to [internal validity](#sec-internalValidity), such as potential confounds or temporal precedence.\index{validity!research design}\index{validity!internal}\index{validity!external}\index{research design!case study}
...
#### (Statistical) Conclusion Validity {#sec-conclusionValidity}

*Conclusion validity*, also called statistical conclusion validity, considers the extent to which conclusions are reasonable about the association among variables based on the data.\index{validity!research design}\index{validity!conclusion}\index{validity!statistical conclusion}
...
Threats to conclusion validity are described by @Shadish2002 (see Table 2.2).\index{validity!conclusion}\index{validity!statistical conclusion}
Did you have adequate statistical power, adequate [reliability](#sec-reliability) of measures, accurate model estimation, and proper treatment of missing data?\index{validity!conclusion}\index{validity!statistical conclusion}
Did you not violate assumptions of the statistical tests used?\index{validity!conclusion}\index{validity!statistical conclusion}
Did any characteristics of the data distort the results (e.g., outliers, missingness, nonnormality, restriction of range, etc.)?\index{validity!conclusion}\index{validity!statistical conclusion}
Did you avoid fishing for significant results (i.e., *p*-hacking) and inflating the Type I error rate by performing multiple tests?\index{validity!conclusion}\index{validity!statistical conclusion}
Did you avoid making causal inferences from correlational data?\index{validity!conclusion}\index{validity!statistical conclusion}
Were effect sizes estimated accurately?\index{validity!conclusion}\index{validity!statistical conclusion}
To what extent are participants heterogeneous?\index{validity!conclusion}\index{validity!statistical conclusion}
If a treatment was administered, was it implemented with fidelity for all participants?\index{validity!conclusion}\index{validity!statistical conclusion}
To what extent was there extraneous variance in the research setting that may have contributed to error variance?\index{validity!conclusion}\index{validity!statistical conclusion}
All of these considerations are important for ensuring that the conclusions drawn from a study are valid.\index{validity!conclusion}\index{validity!statistical conclusion}

### Putting It All Together: An Organizational Framework {#sec-organizationalFramework-validity}

There are many types of [measurement validity](#sec-measurementValidity), but the central psychometric aspect of [measurement validity](#sec-measurementValidity) is [construct validity](#sec-constructValidity).\index{validity}\index{validity!measurement}\index{validity!organizational framework}\index{validity!construct}
That is, whether the measure accurately assesses the target construct is the most important consideration of [measurement validity](#sec-measurementValidity).\index{validity}\index{validity!measurement}\index{validity!organizational framework}\index{validity!construct}
As discussed earlier, [construct validity](#sec-constructValidity) includes the [nomological network](#sec-nomologicalNetwork) of the construct.
[Construct validity](#sec-constructValidity) also subsumes other key types of [measurement validity](#sec-measurementValidity), including\index{validity}\index{validity!measurement}\index{validity!organizational framework}\index{validity!construct}\index{nomological network}

- [Convergent validity](#sec-convergentValidity)\index{validity}\index{validity!measurement}\index{validity!organizational framework}\index{validity!construct}\index{validity!convergent}
- [Discriminant (divergent) validity](#sec-discriminantValidity)\index{validity}\index{validity!measurement}\index{validity!organizational framework}\index{validity!construct}\index{validity!discriminant}
- [Criterion-related validity](#sec-criterionValidity)\index{validity}\index{validity!measurement}\index{validity!organizational framework}\index{validity!construct}\index{validity!criterion}
    - [Concurrent validity](#sec-concurrentValidity)\index{validity}\index{validity!measurement}\index{validity!organizational framework}\index{validity!construct}\index{validity!concurrent}
    - [Predictive validity](#sec-predictiveValidity)\index{validity}\index{validity!measurement}\index{validity!organizational framework}\index{validity!construct}\index{validity!predictive}
- [Content validity](#sec-contentValidity)\index{validity}\index{validity!measurement}\index{validity!organizational framework}\index{validity!construct}\index{validity!content}

The organization of types of [measurement validity](#sec-measurementValidity) that are subsumed by [construct validity](#sec-constructValidity) is depicted in @fig-constructValidityOrganization. \index{validity}\index{validity!measurement}\index{validity!organizational framework}\index{validity!construct}

::: {#fig-constructValidityOrganization}
![](images/Validity-03.png){width=100% fig-alt="Organization of Types of Measurement Validity That Are Subsumed by Construct Validity."}

Organization of Types of Measurement Validity That Are Subsumed by Construct Validity.
:::

Moreover, many different types of [reliability](#sec-reliability) and validity can be viewed through the lens of [construct validity](#sec-constructValidity):\index{validity}\index{validity!measurement}\index{validity!organizational framework}\index{validity!construct}\index{reliability}

- [Internal consistency](#sec-internalConsistency-reliability), which can be estimated as the [coefficient of internal consistency](#sec-internalConsistency-reliability), where the criterion for [criterion-related validity](#sec-criterionValidity) is other items on the same measure\index{validity}\index{validity!measurement}\index{validity!organizational framework}\index{validity!construct}\index{reliability!internal consistency}\index{validity!criterion}
- [Test–retest reliability](#sec-testRetest-reliability), which can be estimated as the [coefficient of stability](#sec-stability), where the criterion for [criterion-related validity](#sec-criterionValidity) is the same measure at another time point\index{validity}\index{validity!measurement}\index{validity!organizational framework}\index{validity!construct}\index{reliability!test–retest}\index{reliability!test–retest!coefficient of stability}\index{validity!criterion}
- [Parallel-forms reliability](#sec-parallelForms-reliability) or [convergent validity](#sec-convergentValidity), which can be estimated as the [coefficient of equivalence](#sec-parallelForms-reliability), where the criterion for [criterion-related validity](#sec-criterionValidity) is the parallel form\index{validity}\index{validity!measurement}\index{validity!organizational framework}\index{validity!construct}\index{reliability!parallel forms}\index{reliability!parallel forms!coefficient of equivalence}\index{validity!criterion}

## Validity Is a Process, Not an Outcome {#sec-validityAsAProcess}
...
We would express the strength of evidence on a measure's validity across the different types of validity for a particular purpose, with a particular population, in a particular context (consistent with [generalizability theory](#sec-gTheoryReliability)).\index{validity}\index{generalizability theory}

## Reliability Versus Validity {#sec-reliabilityVsValidity}

[Reliability](#sec-reliability) and validity are not the same thing.\index{validity}\index{reliability}
[Reliability](#sec-reliability) deals with consistency, whereas validity deals with accuracy.\index{validity}\index{reliability}
A measure can be consistent but not accurate (see @fig-reliabilityVsValidity2). \index{validity}\index{reliability}
That is, a measure can be [reliable](#sec-reliability) but not valid.\index{validity}\index{reliability}
However, a measure cannot be accurate if it is inconsistent; that is, a measure cannot be valid and [unreliable](#sec-reliability).\index{validity}\index{reliability}

::: {#fig-reliabilityVsValidity2}
![](images/ReliabilityvsValidity-02.png){width=100% fig-alt="Traditional Depiction of Reliability (Consistency) Versus Validity (Accuracy)."}

Traditional Depiction of Reliability (Consistency) Versus Validity (Accuracy).
:::

The typical way of depicting the distinction between [reliability](#sec-reliability) and validity is in @fig-reliabilityVsValidity2, in which a measure can either have (a) low [reliability](#sec-reliability) and low validity, (b) high [reliability](#sec-reliability) and low validity, or (c) high [reliability](#sec-reliability) and high validity.\index{validity}\index{reliability}
...
When we distinguish between person- versus group-level accuracy, we can distinguish four general combinations of [reliability](#sec-reliability) and validity, as depicted in @fig-reliabilityVsValidity1: (a) low [reliability](#sec-reliability), low accuracy at the person level, and low accuracy at the group level, (b) low [reliability](#sec-reliability), low accuracy at the person level, and high accuracy at the group level, (c) high [reliability](#sec-reliability), low accuracy at the person level, and low accuracy at the group level, and (d) high [reliability](#sec-reliability), high accuracy at the person level, and high accuracy at the group level.\index{validity}\index{reliability}\index{individual level}\index{group level}
However, as discussed before, [reliability](#sec-reliability) and validity are not binary states of low versus high—they exist to varying degrees.\index{validity}\index{reliability}

::: {#fig-reliabilityVsValidity1}
![](images/ReliabilityvsValidity-01.png){width=100% fig-alt="Depiction of Reliability Versus Validity, While Distinguishing Between Validity (Accuracy) at the Person Versus Group Level."}

Depiction of Reliability Versus Validity, While Distinguishing Between Validity (Accuracy) at the Person Versus Group Level.
:::

Even though [reliability](#sec-reliability) and validity are not the same thing, there is a relation between [reliability](#sec-reliability) and validity.\index{validity}\index{reliability}
Validity depends on [reliability](#sec-reliability).\index{validity}\index{reliability}
[Reliability](#sec-reliability) is necessary but insufficient for validity.\index{validity}\index{reliability}
However, [measurement error](#sec-measurementError) ([unreliability](#sec-reliability)) can be [systematic](#sec-systematicError) or [random](#sec-randomError).\index{validity}\index{reliability}\index{measurement error!systematic error}\index{measurement error!random error}
If [measurement error](#sec-measurementError) is [systematic](#sec-systematicError), it reduces the validity of the measure.\index{validity}\index{reliability}\index{measurement error!systematic error}
If measurement error is [random](#sec-randomError), it reduces the precision of an individual's score on a measure, but the measure could still be a valid measure of the construct at the group level.\index{validity}\index{reliability}\index{measurement error!random error}\index{reliability!precision}
However, [random error](#sec-randomError) would make it more difficult to make an accurate prediction for an individual person.\index{validity}\index{reliability}\index{measurement error!random error}\index{reliability!precision}\index{individual level}

[Reliability](#sec-reliability) places the upper bound on validity because a measure can be no more valid than it is [reliable](#sec-reliability).\index{validity}\index{reliability}
...
Therefore, the maximum validity coefficient is the square root of the product of the [reliability](#sec-reliability) of each measure, as in @eq-maximumAssociation:\index{validity}\index{reliability}
...
So, the maximum validity coefficient is based on the [reliability](#sec-reliability) of each measure.\index{validity}\index{reliability}
To the extent that one of the measures is [unreliable](#sec-reliability), the validity coefficient will be attenuated relative to the true validity (i.e., the true strength of association of the constructs), as we describe below.\index{validity}\index{reliability}\index{association!attenuation of}\index{measurement error!attenuation of association}

## Effect of Measurement Error on Associations {#sec-effectOfMeasurementErrorOnAssociations}

@fig-validity depicts the [classical test theory](#sec-ctt) approach to understanding the validity of a measure, i.e., its association with another measure, which is the validity coefficient ($r_{xy}$).\index{classical test theory}\index{validity!coefficient of}

::: {#fig-validity}
![](images/Validity.png){width=50% fig-alt="The Criterion-Related Validity of a Measure, i.e., Its Association With Another Measure, as Depicted in a Path Diagram."}

The Criterion-Related Validity of a Measure, i.e., Its Association With Another Measure, as Depicted in a Path Diagram.
:::

As described above, ([random](#sec-randomError)) [measurement error](#sec-measurementError) weakens (or attenuates) the association between variables [@Goodwin2006; @Schmidt1996].\index{measurement error!attenuation of association}\index{measurement error!random error}
The greater the [random measurement error](#sec-randomError), the weaker the association.\index{measurement error!attenuation of association}\index{measurement error!random error}
Thus, the correlation between $x$ and $y$ depends on both the true correlation of $x$ and $y$ ($r_{x_{t}y_{t}}$) and the [reliabilities](#sec-reliability) of $x$ ($r_{xx}$) and $y$ ($r_{yy}$).\index{measurement error!attenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}
So, [measurement error](#sec-measurementError) in $x$ and $y$ can reduce the observed correlation below the true correlation.\index{measurement error!attenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}
This is known as the *attenuation formula* (@eq-attenuationFormula):\index{measurement error!attenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}
...
This correction for the attenuation of an association due to [measurement error](#sec-measurementError) (unreliability) is known as the disattenuation of a correlation, i.e., correction for the attenuation of an association due to [measurement error](#sec-measurementError) to get a more accurate estimate of the true association between constructs.\index{measurement error!attenuation of association}\index{measurement error!disattenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}
Rearranging the terms from the attenuation formula, the formula for disattenuation of a correlation (i.e., the *disattenuation formula*) is in @eq-disattenuationFormula:\index{measurement error!attenuation of association}\index{measurement error!disattenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}
...
All of this is implied in the path diagram (see @fig-validity).\index{measurement error!attenuation of association}\index{measurement error!disattenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}\index{validity!path diagram}
The attenuation and disattenuation formulas are based on [classical test theory](#sec-ctt), and therefore assume that all [measurement error](#sec-measurementError) is random, that errors are uncorrelated, etc.\index{measurement error!attenuation of association}\index{measurement error!disattenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}\index{classical test theory}
Nevertheless, the attenuation formula can be informative for understanding how imperiled your research is when your measures have low [reliability](#sec-reliability) (i.e., when there is instability in the measure).\index{measurement error!attenuation of association}\index{measurement error!disattenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}\index{reliability!problems of low reliability}
Unreliability is a strong threat to statistical power—i.e., being able to detect an effect if it exists.\index{reliability!problems of low reliability}\index{statistical power}
Many people do not account for [reliability](#sec-reliability) of the measures when calculating statistical power, but you should if you want your study to be adequately powered!\index{reliability!problems of low reliability}
For instance, one could determine what the expected observed association would be given the expected true association and the expected [reliability](#sec-reliability) of the measures, and then use that expected *observed* association to calculate statistical power.\index{reliability!problems of low reliability}\index{statistical power}
Researchers recommend accounting for measurement [reliability](#sec-reliability), to better estimate the association between constructs, either with the disattenuation formula [@Schmidt1996] or with [structural equation modeling](#sec-sem), as described in the next chapter.\index{measurement error!attenuation of association}\index{measurement error!disattenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}\index{structural equation modeling}

Although [unreliability](#sec-reliability) tends to weaken *bivariate* associations, its effects are less predictable in the context of three or more variables, such as when including covariates in multiple regression [@Shadish2002].\index{reliability!problems of low reliability}
[Unreliability](#sec-reliability) of a covariate has shown, in some cases, to lead to artificially *stronger* associations when between the predictor and outcome variables [@Shadish2002].\index{reliability!problems of low reliability}

### Test with Simulated Data {#sec-measurementErrorEffects-simulatedData}

#### Reliability of Predictor {#sec-measurementErrorEffects-reliabilityPredictor}
...
#### Reliability of Criterion {#sec-measurementErrorEffects-reliabilityCriterion}
...
#### True Association {#sec-measurementErrorEffects-trueAssociation}
...
#### Observed Association (After Adding Measurement Error) {#sec-measurementErrorEffects-observedAssociation}
...
Using simulated data, when the [reliability](#sec-reliability) of the predictor is `r apa(reliabilityPredictor_sim, 2, leading = FALSE)`, the [reliability](#sec-reliability) of the criterion is `r apa(reliabilityCriterion_sim, 2, leading = FALSE)`, and the true association between the predictor and criterion is $r = `r apa(trueAssociation_sim, 2, leading = FALSE)`$, the observed association is attenuated to $r = `r apa(observedAssociation_sim, 2, leading = FALSE)`$.\index{measurement error!attenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}

### Attenuation of True Correlation Due to Measurement Error {#sec-attenuation}

The attenuation formula is presented in @eq-attenuationFormula.
We extend it to a specific example in @eq-attenuationFormulaExample:\index{measurement error!attenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}
...
Below is an example of how to find the observed correlation between the predictor and criterion if the true association between the constructs (i.e., correlation between true scores of constructs) is .70, the [reliability](#sec-reliability) of the predictor is .90, and the [reliability](#sec-reliability) of the criterion is .85:\index{measurement error!attenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}
...
The [`petersenlab`](https://github.com/DevPsyLab/petersenlab) package [@R-petersenlab] contains the `attenuationCorrelation()` function that estimates the observed association given the true association and the reliability of the predictor and criterion:\index{measurement error!attenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}\index{petersenlab package}
...
attenuatedAssociation <- attenuationCorrelation(
  trueAssociation = 0.7, 
  reliabilityOfPredictor = 0.9,
  reliabilityOfCriterion = 0.85)
```

The observed association ($r = `r apa(attenuatedAssociation, 2, leading = FALSE)`$) is attenuated relative to the true association ($r = .70$).\index{measurement error!attenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}

### Disattenuation of Observed Correlation Due to Measurement Error {#sec-disattenuation}

The disattenuation formula is presented in @eq-disattenuationFormula.\index{measurement error!disattenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}
We extend it to a specific example in @eq-disattenuationFormulaExample:\index{measurement error!disattenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}
...
Find the true association between the construct assessed by the predictor and the construct assessed by the criterion given an observed association if the [reliability](#sec-reliability) of the predictor is .9, and the [reliability](#sec-reliability) of the criterion is .85:\index{measurement error!disattenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}
...
The disattenuation of an observed correlation due to [measurement error](#sec-measurementError) can be demonstrated using [structural equation modeling](#sec-sem).\index{measurement error!disattenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}\index{structural equation modeling}
...
A path diagram of the observed association is depicted in @fig-observedSEM.\index{measurement error!disattenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}\index{structural equation modeling}

```{r}
#| label: fig-observedSEM
#| fig-cap: "Observed Association, as Depicted in a Structural Equation Model."
#| fig-alt: "Observed Association, as Depicted in a Structural Equation Model."
...
Now consider when we account for the degree of [unreliability](#sec-measurementError) of each measure.\index{measurement error!disattenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}\index{structural equation modeling}
We can account for the [(un)reliability](#sec-reliability) of each measure by specifying the residual errors as $1 - \text{reliability}$, as below:\index{measurement error!disattenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}\index{structural equation modeling}
...
A path diagram of the true association is depicted in Figures [-@fig-disattenuationSEM] and [-@fig-semDisattenuation].\index{measurement error!disattenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}\index{structural equation modeling}

```{r}
#| label: fig-disattenuationSEM
#| fig-cap: "Disattenuation of an Observed Association Due to Measurement Error, as Depicted in a Structural Equation Model."
#| fig-alt: "Disattenuation of an Observed Association Due to Measurement Error, as Depicted in a Structural Equation Model."
...
::: {#fig-semDisattenuation}
![](images/SEM-01.png){width=50% fig-alt="Disattenuation of an Observed Association Due to Measurement Error, as Depicted in a Structural Equation Model."}

Disattenuation of an Observed Association Due to Measurement Error, as Depicted in a Structural Equation Model.
:::

The observed association ($\beta = `r apa(observedBeta, 2)`$) becomes $\beta = `r apa(trueBeta, 2)`$ when it is disattenuated for [measurement error](#sec-measurementError).\index{measurement error!disattenuation of association}\index{measurement error!random error}\index{true association}\index{observed association}\index{reliability}\index{validity!coefficient of}\index{structural equation modeling}

## Generalizability Theory (G-Theory) {#sec-gTheoryValidity}

[Generalizability theory](#sec-gTheory) (G-theory) is discussed in greater detail in the chapter on [reliability](#sec-reliability) in @sec-gTheoryReliability and in the chapter on [generalizability theory](#sec-gTheory).
As a brief reminder, [G-theory](#sec-gTheory) is a measurement theory that, unlike [classical test theory](#sec-ctt), does not treat all measurement differences across time, rater, or situation as "error" but rather as a phenomenon of interest.\index{generalizability theory}\index{validity}\index{classical test theory}
G-theory can simultaneously consider multiple aspects of [reliability](#sec-reliability) and validity in the same model, something that [classical test theory](#sec-ctt) cannot achieve.\index{generalizability theory}\index{validity}\index{reliability}\index{classical test theory}
...
- Make sure the measure's scores are [reliable](#sec-reliability).\index{validity!ways to increase}\index{reliability}
For potential ways to increase the [reliability](#sec-reliability) of measurement, see @sec-increaseReliability.\index{validity!ways to increase}\index{reliability}
But do not switch to a less valid measure or to items that are less valid merely because they are more [reliable](#sec-reliability).\index{validity!ways to increase}\index{reliability}
- Use multiple measures and multiple methods to remedy the effects of [method bias](#sec-methodBias).\index{validity!ways to increase}\index{method bias}\index{methods!multiple}
...
For instance, the items should show [content validity](#sec-contentValidity)—the items should assess facets of the target construct for the target population as defined by experts, without item intrusions from other constructs.\index{validity!ways to increase}\index{validity!content}
The items' scores should show [convergent validity](#sec-convergentValidity)—the items' scores should be related to other measures of the construct, within the population of interest.\index{validity!ways to increase}\index{validity!convergent}
The items' scores should show [discriminant validity](#sec-discriminantValidity)—the items' scores should be more strongly related to measures that are intended to assess the same construct than they are to measures that are intended to assess other constructs.\index{validity!ways to increase}\index{validity!discriminant}
- Combine the items with [latent variable modeling](#sec-latentVariableModeling), such as with [item response theory](#sec-irt) (IRT), [structural equation modeling](#sec-sem) (SEM), or [factor analysis](#sec-factorAnalysisOverview), to reduce the contribution of [random error](#sec-randomError) and to [disattenuate associations for measurement error](#sec-disattenuation).\index{validity!ways to increase}\index{latent variable}\index{structural equation modeling}\index{factor analysis}\index{factor analysis!confirmatory}\index{item response theory}
- Obtain samples that are as representative of the population as possible, paying attention to including people who are traditionally under-represented in research (if such groups are part of the target population).\index{validity!ways to increase}\index{sample!representative}
...
[Test bias](#sec-bias) is discussed in @sec-bias.\index{validity!ways to increase}\index{bias}
...
If people or organizations change their behavior in response to a measure, this can invalidate a measure's scores for the intended purpose, as described in @sec-consequentialValidity when discussing consequential validity.\index{validity!ways to increase}\index{validity!consequential}

## Conclusion {#sec-conclusion-validity}
...
Like [reliability](#sec-reliability), validity is not one thing.\index{validity}\index{reliability}
...
## Suggested Readings {#sec-readings-validity}

@Cronbach1955; @Clark2019

## Exercises {#sec-exercises-validity}
...
### Questions {#sec-exercisesQuestions-validity}
...
5. In @sec-mtmmCorrelationMatrix, we simulated data for a multitrait-multimethod matrix. The simulated data includes data on participants' verbal, spatial, and quantitative abilities, each assessed in three subtests in each of three methods: oral, written, and manipulative.
...
### Answers {#sec-exercisesAnswers-validity}
...
::: {#fig-mtmmMatrix}
![](images/MTMM.png){width=100% fig-alt="Multitrait-Multimethod Matrix."}
...
## Session Info {#sec-sessionInfo-validity}
