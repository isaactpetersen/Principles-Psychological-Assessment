...
Spearman identified a single factor, *g*, that accounted for most of the covariation between the measures of intelligence.\index{factor analysis!confirmatory}\index{intellectual assessment!$g$}
...
Given the extensive [method variance](#methodBias) that measures have, [factor analysis](#factorAnalysis) (and [principal component analysis](#pca)) tends to extract method factors.\index{factor analysis}\index{principal component analysis}\index{method bias}
Method factors are factors that are related to the methods being assessed rather than the construct of interest.\index{factor analysis}\index{method bias}
...
For instance, using path tracing rules (described in Section \@ref(ctt)), the correlation of measures within a factor in this model is calculated as: $0.60 = .77 \times .77$.\index{factor analysis}\index{path tracing rules}
Using path tracing rules, the correlation of measures across factors in this model is calculated as: $.30 = .77 \times .50 \times .77$.\index{factor analysis}\index{path tracing rules}
...
Using path tracing rules, the correlation of measures across factors in this model is calculated as: $.30 = .77 \times .71 \times .71 \times .77$.\index{factor analysis}\index{path tracing rules}
...
In this model, the measures no longer have random error: measures 1, 2, and 3 have correlated residuals—that is, they share error variance; likewise, measures 4, 5, and 6 have correlated residuals.\index{residuals!correlated}\index{factor analysis}
...
This model has the exact same same fit as the previous models.\index{factor analysis}
Using path tracing rules, the correlation of measures within a factor in this model is calculated as: $.60 = (.55 \times .55) + (.70 \times .43 \times .70) + (.70 \times .43 \times .43 \times .70)$.\index{factor analysis}\index{path tracing rules}
Using path tracing rules, the correlation of measures across factors in this model is calculated as: $.30 = .55 \times .55$.\index{factor analysis}\index{path tracing rules}
...
But if there are five factors and some correlated errors, then the sample size will need to be closer to ~5,000.\index{residuals!correlated}\index{factor analysis!practical considerations}
...
2. Method of factor extraction: [factor analysis](#factorAnalysis) or [PCA](#pca)\index{factor analysis!decisions}\index{factor analysis}\index{principal component analysis}
3. If [factor analysis](#factorAnalysis), the kind of [factor analysis](#factorAnalysis): [EFA](#efa) or [CFA](#cfa)\index{factor analysis!decisions}\index{factor analysis!confirmatory}\index{factor analysis!exploratory}
4. How many factors to retain\index{factor analysis}
5. If [EFA](#efa) or [PCA](#pca), whether and how to rotate factors (factor rotation)\index{factor analysis!decisions}\index{factor analysis!exploratory}\index{principal component analysis}
6. Model selection and interpretation\index{factor analysis!decisions}
...
The eigenvectors attempt to simplify the data in the matrix.\index{parsimony}
...
When using oblique rotation, however, the items explain more variance than is attributable to their factor loadings because the factors are correlated.
...
But if you estimate (6) eigenvectors and only keep (2), the model is a two-component model and whatever left becomes error.
...
But computers are so fast now—just do a real [factor analysis](#factorAnalysis)!
[Factor analysis](#factorAnalysis) better handles error than PCA—[factor analysis](#factorAnalysis) assumes that what is in the variable is the combination of common construct variance and error.
...
All cross loadings are freely estimated.\index{cross loading}
...
In CFA, cross loadings are not estimated unless the researcher specifies them.\index{cross loading}
...
In [CFA](#cfa), researchers often set only half of the constraints, and let the data fill in the rest.
...
ESEM can include any combination of exploratory (i.e., [EFA](#efa)) and confirmatory ([CFA](#cfa)) factors.
ESEM, unlike traditional [CFA](#cfa) models, typically estimates all cross loadings—at least for the exploratory factors.\index{cross loading}
If a [CFA](#cfa) model without cross loadings and correlated residuals fits as well as an ESEM model with all cross loadings, the [CFA](#cfa) model should be retained for its simplicity.\index{cross loading}\index{residuals!correlated}\index{parsimony}
However, ESEM models often fit better than [CFA](#cfa) models because requiring no cross loadings is an unrealistic expectation of items from many psychological instruments [@Marsh2014].\index{cross loading}
The correlations between factors tend to be positively biased when fitting [CFA](#cfa) models without cross loadings, which leads to challenges in using [CFA](#cfa) to establish [discriminant validity](#discriminantValidity) [@Marsh2014].\index{cross loading}
...
We provide examples of ESEM in Section \@ref(esemModel).

#### 4. How Many Factors to Retain {#factorsToRetain}

A goal of [factor analysis](#factorAnalysis) and [PCA](#pca) is simplification or parsimony, while still explaining as much variance as possible.\index{parsimony}
...
That is, for the sake of simplicity, parsimony, and data reduction, you should take any factors that explain more than a single variable would explain.\index{parsimony}
...
If you are using maximum likelihood [factor analysis](#factorAnalysis), you can compare the fit of various models with model fit criteria to see which model fits best for its parsimony.\index{parsimony}
...
*simple structure* means that every variable loads perfectly on one and only one factor, as operationalized by a matrix of factor loadings with values of one and zero and nothing else.\index{simple structure}
An example of a factor matrix that follows simple structure is depicted in Figure \@ref(fig:simpleStructure).\index{simple structure}
...
An example of a measurement model that follows simple structure is depicted in Figure \@ref(fig:factorSolutionSimpleStructure).\index{simple structure}
...
However, pure simple structure only occurs in simulations, not in real-life data.\index{simple structure}
...
In this example, the measurement model does not show simple structure because the items have cross loadings—that is, the items load onto more than one factor.\index{cross loading}
The cross loadings make it difficult to interpret the factors, because all of the items load onto on the all of the factors, so the factors are not very distinct from each other, which makes it difficult to interpret what the factors mean.\index{cross loading}
...
As a result of the challenges of intepretability caused by crossloadings, factor rotations are often performed.\index{cross loading}
...
An orthogonal rotation (e.g., varimax) can help with simplicity of interpretation because it seeks to yield simple structure without cross loadings.\index{simple structure}\index{cross loading}
Cross loadings are instances where a variable loads onto multiple factors.\index{cross loading}
My recommendation would always be to use an orthogonal rotation if you have reason to believe that finding simple structure in your data is possible; otherwise the factors are extremely difficult to interpret—what exactly does a cross loading even mean?\index{cross loading}
...
Here are correlation matrices of various types using the `cor.table()` from the `petersenlab` package [@R-petersenlab]:\index{petersenlab}
...
Based on the rule to keep factors whose eigenvalues are greater than zero and based on the parallel test, we would keep three factors.
However, based on the Cattell scree test (as operationalized by the optimal coordinates and acceleration factor), we would keep one factor.
Therefore, interpretability of the factors would be important for deciding between whether to keep one, two, or three factors.

A scree plot from a parallel analysis is in Figure \@ref(fig:screePlotEFAparallel).
...
A scree plot from EFA is in Figure \@ref(fig:screePlotEFA). 
...
In the example with orthogonal rotation below, the VSS criterion is highest with 3 or 4 factors.
A three-factor solution is supported by the lowest BIC, whereas a four-factor solution is supported by the lowest SABIC.

A VSS plot is in Figure \@ref(fig:vssOrthogonal).
...
Multiple VSS-related fit indices are in Figure \@ref(fig:vssComplexOrthogonal).
...
In the example with oblique rotation below, the VSS criterion is highest with 3 factors.
A three-factor solution is supported by the lowest BIC.

A VSS plot is in Figure \@ref(fig:vssOblique).
...
Multiple VSS-related fit indices are in Figure \@ref(fig:vssComplexOblique).
...
In the example with no rotation below, the VSS criterion is highest with 3 or 4 factors.
A three-factor solution is supported by the lowest BIC, whereas a four-factor solution is supported by the lowest SABIC.

A VSS plot is in Figure \@ref(fig:vssUnrotated).
...
Multiple VSS-related fit indices are in Figure \@ref(fig:vssComplexUnrotated).
...
A scree plot from a model with oblique rotation is in Figure \@ref(fig:screePlotPsychOrthogonal).
...
A scree plot based on factor loadings from `lavaan` is in Figure \@ref(fig:screePlotLavaanOrthogonal).
When the factors are uncorrelated (orthogonal rotation), the eigenvalue for a factor is calculated as the sum of squared factor loadings across all items, as described in Section \@ref(pca).
...
A biplot is in Figure \@ref(fig:biplotOrthogonal).
...
A factor plot is in Figure \@ref(fig:factorPlotOrthogonal).
...
A factor diagram is in Figure \@ref(fig:factorDiagramOrthogonal).
...
A pairs panel plot is in Figure \@ref(fig:pairsPlotOrthogonal).
...
A correlation plot is in Figure \@ref(fig:correlationPlotOrthogonal).
...
A biplot is in Figure \@ref(fig:biplotOblique).
...
A factor plot is in Figure \@ref(fig:factorPlotOblique).
...
A factor diagram is in Figure \@ref(fig:factorDiagramOblique).
...
A pairs panel plot is in Figure \@ref(fig:pairsPlotOblique).
...
A correlation plot is in Figure \@ref(fig:correlationPlotOblique).
...
As would be expected, the factor scores from the [CFA](#cfa) model are highly correlated with the factor scores from the [EFA](#efa) model.
...
A path diagram of the model generated using the `semPlot` package [@R-semPlot] is in Figure \@ref(fig:cfaPathDiagram).
...
A path diagram of the model generated using the `semPlot` package [@R-semPlot] is in Figure \@ref(fig:cfaPathDiagramClass1).
...
A path diagram of the model generated using the `semPlot` package [@R-semPlot] is in Figure \@ref(fig:cfaPathDiagramClass2).
...
Path diagrams of the models generated using the `semPlot` package [@R-semPlot] are in Figures \@ref(fig:cfaPathDiagramClass3A) and \@ref(fig:cfaPathDiagramClass3B).
...
Path diagrams of the models generated using the `semPlot` package [@R-semPlot] are in Figures \@ref(fig:cfaModel4AFit), \@ref(fig:cfaModel4BFit), \@ref(fig:cfaModel4CFit), and \@ref(fig:cfaModel4DFit).
...
As estimated below, the general factor is orthogonal to the specific factors.
Some modified versions of the bifactor allow the specific factors to inter-correlate (in addition to estimating the general factor).
...
A path diagram of the model generated using the `semPlot` package [@R-semPlot] is in Figure \@ref(fig:bifactorModelPathDiagram).
...
A path diagram of the model generated using the `semPlot` package [@R-semPlot] is in Figure \@ref(fig:mtmmCFApathDiagram).
...
To specify an [ESEM](#efa-cfa-esem) model, you use the factor loadings from an [EFA](#efa) model as the starting values in a [structural equation model](#sem).
...
In [ESEM](#efa-cfa-esem), you specify one anchor item for each latent factor.
You want the anchor item to be an item that has a strong loading on one latent factor and to have weak loadings on every other latent factor.
The cross loadings for the anchor item are fixed to the respective loadings from the original [EFA](#efa) model.\index{cross loading}
I specify an anchor item for each latent factor below:

```{r}
anchors <- c(f1 = "x3", f2 = "x5", f3 = "x7")
```

The `petersenlab` package [@R-petersenlab] includes the `make_esem_model()` function that creates `lavaan` syntax for an [ESEM](#efa-cfa-esem) model from the factor loadings of an [EFA](#efa) model, as adapted from Mateus Silvestrin:\index{petersenlab} https://msilvestrin.me/post/esem
...
A nested model comparison indicates that the [ESEM](#efa-cfa-esem) model fits significantly better than the [CFA](#cfa) model.
...
A path diagram of the model generated using the `semPlot` package [@R-semPlot] is in Figure \@ref(fig:esemModelPathDiagram).
...
Another way to fit a [ESEM](#efa-cfa-esem) model in `lavaan` is to fit exploratory factors, confirmatory factors, and structural components in the same model.
...
A path diagram of the model generated using the `semPlot` package [@R-semPlot] is in Figure \@ref(fig:esemModelPathDiagram2).
...
That is, [PCA](#pca) seeks to identify the smalles set of components that explain the most variance in the variables.
...
A scree plot based on parallel analysis is in Figure \@ref(fig:screePlotPCAParallel).

The number of components to keep would depend on which criteria one uses.
Based on the rule to keep components whose eigenvalues are greater than one and based on the parallel test, we would keep three factors.
However, based on the Cattell scree test (as operationalized by the optimal coordinates and acceleration factor), we would keep one or three factors.
Therefore, interpretability of the factors would be important for deciding between whether to keep one, two, or three factors.
...
A scree plot is in Figure \@ref(fig:screePlotPCA).
...
In the example with orthogonal rotation below, the VSS criterion is highest with 3 or 4 components.

A VSS plot of PCA with orthogonal rotation is in Figure \@ref(fig:vssOrthogonalPCA).

```{r vssOrthogonalPCA, out.width = "100%", fig.align = "center", fig.cap = "Very Simple Structure Plot in Principal Component Analysis."}
vss(
  HolzingerSwineford1939[,vars],
  rotate = "varimax",
  fm = "pc")
...
A VSS plot of PCA with oblique rotation is in Figure \@ref(fig:vssObliquePCA).

```{r vssObliquePCA, out.width = "100%", fig.align = "center", fig.cap = "Very Simple Structure Plot in Principal Component Analysis."}
vss(
  HolzingerSwineford1939[,vars],
  rotate = "oblimin",
  fm = "pc")
...
A VSS plot of PCA with no rotation is in Figure \@ref(fig:vssUnrotatedPCA).

```{r vssUnrotatedPCA, out.width = "100%", fig.align = "center", fig.cap = "Very Simple Structure Plot in Principal Component Analysis."}
vss(
  HolzingerSwineford1939[,vars],
  rotate = "none",
  fm = "pc")
...
A biplot is in Figure \@ref(fig:pcaBiplot).
...
A pairs panel plot is in Figure \@ref(fig:pairsPlotOrthogonalPCA).
...
A correlation plot is in Figure \@ref(fig:correlationPlotOrthogonalPCA).
...
A pairs panel plot is in Figure \@ref(fig:pairsPlotObliquePCA).
...
A correlation plot is in Figure \@ref(fig:correlationPlotObliquePCA).
