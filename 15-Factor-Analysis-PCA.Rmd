...
In reality, part of the unique variance is error and part of it is specific variance—systematic variance unique to the item, which may reflect either a unique facet of the construct or [systematic error](#systematicError) such as [method bias](#methodBias).\index{factor analysis!decisions}\index{factor analysis}\index{measurement error}\index{systematic error}\index{method bias}
The common variance of an item is called its communality, which is the proportion of the item's variance that is explained by the latent factors (i.e., the $R^2$ of an item).\index{factor analysis!decisions}\index{factor analysis}\index{structural equation modeling!common variance}\index{communality}
...
Oblique rotation allows for factors to be correlated and yields axes with less an angle of less than 90 degrees.\index{factor analysis!decisions}\index{factor analysis}\index{principal component analysis}\index{rotation}\index{oblique rotation}
