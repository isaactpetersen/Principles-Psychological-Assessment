...
However, [random error](#randomError) would make it more difficult to make an accurate prediction at the individual person-level.
...
```{r, include = FALSE}
trueAssociation_sim <- 
  cor.test(x = mydataValidity$predictor,
...
```{r}
cor.test(x = mydataValidity$predictorObservedSEM,
...
```{r, include = FALSE}
observedSEM_parameters <- parameterEstimates(observedSEM_fit,
                                             standardized = TRUE)

observedBeta <- observedSEM_parameters[which(observedSEM_parameters$lhs == "criterionObservedSEM" & observedSEM_parameters$rhs == "predictorObservedSEM"), "std.all"]
```
...
```{r, include = FALSE}
disattenuationSEM_parameters <- parameterEstimates(disattenuationSEM_fit,
                                                   standardized = TRUE)

trueBeta <- disattenuationSEM_parameters[which(disattenuationSEM_parameters$lhs == "criterionLatent" & disattenuationSEM_parameters$rhs == "predictorLatent"), "std.all"]
```
