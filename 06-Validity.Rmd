...
An example of predictive validity would be examining the predictive association between children's scores on an academic achievement test in first grade and their eventual academic outcomes years later.\index{validity!criterion}\index{validity!predictive}
...
```{r commonCause1, out.width = "50%", fig.align = "center", fig.cap = "Hypothesized Causal Effect Based on an Observed Association Between $X$ and $Y$, Such That $X$ Causes $Y$.", echo = FALSE}
...
```{r commonCause2, out.width = "50%", fig.align = "center", fig.cap = "Reverse (Opposite) Direction of Effect from the Hypothesized Effect, Where $Y$ Causes $X$.", echo = FALSE}
...
```{r commonCause3, out.width = "50%", fig.align = "center", fig.cap = "Confounded Association Between $X$ and $Y$ due to a Common Cause, $Z$.", echo = FALSE}
...
For instance, we have made great strides in the last decades when it comes to more accurate weather forecasts, including extreme weather events like hurricanes.\index{causality}
...
Indeed, we have misgivings calling them theories because they do not meet the traditional scientific standard for a theory.\index{theory}
...
```{r empiricism, out.width = "25%", fig.align = "center", fig.cap = "Conceptual Depiction of Empiricism.", echo = FALSE}
...
As described here (https://www.henseler.com/htmt.html; archived at https://perma.cc/A9DU-WZWQ) based on evidence from @Voorhees2016,\index{multitrait-multimethod matrix}\index{validity!construct}\index{validity!discriminant}
...
A multitrait-multimethod matrix correlation matrix is presented in Table \@ref(tab:mtmmCorrelationPDF).\index{multitrait-multimethod matrix}\index{validity!construct}

```{r mtmmCorrelationPDF, echo = FALSE}
MTMM_corMatrix <- cor(MTMM_data) %>% 
  specify_decimal(., 2) %>% 
  suppressLeadingZero(.) %>% 
  matrix(., nrow = ncol(MTMM_data), ncol = ncol(MTMM_data)) %>% 
  data.frame()

names(MTMM_corMatrix) <- names(MTMM_data)
row.names(MTMM_corMatrix) <- names(MTMM_data)
  
MTMM_corMatrix[upper.tri(MTMM_corMatrix)] <- ""

kable(
  MTMM_corMatrix,
  caption = "Multitrait-Multimethod Correlation Matrix.",
  format = "latex",
  booktabs = TRUE,
...
###### MTMM in confirmatory factor analysis {#mtmmCFA-validity}
...
```{r mtmmCFA, out.width = "100%", fig.align = "center", fig.cap = "Multitrait-Multimethod Model in Confirmatory Factor Analysis with Three Constructs (Internalizing, Externalizing, and Thought-Disordered Problems) and Three Methods (Mother-, Father-, and Teacher-report).", fig.scap = "Multitrait-Multimethod Model in Confirmatory Factor Analysis with Three Constructs and Three Methods.", echo = FALSE}
...
```{r reliabilityVsValidity2, out.width = "100%", fig.align = "center", fig.cap = "Traditional Depiction of Reliability (Consistency) Versus Validity (Accuracy).", fig.scap = "Traditional Depiction of Reliability Versus Validity.", echo = FALSE}
...
```{r reliabilityVsValidity1, out.width = "100%", fig.align = "center", fig.cap = "Depiction of Reliability Versus Validity, While Distinguishing Between Validity (Accuracy) at the Person Versus Group Level.", fig.scap = "Depiction of Reliability Versus Validity, While Distinguishing Between Validity at the Person Versus Group Level.", echo = FALSE}
...
Parameter estimates from the model are shown in Table \@ref(tab:observedAssociationSEM).\index{observed association}\index{structural equation modeling}
...
numColumns <- ncol(observedSEM_parameters)

digits <- rep(3, length.out = numColumns)
for (j in seq_len(numColumns)) {
  if (is.numeric(observedSEM_parameters[, j])) observedSEM_parameters[, j] = sprintf(sprintf('$%%.%sf$', digits[j]), observedSEM_parameters[, j])
}
```

```{r observedAssociationSEM, echo = FALSE}
kable(
  observedSEM_parameters,
  caption = "Parameter Estimates of Observed Association in Structural Equation Model.",
  format = "latex",
  booktabs = TRUE,
  align = c("l","l","l","r","r","r","r","r","r","r","r","r"),
  escape = FALSE) %>%
  kable_styling(latex_options = "scale_down")
```
...
Parameter estimates from the model are presented in Table \@ref(tab:trueAssociationSEM).\index{measurement error!disattenuation of association}\index{true association}\index{observed association}\index{structural equation modeling}
...
numColumns <- ncol(disattenuationSEM_parameters)

digits <- rep(3, length.out = numColumns)
for (j in seq_len(numColumns)) {
  if (is.numeric(disattenuationSEM_parameters[, j])) disattenuationSEM_parameters[, j] = sprintf(sprintf('$%%.%sf$', digits[j]), disattenuationSEM_parameters[, j])
}

disattenuationSEM_parameters[disattenuationSEM_parameters == "$NA$"] <- "NA"
```

```{r trueAssociationSEM, echo = FALSE}
kable(
  disattenuationSEM_parameters,
  caption = "Parameter Estimates of Disattenuated Association in Structural Equation Model.",
  format = "latex",
  booktabs = TRUE,
  linesep = "",
  align = c("l","l","l","r","r","r","r","r","r","r","r","r"),
