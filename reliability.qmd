...
Around 95% of observed scores would fall within $\pm 2$ SEMs of the true score (or, more precisely, within $\pm `r qnorm(.975)`$ SEMs of the true score).\index{reliability!standard error of measurement}\index{true score}
Using the SEM, we can compute a confidence interval that represents a range of plausible values for the person's true score (with a specified confidence level, e.g., 95%), given [measurement error](#sec-measurementError).
In other words, if we repeatedly observed this person and constructed intervals as the observed score $\pm 2$ SEMs, about 95% of those intervals would contain the true score.\index{reliability!standard error of measurement}\index{true score}\index{observed score}
...
So if a person gets a score of 15 on the measure, it is likely that their true score falls within 11–19.\index{reliability!standard error of measurement}\index{true score}\index{observed score}
The 95% confidence interval represents that, if we repeatedly observed this person under identical conditions and constructed a confidence interval each time, 95% of those intervals would contain the person's true score.\index{reliability!standard error of measurement}\index{true score}
However, the confidence interval does not indicate that there is a 95% probability that their true score falls within the interval.\index{reliability!standard error of measurement}\index{true score}
...
For instance, Anscombe's quartet includes four sets of data that have nearly identical basic descriptive statistics (see Tables [-@tbl-anscombe] and [-@tbl-anscombeStats]), including the same bivariate correlation, yet have very different distributions and whose association takes very different forms (see @fig-anscombeQuartetPDF).\index{correlation}

```{r}
#| label: fig-anscombeQuartetPDF
#| fig-cap: "Anscombe's Quartet."
#| fig-alt: "Anscombe's Quartet."
#| fig-width: 6
#| fig-height: 6
...
In sum, the correlation coefficient can be greatly impacted by the extent of variability in the data, differences in the shapes of the two distributions, nonlinearity, outliers, characteristics of the sample, and measurement error [@Goodwin2006].
These factors tend to have greater impact when the sample size is smaller (compared to when the sample size is larger).
The Pearson correlation coefficient tends to be smaller when:
...
- the association differs across subgroups of the sample (i.e., moderation)
- the data contain [random measurement error](#sec-randomError)
...
- [systematic measurement error](#sec-systematicError) (e.g., common [method bias](#sec-methodBias))
...
The precise value is $`r qnorm(.975)`$ (i.e., `qnorm(.975)`) times the standard deviation of the within-subject differences between the two measurements.\index{reliability!test–retest}\index{reliability!test–retest!coefficient of repeatability}
...
However, some researchers round $`r qnorm(.975)`$ to 1.96 or to 2 for simplicity.
...
Although establishing internal consistency can help ensure that items in the measure are interrelated, you do not want to select items just based on internal consistency [@Revelle2024].\index{reliability!internal consistency}
...
For example, based on arguments by Cattell [-@Cattell1973, p. 359], Cooper [-@Cooper2023, p. 159] provides a conceptual example of how items could show zero correlation between them (and thus not be unidimensional or show internal consistency) but still be useful for assessing a construct like depression—because the scale should assess all of the various facets of the construct, which could lead an item that assesses a given facet (e.g., sad mood) to be only weakly associated with items that assess other facets (e.g., weight/appetite changes).
Researchers have thus argued that the field has focused too much on internal consistency to the exclusion of other important psychometric considerations such as [utility](#sec-validity) and [predictive validity](#sec-predictiveValidity) [@Revelle2024].
...
#### Maximal Reliability {#sec-maximalReliability}

Maximal reliability is a model-based approach to estimating internal consistency.
Maximal reliability is the highest possible internal consistency reliability estimate that can be achieved from a set of items by applying optimal weights for each item (i.e., an optimal linear combination) rather than equal weights for each item.
Coefficient *H* is a common index of maximal reliability.

Maximal reliability was estimated using a [confirmatory factor analysis](#sec-cfa) model using the `lavaan` [@R-lavaan] and `semTools` [@R-semTools] packages.\index{reliability!internal consistency}\index{reliability!internal consistency!maximal reliability}
The [confirmatory factor analysis](#sec-cfa) model was fit above in @sec-averageVarianceExtracted.

```{r}
semTools::maximalRelia(latentFactorModel_fit)
```
...
As depicted, a variable would not be expected to be more strongly associated with other variables (i.e., the correlation between the indices) than it is with itself (i.e., its reliability).
...
Similar challenges arise with ratio scores (i.e., scores that represent a ratio of two variables) because both variables included in the ratio calculation include error [@Cronbach1941; @Arndt1991].\index{reliability!product term}
...
The reliability of a product term can also be used to find the reliability of ratio scores (e.g., $\frac{A}{B}$) because a ratio can be conceived of as a product term where one variable (e.g., $A$) is multiplied by the reciprocal of another variable (e.g., $\frac{1}{B}$).\index{reliability!product term}
...
A confidence interval indicates a range of plausible values for the person's true score (with a specified confidence level, e.g., 95%), given [measurement error](#sec-measurementError).\index{reliability!standard error of measurement}\index{confidence interval}
The 95% confidence interval represents that, if we repeatedly observed this person under identical conditions and constructed a confidence interval each time (as observed score $\pm `r qnorm(.975)` \ \text{SEMs}$), about 95% of those intervals would contain the person's true score.\index{reliability!standard error of measurement}\index{confidence interval}\index{true score}\index{observed score}
...
Examples of data from nonindependent units include nested data, multiple membership data, and cross-classified data, as depicted in @fig-nestedDataStructures.\index{mixed model}\index{nonindependent data}
...
When sample sizes are large, ([random](#sec-randomError)) [measurement error](#sec-measurementError) tends to weaken effect sizes.\index{reliability!problems of low reliability}\index{measurement error}\index{measurement error!attenuation of association}
However, when sample sizes are small to modest, [random](#sec-randomError) [measurement error](#sec-measurementError) can actually lead to over-estimation of effect sizes as well as inaccurate estimates of the sign of the effect (positive or negative), due to the statistical significance filter [@Loken2017], as described in @sec-assessmentReplicationCrisis.\index{reliability!problems of low reliability}\index{statistical significance!filter}
[Within-person systematic error](#sec-withinPersonSystematicError) (e.g., common [method bias](#sec-methodBias)) can also lead to over-estimation of effect sizes.
...
## Session Info {#sec-sessionInfo-reliability}
