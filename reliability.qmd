...
![](images/classicalTestTheory.png){width=100% fig-alt="Classical Test Theory Formula in a Path Diagram."}
...
Then, the researcher can see what the associations would be when disattenuated for unreliability, as described in Section \@ref(effectOfMeasurementErrorOnAssociations) of the chapter on [validity](#sec-validity).\index{measurement error!disattenuation of association}
...
The SEM is equivalent to the standard deviation of [measurement error](#sec-measurementError) ($e$) [@Lek2018a], as in Equation \@ref(eq:standardErrorOfMeasurementAlternative):\index{reliability!standard error of measurement}
...
For more information see Section \@ref(reliabilityParadox) on the reliability paradox in the chapter on [cognitive](#sec-cognition) assessment.\index{reliability!paradox}
...
The [`petersenlab`](https://github.com/DevPsyLab/petersenlab) package [@R-petersenlab] contains the `repeatability()` function that estimates the repeatability of a measure's scores and generates a [Bland-Altman plot](#sec-blandAltmanPlot) in Figure \@ref(fig:repeatability).\index{reliability!test–retest}\index{reliability!test–retest!coefficient of repeatability}\index{reliability!test–retest!Bland-Altman plot}\index{petersenlab package}
...
#### Ordinal Data {#interraterReliability-ordinal}
...
### Reliability of a Product Term {#productTerm-reliability}
...
The reliability of a product term from mean-centered variables can be estimated using Equation \@ref(eq:reliabilityOfProductTerm) [@Busemeyer1983; @Aguinis2017]:\index{reliability!product term}\index{moderation}\index{moderation@\igobble|seealso{interaction}}

$$
r_{x*y, x*y} = \frac{r_{xx} r_{yy} + r^2_{xy}}{1 + r^2_{xy}}
$$ {#eq-reliabilityOfProductTerm}

If the two variables (`x` and `y`) are uncorrelated, the reliability of the product term simplifies to the product of their reliabilities, as in Equation \@ref(eq:reliabilityOfProductTermUncorrelated):\index{reliability!product term}\index{moderation}\index{moderation@\igobble|seealso{interaction}}
...
As discussed in Section \@ref(measurementError), a given error can be either [systematic](#sec-systematicError) or [random](#sec-randomError).
...
As we describe in greater detail in the chapter on [validity](#sec-validity) in Section \@ref(effectOfMeasurementErrorOnAssociations), ([random](#sec-randomError)) [measurement error](#sec-measurementError) weakens (or attenuates) the association between variables.\index{measurement error!random error}\index{measurement error!attenuation of association}
...
In the context of examining the association between test and retest (i.e., [test–retest reliability](#sec-testRetest-reliability)), the estimate of the stability of the construct (i.e., true test–retest association) is attenuated to the extent that the measure is unreliable, as described in Equation \@ref(eq:attenuationRetestReliability):\index{reliability!test–retest}\index{measurement error!attenuation of association}
...
- use [confirmatory factor analysis](#sec-cfa) or [structural equation modeling](#sec-sem), as described in Section \@ref(semMethodBias)\index{method bias!ways to address}\index{factor analysis!confirmatory}
...
As we described in the chapter on [validity](#sec-validity) in Section \@ref(reliabilityVsValidity), reliability constrains [validity](#sec-validity).\index{reliability!problems of low reliability}\index{validity!invalid}
...
In the next chapter (Chapter \@ref(validity)), we discuss [validity](#sec-validity) of measurement.\index{validity}
