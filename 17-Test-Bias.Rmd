...
Test bias would also exist if scores on the Scholastic Aptitude Test (SAT) underestimate women's grades in college, for instance.\index{bias}\index{measurement error!systematic error}\index{bias!types of}
...
For instance, consider a [2-by-2 confusion matrix](#confusionMatrix) used for the standard prediction problem.\index{confusion matrix}
...
```{r jobSelection, out.width = "80%", fig.align = "center", fig.cap = "2-by-2 Confusion Matrix for Job Selection. TP = true positive; TN = true negative; FP = false positive; FN = false negative.", fig.scap = "2-by-2 Confusion Matrix for Job Selection.", echo = FALSE}
...
The quadrants reflect the cutoffs (i.e., thresholds) imposed from the 2-by-2 confusion matrix.\index{confusion matrix}\index{bias!predictive}
...
```{r jobSelection2, fig.height = 8, fig.align = "center", fig.cap = "2-by-2 Confusion Matrix for Job Selection in the Form of a Graph with Predicted Performance on the x-Axis and Actual Job Performance on the y-Axis. TP = true positive; TN = true negative; FP = false positive; FN = false negative.", fig.scap = "2-by-2 Confusion Matrix for Job Selection in the Form of a Graph.", echo = FALSE}
plot.new()
...
When the distributions have similar slopes, intercept differences suggest that the measure systematically under- or overestimates group performance relative to the person's ability.\index{bias!predictive!different intercepts}
...
Figure \@ref(fig:testBias2) indicates that the measure systematically underestimates the job performance of the minority group.\index{bias!predictive!different intercepts}

Performance among members of a minority group could be under- or overestimated.\index{bias!predictive!different intercepts}
For example, historically, women's grades in math and engineering classes tended to be underestimated by the Scholastic Aptitude Test [(SAT); @Clark1984].\index{bias!predictive!different intercepts}
...
For example, women's physical strength and endurance is overestimated based on physical ability tests [@Sackett1994].\index{bias!predictive!different intercepts}
In addition, overestimation of African Americans' and Hispanics' school and job performance has been observed based on cognitive ability tests [@Cole1981; @Reynolds2012; @Sackett1994; @Sackett2008].\index{bias!predictive!different intercepts}
...
The overestimation of African Americans' and Hispanics' school and job performance may be due to [measurement error](#measurementError) in the tests.\index{bias!predictive!different intercepts}
...
In cases of different intercepts and slopes, there is both differential [validity](#validity) (because the regression lines have different slopes), as well as varying under- and overestimation of groups' performance at particular scores.\index{bias!predictive!different intercepts and slopes}\index{validity}
...
In instances of different intercepts and slopes predictive bias, a measure can simultaneously overestimate and underestimate a person's ability at different test scores.\index{bias!predictive!different intercepts and slopes}
For instance, a measure can underestimate a person's ability at higher test scores and can overestimate a person's ability at lower test scores.\index{bias!predictive!different intercepts and slopes}
...
So, if women show higher scores on a depression measure compared to men, we would also expect them to show similar elevations on each item (e.g., sleep loss).\index{bias!test structure}
...
A construct may include some content facets in one group, but it may include different content facets in another group, as depicted in Figure \@ref(fig:testBias5).\index{bias!test structure}\index{validity!content}
...
One instance of test bias is that, historically, women's grades in math and engineering classes tended to be underestimated by the Scholastic Aptitude Test [(SAT); @Clark1984].\index{bias!examples}
...
Thus, the performance of one group is not overestimated relative to the performance of the other group.\index{fairness}
...
- physical ability tests, which produce substantial adverse impact against women (despite overestimation of women's performance),\index{adverse impact}
- cognitive ability tests, which produce substantial impact against some ethnic minority groups, especially Black and Hispanic people (despite overestimation of Black and Hispanic people's performance), even though cognitive ability tests tend to be among the strongest predictors of job performance [@Sackett2008; @Schmidt1981], and\index{adverse impact}
...
Within-group norming converts an individual's score to [standardized scores](#standardizedScores) (e.g., T scores) or [percentiles](#percentileRanks) within one's own group.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}\index{data!standardized}
Then, the people are selected based on the highest standard scores across groups.
Within-group norming is used to correct for [predictive bias](#predictiveBias) [differences in slopes](#differentSlopes) between groups.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}\index{bias!predictive!different slopes}
...
However, [percentile scores](#percentileRanks) will consistently eliminate [adverse impact](#adverseImpact).\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!within-group norming}\index{data!percentile rank}\index{adverse impact}
...
So that we do not overestimate small score differences, scores within the same band are seen as equivalent—and the order of selection within the band can be modified depending on selection goals.\index{bias!correcting for}\index{bias!correcting for!score adjustment}\index{bias!correcting for!banding}
...
numColumns <- ncol(difDropItems)

digits <- c(0,0,2,2,2,2,2,0,3)
for (j in seq_len(numColumns)) {
  if (is.numeric(difDropItems[, j])) difDropItems[, j] <- paste("$", gsub(" ", "", format(round(difDropItems[, j], digits = digits[j]), nsmall = digits[j]), fixed = TRUE), "$", sep = "")
}

row.names(difDropItems) <- gsub("_", "\\\\_", row.names(difDropItems))
...
difDropItems %>% 
  ...
  col.names = c("Item","Groups","Converged","AIC","SABIC","HQ","BIC","$\\chi^2$","$df$","$p$"),
  escape = FALSE) %>%
...
digits <- c(0,0,2,2,2,2,2,0,3)
for (j in seq_len(numColumns)) {
  if (is.numeric(difDropItemsDiscrimination[, j])) difDropItemsDiscrimination[, j] <- paste("$", gsub(" ", "", format(round(difDropItemsDiscrimination[, j], digits = digits[j]), nsmall = digits[j]), fixed = TRUE), "$", sep = "")
}

row.names(difDropItemsDiscrimination) <- gsub("_", "\\\\_", row.names(difDropItemsDiscrimination))
...
digits <- c(0,0,2,2,2,2,2,0,3)
for (j in seq_len(numColumns)) {
  if (is.numeric(difDropItemsSeverity[, j])) difDropItemsSeverity[, j] <- paste("$", gsub(" ", "", format(round(difDropItemsSeverity[, j], digits = digits[j]), nsmall = digits[j]), fixed = TRUE), "$", sep = "")
}

row.names(difDropItemsSeverity) <- gsub("_", "\\\\_", row.names(difDropItemsSeverity))
...
numColumns <- ncol(itemLevelDIF)

digits <- rep(2, length.out = numColumns)
for (j in seq_len(numColumns)) {
  if (is.numeric(itemLevelDIF[, j])) itemLevelDIF[, j] <- paste("$", gsub(" ", "", format(round(itemLevelDIF[, j], digits = digits[j]), nsmall = digits[j]), fixed = TRUE), "$", sep = "")
}

row.names(itemLevelDIF) <- gsub("_", "\\\\_", row.names(itemLevelDIF))
...
Estimates of [differential item functioning](#dif) are given in Table \@ref(tab:itemLevelDIF).\index{item response theory!differential item functioning}
...
Item response category characteristic curves by sex for item 4 are shown in Figure \@ref(fig:ircSex4).\index{item response theory!differential item functioning}\index{item response theory!item response category characteristic curve}
...
Plots of item [information](#irtReliability) functions by sex for item 6 are shown in Figure \@ref(fig:itemInfo6).\index{item response theory!differential item functioning}\index{item response theory!item information curve}
...
Plots of expected item scores by sex for items 4 and 6 are shown in Figures \@ref(fig:itemScore4) and \@ref(fig:itemScore6), respectively.\index{item response theory!differential item functioning}
...
numColumns <- ncol(difDropItemsNew)

digits <- c(0,0,2,2,2,2,2,0,3)
for (j in seq_len(numColumns)) {
  if (is.numeric(difDropItemsNew[, j])) difDropItemsNew[, j] <- paste("$", gsub(" ", "", format(round(difDropItemsNew[, j], digits = digits[j]), nsmall = digits[j]), fixed = TRUE), "$", sep = "")
}

difDropItemsNew[difDropItemsNew == "$NaN$"] <- "NaN"

row.names(difDropItemsNew) <- gsub("_", "\\\\_", row.names(difDropItemsNew))
...
Estimates of [DIF](#dif) after resolving item 5 are given in Table \@ref(tab:itemLevelDIFAfterResolving).\index{item response theory!differential item functioning}
...
In addition to nested chi-square difference tests ($\chi^2_{\text{diff}}$), we also demonstrate permutation procedures for testing measurement invariance, as described by @Jorgensen2018.\index{structural equation modeling!measurement invariance}\index{factor analysis!confirmatory}
...
A path diagram of the model is in Figures 15.33 (group 1) and 15.34 (group 2).\index{structural equation modeling!measurement invariance!configural}\index{structural equation modeling!path diagram}

```{r measurementInvariancePathDiagram, out.width = "100%", fig.align = "center", fig.cap = "Configural Invariance Model in Confirmatory Factor Analysis (1 = Group 1; 2 = Group 2).", fig.scap = "Configural Invariance Model in Confirmatory Factor Analysis."}
semPaths(
  configuralInvarianceModel_fit,
  what = "est",
  layout = "tree2",
...
Thus, we would want to identify which items show non-invariant intercepts across groups.\index{structural equation modeling!factor loading}\index{structural equation modeling!intercept}
...
So, we can use the metric invariance model as the baseline model (in which item intercepts are allowed to differ across groups), and iteratively add constraints to identify which items show non-invariant intercepts across groups.\index{structural equation modeling!intercept}\index{structural equation modeling!measurement invariance}\index{structural equation modeling!measurement invariance!metric}
...
So, we can use the scalar invariance model as the baseline model (in which item intercepts are constrained to be the same across groups), and we can iteratively drop constraints to identify the items for which measurement invariance becomes established (relative to the metric invariance model which allows item intercepts to differ across groups), and therefore, identifies which items show non-invariant intercepts across groups.\index{structural equation modeling!measurement invariance}\index{structural equation modeling!measurement invariance!scalar}\index{structural equation modeling!intercept}

#### Next Steps {#measurementInvarianceNextSteps}

After identifying which items show measurement non-invariance, we would evaluate the measurement non-invariance according to theory and effect sizes.
