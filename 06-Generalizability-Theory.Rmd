...
Using such a G-theory model, we can determine the extent to which scores on a measure generalize to other conditions, measures, etc. In sum, G-theory can be a useful way of estimating the degree of [reliability](#reliability) and [validity](#validity) of a measure's scores in the same model.
...
The generalizability coefficient is analogous to the [reliability](#reliability) coefficient in CTT. It divides the estimated person variance component (the universe score variance) by the estimated observed-score variance (with some adjustment for the number of observations). It uses relative error variance, so it is relevant when decisions are made based on relative (norm-referenced) scores (e.g., percentiles).
