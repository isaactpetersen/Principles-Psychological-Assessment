...
For example, CTT assumes that that all error is [random](#unsystematicError).
...
In other words, variance in a reliable measure should mostly be due to person variance rather than variance as a function of items, occasion, raters, methods, or other factors.
The generalizability coefficient uses relative error variance, so it is relevant when decisions are made based on relative (norm-referenced) scores (e.g., percentiles).
...
It uses absolute error variance, so it is relevant when decisions are made based on absolute (e.g., raw) scores.
