...
The generalizability coefficient is analogous to the reliability coefficient in CTT. It divides the estimated person variance component (the universe score variance) by the estimated observed-score variance (with some adjustment for the number of items). It uses relative error variance, so it is relevant when decisions are made based on relative (norm-referenced) scores (e.g., percentiles).
