...
# Scores and Scales {#sec-scoresScales}

Assessments yield information.
The information is encoded in scores or in other [types of data](#sec-dataTypes).\index{data!type}
It is important to consider the different [types of data](#sec-dataTypes) because the [types of data](#sec-dataTypes) restrict what options are available to analyze the data.\index{data!type}

## Getting Started {#sec-gettingStarted-scoresScales}

### Load Libraries {#sec-loadLibraries-scoresScales}
...
### Prepare Data {#sec-prepareData-scoresScales}

#### Simulate Data {#sec-simulateData-scoresScales}
...
#### Add Missing Data {#sec-addMissingData-scoresScales}
...
There are four general data types: [nominal](#sec-nominalData), [ordinal](#sec-ordinalData), [interval](#sec-intervalData), and [ratio](#sec-ratioData).\index{data!type}
...
For instance, parametric statistical analysis (e.g., *t* test, analysis of variance [ANOVA], and linear regression) assumes that data are [interval](#sec-intervalData) or [ratio](#sec-ratioData).\index{data!type}

### Nominal {#sec-nominalData}
...
Most data in psychology are ordinal data even though they are often treated as if they were [interval](#sec-intervalData) data.\index{data!ordinal}\index{data!interval}

### Interval {#sec-intervalData}
...
## Score Transformation {#sec-scoreTransformation}
...
For instance, transformations change the structure of correlations between variables.\index{data!transformation}
...
An example histogram of raw scores is in @fig-rawScores.\index{data!raw}\index{histogram}

```{r}
#| label: fig-rawScores
#| fig-cap: "Histogram of Raw Scores."
#| fig-alt: "Histogram of Raw Scores."
#| code-fold: true
...
### Norm-Referenced Scores {#sec-norm}
...
Pros and cons of group-based norms are discussed in @sec-withinGroupNorming.\index{norm-referenced}\index{norm}
Guidelines for reporting on norm-referenced scores are provided by @TimmermanInPress.\index{norm-referenced}\index{norm}

A standard normal distribution on various norm-referenced scales is depicted in @fig-normedDistribution, as adapted from @Bandalos2018.\index{norm-referenced}

```{r}
#| label: fig-normedDistribution
#| fig-cap: "Various Norm-Referenced Scales."
#| fig-alt: "Various Norm-Referenced Scales."
#| echo: false
...
#### Percentile Ranks {#sec-percentileRanks}
...
A histogram of percentile rank scores is in @fig-percentileRanks.\index{data!percentile rank}\index{histogram}

```{r}
#| label: fig-percentileRanks
#| fig-cap: "Histogram of Percentile Ranks."
#| fig-alt: "Histogram of Percentile Ranks."
#| code-fold: true
...
#### Deviation (Standardized) Scores {#sec-standardizedScores}
...
With deviation scores, you have similar challenges as with [percentile ranks](#sec-percentileRanks), including which reference group to use, but there are additional challenges.\index{data!standardized}\index{data!percentile rank}
...
It might follow a folded distribution, as depicted in @fig-foldedDistribution:\index{data!not normally distributed}\index{histogram}

```{r}
#| label: fig-foldedDistribution
#| fig-cap: "Histogram of Hallucinations (Raw Score)."
#| fig-alt: "Histogram of Hallucinations (Raw Score)."
#| code-fold: true
...
Now consider the same distribution converted to a standardized (*z*) score, as depicted in @fig-foldedDistributionZscore:\index{data!standardized}\index{data!\textit{z} score}\index{histogram}

```{r}
#| label: fig-foldedDistributionZscore
#| fig-cap: "Histogram of Hallucinations (z Score)."
#| fig-alt: "Histogram of Hallucinations (z Score)."
#| code-fold: true
...
The area under a normal curve within one standard deviation of the mean is depicted in @fig-zScoreDensity1SD.\index{data!standardized}
...
#| fig-alt: "Density of Standard Normal Distribution. The blue region represents the area within one standard deviation of the mean."
...
The area under a normal curve within two standard deviations of the mean is depicted in @fig-zScoreDensity2SD.\index{data!standardized}
...
#| fig-alt: "Density of Standard Normal Distribution. The blue region represents the area within two standard deviations of the mean."
...
The area under a normal curve within three standard deviations of the mean is depicted in @fig-zScoreDensity3SD.\index{data!standardized}
...
#| fig-alt: "Density of Standard Normal Distribution. The blue region represents the area within three standard deviations of the mean."
...
*z* scores are calculated using @eq-zScore:\index{data!standardized}\index{data!\textit{z} score}
...
An example histogram of *z* scores is in @fig-zScores.\index{data!standardized}\index{data!\textit{z} score}\index{histogram}

```{r}
#| label: fig-zScores
#| fig-cap: "Histogram of *z* Scores."
#| fig-alt: "Histogram of *z* Scores."
#| code-fold: true
...
*T* scores are calculated using @eq-TScore:\index{data!standardized}\index{data!\textit{T} score}
...
An example histogram of *T* scores is in @fig-tScores.\index{data!\textit{T} score}\index{histogram}

```{r}
#| label: fig-tScores
#| fig-cap: "Histogram of *T* Scores."
#| fig-alt: "Histogram of *T* Scores."
#| code-fold: true
...
Standard scores with a mean of 100 and standard deviation of 15 are calculated using @eq-standardScore:\index{data!standardized}\index{data!standard score}
...
An example histogram of standard scores is in @fig-standardScores.\index{data!standardized}\index{data!standard score}\index{histogram}

```{r}
#| label: fig-standardScores
#| fig-cap: "Histogram of Standard Scores."
#| fig-alt: "Histogram of Standard Scores."
#| code-fold: true
...
##### Scaled scores {#sec-scaledScores}
...
Using this metric, scaled scores can be calculated using @eq-scaleScore:\index{data!standardized}\index{data!scaled score}
...
An example histogram of scaled scores is in @fig-scaledScores.\index{data!standardized}\index{data!scaled score}\index{histogram}

```{r}
#| label: fig-scaledScores
#| fig-cap: "Histogram of Scaled Scores."
#| fig-alt: "Histogram of Scaled Scores."
#| code-fold: true
...
##### Stanine scores {#sec-stanineScores}
...
Stanine scores are calculated using the bracketed proportions in @tbl-stanineConversionTable.\index{data!standardized}\index{data!stanine score}
...
```{r}
#| label: tbl-stanineConversionTable
#| tbl-cap: "Calculating Stanine Scores."
#| echo: false
...
A histogram of stanine scores is in @fig-stanineScores.\index{data!standardized}\index{data!stanine score}\index{data!histogram}

```{r}
#| label: fig-stanineScores
#| fig-cap: "Histogram of Stanine Scores."
#| fig-alt: "Histogram of Stanine Scores."
#| code-fold: true
...
## Conclusion {#sec-conclusion-scoresScales}

It is important to consider the [types of data](#sec-dataTypes) your data are because the [types of data](#sec-dataTypes) restrict what options are available to analyze the data.\index{data!type}
It is also important to consider whether the data were [transformed](#sec-scoreTransformation) because score transformations are not neutral—they can impact the results.\index{data!transformation}

## Suggested Readings {#sec-readings-scoresScales}

@Childs2021: <https://dzchilds.github.io/stats-for-bio/data-transformations.html> (archived at <https://perma.cc/3YEB-QW2V>)

## Exercises {#sec-exercises-scoresScales}

## Session Info {#sec-sessionInfo-scoresScales}
